...
- **SAD and SSAD share one gate and one module set.** Per the 2026-08-17 clarification, SSAD is a variation on SAD serving the same purpose, not a structurally distinct artefact. Both are declared as separate `artefacts` entries in `definition.yaml`, with field-level `requires` lists over the same shared modules rather than each having its own module set — consistent with Gantry's "data over documents" principle. Their requirements differ because SAD and SSAD present different views of that shared data; see ADR-0021.
