...
**Amended by `#129` (2026-09-23), naming only.** The two environment variables this decision introduced for the MCP server were renamed to carry the `GANTRY_MCP_` prefix the access token already used — `GANTRY_BASE_URL` -> `GANTRY_MCP_BASE_URL`, `GANTRY_WORKSPACE_PATS` -> `GANTRY_MCP_WORKSPACE_PATS` — with no aliases, matching the `GANTRY_BOOTSTRAP_PATS` -> `GANTRY_SHARED_WORKSPACE_PATS` precedent in `docs/adr/0047`. Because this ADR's whole point is that the MCP server is a *separate deployment* reading a *separate* set of variables, the unprefixed names read as though the `gantry serve` web service owned them; set there they did nothing, silently, and cost a real afternoon of misdiagnosis. Nothing about the decision above changes — same two auth layers, same boot-time PAT map, same thin-client shape — so the original names are left standing in the text as the record of what was decided. `mcp-server/README.md` is the current configuration reference.
