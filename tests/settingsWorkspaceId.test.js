...
// and the MCP server's `GANTRY_MCP_WORKSPACE_PATS` (ADR-0043) are both keyed by, so it only exists for a
...
test('a registered remote workspace shows the id its credential-map entry would be keyed by', async () => {
  await withScratchInstances((instancesDir) => {
    ...
    // `GANTRY_SHARED_WORKSPACE_PATS`/`GANTRY_MCP_WORKSPACE_PATS` are actually keyed by, so this reads it back
