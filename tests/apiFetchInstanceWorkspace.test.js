...
// `apiFetchForInstance`'s workspace-id-for-slug cache (#104, web/lib/
// apiFetch.js) — a review pass on this ticket flagged the original version
// as caching *every* lookup outcome, including a failed one, permanently
// for the page's life. This exercises the fix directly: a stubbed global
// `fetch` simulates a transient failure on the first lookup, then a real
// answer on the second, proving the failure was not cached and a later
// call still applies the workspace's PAT override.
//
// `credential.js` is imported as a plain (non-cache-busted) static import
// here deliberately — it's the same singleton module instance
// `web/lib/apiFetch.js`'s own internal `import './credential.js'` resolves
// to, so calls to `setPat`/`setWorkspacePatOverride` here are visible to
// `apiFetch.js` without needing to reach into its internals. Only
// `apiFetch.js` itself is re-imported fresh per test (via a cache-busting
// query string), to reset its own private `workspaceIdBySlug` cache
// between tests.
...
test('a failed workspace-id lookup is not cached — a later successful lookup still applies that workspace\'s PAT override', async () => {
  ...
  try {
    ...
    globalThis.fetch = async (url, options) => {
      ...
      // The "real" instance-data request — reports back whatever
      // Authorization header apiFetch actually attached, so the test can
      // assert on which PAT was used without a real server.
    ...
    // First call: the workspace lookup fails, so this falls back to the
    // global default rather than the workspace's override.
    ...
    // Second call: the lookup now succeeds. If the earlier failure had been
    // cached, this would still resolve to "no workspace" and reuse the
    // global default — asserting on the override here is the whole point.
    failLookup = false
    ...
    // Third call: the now-successful resolution *is* cached — no further
