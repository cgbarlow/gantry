...
// ---------- addLibraryRepo (pre-#3 flat shape — no consumer of this shape is migrated) ----------

test('addLibraryRepo persists organization/project/repository, generates an id and addedAt', async () => {
  await withScratchInstances((instancesDir) => {
    const repo = addLibraryRepo(LOCATION, { instancesDir })
    assert.equal(typeof repo.id, 'string')
    assert.ok(repo.id.length > 0)
    assert.equal(repo.organization, 'fake-org')
    assert.equal(repo.project, 'fake-project')
    assert.equal(repo.repository, 'fake-repo')
...
test('addLibraryRepo persists an optional codeOwner and baseUrl', async () => {
  await withScratchInstances((instancesDir) => {
    const repo = addLibraryRepo({ ...LOCATION, baseUrl: 'https://ado.example.internal', codeOwner: 'c.barlow' }, { instancesDir })
    assert.equal(repo.baseUrl, 'https://ado.example.internal')
...
test('addLibraryRepo rejects a location missing organization/project/repository', async () => {
  await withScratchInstances((instancesDir) => {
    assert.throws(() => addLibraryRepo({ organization: 'org' }, { instancesDir }), /missing: project, repository/)
  })
})

test('addLibraryRepo always creates a fresh id, even for a repeated location', async () => {
  await withScratchInstances((instancesDir) => {
    const first = addLibraryRepo(LOCATION, { instancesDir })
    const second = addLibraryRepo(LOCATION, { instancesDir })
...
test('addLibraryRepo accepts the nested {provider, location} shape for an Azure DevOps repo, and emits both shapes', async () => {
  await withScratchInstances((instancesDir) => {
...
test('resolveLibraryRepo round-trips a registered repo by id, and returns undefined for an unknown one', async () => {
  await withScratchInstances((instancesDir) => {
    const created = addLibraryRepo(LOCATION, { instancesDir })
...
test('resolveLibraryRepo returns undefined for "__proto__" — an inherited property, never a real registry entry', async () => {
  await withScratchInstances((instancesDir) => {
    addLibraryRepo(LOCATION, { instancesDir })
...
test('findLibraryRepoByLocation matches within a provider only — a GitHub repo sharing a repository name with an Azure DevOps one is distinct', async () => {
  await withScratchInstances((instancesDir) => {
    const ado = addLibraryRepo({ organization: 'shared', project: 'shared', repository: 'shared-repo' }, { instancesDir })
    const gh = addLibraryRepo({ provider: 'github', location: { owner: 'shared', repository: 'shared-repo' } }, { instancesDir })

    assert.deepEqual(
      findLibraryRepoByLocation({ organization: 'shared', project: 'shared', repository: 'shared-repo' }, { instancesDir }),
      ado
    )
    assert.deepEqual(
...
test('findLibraryRepoByLocation returns undefined when nothing matches', async () => {
  await withScratchInstances((instancesDir) => {
    addLibraryRepo(LOCATION, { instancesDir })
    assert.equal(findLibraryRepoByLocation({ organization: 'other', project: 'p', repository: 'r' }, { instancesDir }), undefined)
  })
})
