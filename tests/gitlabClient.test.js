...
// lib/gitlabClient.js's content-store capability (#26): proving a PAT reaches a real project
// (getRepo/repoExists), reading an existing file/folder over the Repository Files and Repository Tree
...
test('getRepo resolves the project metadata when the PAT is accepted and the project exists', async () => {
  await withFakeGitLabServer({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT }, async (baseUrl) => {
    ...
    assert.equal(repo.path, GITLAB_REPOSITORY)
    assert.equal(repo.path_with_namespace, `${GITLAB_NAMESPACE}/${GITLAB_REPOSITORY}`)
  })
})

test('repoExists is true when the project exists and the PAT is accepted', async () => {
  await withFakeGitLabServer({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT }, async (baseUrl) => {
    const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: GITLAB_VALID_PAT, baseUrl })
...
test('getRepo throws GitLabAuthenticationError when GitLab rejects the PAT', async () => {
  await withFakeGitLabServer({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT }, async (baseUrl) => {
    const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: 'wrong-pat', baseUrl })
    await assert.rejects(() => client.getRepo(), GitLabAuthenticationError)
  })
})

test('getRepo throws GitLabRepoNotFoundError when the project does not exist', async () => {
  await withFakeGitLabServer(
    ...
    async (baseUrl) => {
      const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: GITLAB_VALID_PAT, baseUrl })
      await assert.rejects(() => client.getRepo(), GitLabRepoNotFoundError)
    }
  )
})

test("getRepo's not-found message explains that a missing PAT scope looks identical to a missing project", async () => {
  await withFakeGitLabServer(
    { namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT, repoExists: false },
    async (baseUrl) => {
      const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: GITLAB_VALID_PAT, baseUrl })
...
test('repoExists is false, not thrown, when the project does not exist', async () => {
  await withFakeGitLabServer(
    { namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT, repoExists: false },
    async (baseUrl) => {
      const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: GITLAB_VALID_PAT, baseUrl })
...
test('getRepo tags a repo-not-found error with provider: "gitlab"', async () => {
  await withFakeGitLabServer(
    { namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT, repoExists: false },
    async (baseUrl) => {
      const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: GITLAB_VALID_PAT, baseUrl })
...
test('a self-hosted-shaped baseUrl (a host + /api/v4 path) is honoured as-is', async () => {
  await withFakeGitLabServer({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT }, async (baseUrl) => {
    // The fake server serves its own /api/v4 route already — this proves the client sends requests to
    // exactly the baseUrl it was given, the same convention a real self-hosted GitLab CE/EE host would need.
    const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: GITLAB_VALID_PAT, baseUrl })
...
test('getFileContent reads an existing file back unchanged', async () => {
  await withFakeGitLabServer(
    { namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT, files: { 'definitions/design/1/definition.yaml': 'id: design\n' } },
    async (baseUrl) => {
      const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: GITLAB_VALID_PAT, baseUrl })
...
test('getFileContent on a missing path throws the neutral NotFoundError, tagged gitlab', async () => {
  await withFakeGitLabServer({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT }, async (baseUrl) => {
    const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: GITLAB_VALID_PAT, baseUrl })
    await assert.rejects(() => client.getFileContent('does/not/exist.yaml'), (err) => {
      assert.ok(err instanceof GitLabNotFoundError)
...
test('getFileContent on a directory path throws NotFoundError (a directory is not a file)', async () => {
  await withFakeGitLabServer(
    ...
    async (baseUrl) => {
      const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: GITLAB_VALID_PAT, baseUrl })
      await assert.rejects(() => client.getFileContent('definitions/design'), GitLabNotFoundError)
    }
  )
})

test('fileExists is true after a file is present, false for an unwritten path', async () => {
  await withFakeGitLabServer({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT, files: { 'exists.md': 'x\n' } }, async (baseUrl) => {
    const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: GITLAB_VALID_PAT, baseUrl })
...
test('a rejected PAT surfaces as the neutral AuthenticationError, tagged gitlab', async () => {
  await withFakeGitLabServer({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT }, async (baseUrl) => {
    ...
    await assert.rejects(() => client.getFileContent('anything.md'), (err) => {
      assert.ok(err instanceof GitLabAuthenticationError)
...
test('writeFile on an already-existing path reports changeType "edit", and getFileContent reads the new content', async () => {
  await withFakeGitLabServer(
    { namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT, files: { 'notes/hello.md': 'v1\n' } },
    async (baseUrl) => {
      const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: GITLAB_VALID_PAT, baseUrl })
...
test('writeFiles lands several files in exactly one commit, leaving an untouched sibling file byte-identical', async () => {
  await withFakeGitLabServer(
    { namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT, files: { 'modules/untouched.md': 'unchanged\n' } },
    async (baseUrl) => {
      const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: GITLAB_VALID_PAT, baseUrl })
...
test('writeFiles targets a non-default branch without touching main', async () => {
  await withFakeGitLabServer(
    { namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT, files: { 'main-only.md': 'main\n' }, branchFiles: { 'feature-branch': { 'main-only.md': 'main\n' } } },
    async (baseUrl) => {
      const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: GITLAB_VALID_PAT, baseUrl })
...
test('a rejected PAT on writeFile surfaces as the neutral AuthenticationError, tagged gitlab', async () => {
  await withFakeGitLabServer({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT }, async (baseUrl) => {
    const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: 'wrong-pat', baseUrl })
...
test('deleteFile removes an existing file, and a delete on a path that never existed is a harmless no-op', async () => {
  await withFakeGitLabServer(
    { namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT, files: { 'notes/gone.md': 'bye\n' } },
    async (baseUrl) => {
      const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: GITLAB_VALID_PAT, baseUrl })
      await client.deleteFile('notes/gone.md')
      assert.equal(await client.fileExists('notes/gone.md'), false)

      // No files ever existed there — writeFiles' actions array is empty, short-circuited before any
      // network call (GitLab's own Commits API refuses an empty actions list).
...
test('createBranch creates a new branch pointing at the source branch\'s current tip, and branchExists then reports it', async () => {
  await withFakeGitLabServer({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT, files: { 'instance.yaml': 'stage: shape\n' } }, async (baseUrl) => {
    const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: GITLAB_VALID_PAT, baseUrl })
...
test('createBranch can stack a new branch on another (non-main) branch, not just on main', async () => {
  await withFakeGitLabServer(
    { namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT, files: { 'instance.yaml': 'stage: shape\n' } },
    async (baseUrl) => {
      const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: GITLAB_VALID_PAT, baseUrl })
...
test('createBranch points the new branch at the current tip of "from", and a write on the new branch never leaks back onto "from"', async () => {
  await withFakeGitLabServer({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT, files: { 'README.md': '# repo' } }, async (baseUrl) => {
    const client = createGitLabClient({ namespace: GITLAB_NAMESPACE, repository: GITLAB_REPOSITORY, pat: GITLAB_VALID_PAT, baseUrl })
...
// ---------- namespace addressing (ADR-0041) ----------

test('a multi-segment namespace (a subgroup path) is addressed correctly end to end', async () => {
  const namespace = 'engineering/platform/backend-services'
  await withFakeGitLabServer({ namespace, repository: GITLAB_REPOSITORY, validPat: GITLAB_VALID_PAT }, async (baseUrl) => {
    const client = createGitLabClient({ namespace, repository: GITLAB_REPOSITORY, pat: GITLAB_VALID_PAT, baseUrl })
    assert.equal(await client.repoExists(), true)
