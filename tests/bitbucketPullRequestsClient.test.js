...
test('interpretBitbucketPullRequest reads "pending" for a pull request with no participant action yet', () => {
  ...
  assert.equal(interpretBitbucketPullRequest({ participants: [{ state: null }] }), 'pending')
  assert.equal(interpretBitbucketPullRequest({}), 'pending')
})

test('interpretBitbucketPullRequest reads "approved" once a participant\'s state is approved', () => {
  assert.equal(interpretBitbucketPullRequest({ participants: [{ state: 'approved' }] }), 'approved')
})

test('interpretBitbucketPullRequest reads "changes-requested" once a participant requests changes, even alongside another approval', () => {
  assert.equal(interpretBitbucketPullRequest({ participants: [{ state: 'approved' }, { state: 'changes_requested' }] }), 'changes-requested')
})
