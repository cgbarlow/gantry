/**
 * Browser-side verbatim port of `lib/copyPlanner.js` (WI #382, Feature #380 phase 2) — same
 * "port, not an import" convention as `web/lib/localStatus.js`'s copy of `lib/status.js`: `web/`
 * is served straight to the browser with no build step, so it can't `import` a top-level `lib/`
 * file directly (nothing serves that directory to the browser, unlike `web/` and `node_modules/`
 * — see `lib/server.js`). This file is pure (no DOM, no `node:fs`, no fetch) so the port is a
 * plain copy with no browser-specific rewriting needed; keep it in lock-step with `lib/copyPlanner.js`
 * — that file, and `tests/copyPlanner.test.js`, are the source of truth and the tested copy.
 *
