...
export async function requestStageReview(slug, { reviewer, stageId, instanceUrl, requestedBy } = {}, options = {}) {
  ...
  const review = {
    ...
    status: created.fields?.[REVIEW_STATUS_FIELD] ?? REVIEW_STATUS.REQUESTED,
  }
...
export async function checkStageReviewStatus(slug, { reviewId, stageId } = {}, options = {}) {
  ...
  const workItem = await client.getWorkItem(review.workItemId, { fields: ['System.State', REVIEW_STATUS_FIELD] })
  const status =
