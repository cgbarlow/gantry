...
/**
 * Jira Cloud's own carrier for this same five-value lifecycle (#47, docs/adr/0042: "Status rides the
 * same reserved gantry:review/<status> labels already used on GitHub and GitLab issues — identical
 * vocabulary and slugs, since it's one gantry-owned convention layered onto a third issue tracker, not
 * a per-provider naming choice"). Unlike GitHub/GitLab, Jira's own `labels` field needs no on-demand
 * "does this label exist yet" registry step first (`ensureLabelsExist`'s GitHub/GitLab twin has none
 * here): a Jira Cloud label is a freeform string with no separate colour/description resource of its
 * own — attaching one to an issue's `fields.labels` at create time is enough, so there is no
 * `allJiraReviewLabels`/colour table to go with this pair either.
 */
export const JIRA_REVIEW_LABEL_PREFIX = 'gantry:review/'

const JIRA_REVIEW_LABEL_SLUGS = Object.freeze({
  [REVIEW_STATUS.REQUESTED]: 'requested',
  [REVIEW_STATUS.IN_REVIEW]: 'in-review',
  [REVIEW_STATUS.CHANGES_REQUESTED]: 'changes-requested',
  [REVIEW_STATUS.APPROVED]: 'approved',
...
/** The `gantry:review/<slug>` label name carrying `status`, Jira's twin of `reviewStatusToGitHubLabel`/`reviewStatusToGitLabLabel`. Throws for anything outside `REVIEW_STATUS_VALUES`. */
export function reviewStatusToJiraLabel(status) {
  const slug = JIRA_REVIEW_LABEL_SLUGS[status]
  if (!slug) throw new Error(`No gantry:review/* label is mapped for review status "${status}"`)
  return `${JIRA_REVIEW_LABEL_PREFIX}${slug}`
}

/**
 * The reverse of `reviewStatusToJiraLabel`, applied to a whole label-name list — Jira's twin of
 * `gitHubLabelToReviewStatus`/`gitlabLabelToReviewStatus`. Jira Cloud's REST API v3 already returns
 * `fields.labels` as an array of plain name strings (the same shape GitLab's own Issues API returns,
 * unlike GitHub's array of label objects), so a caller here passes an issue's `labels` straight
 * through with no `.map((l) => l.name)` step first. A human is free to remove every gantry:review/*
 * label from an issue (or the issue could predate this feature entirely); this returns `undefined` for
 * that case rather than guessing a status.
 */
export function jiraLabelToReviewStatus(labelNames) {
  for (const status of REVIEW_STATUS_VALUES) {
    if (labelNames.includes(reviewStatusToJiraLabel(status))) return status
  }
  return undefined
}
