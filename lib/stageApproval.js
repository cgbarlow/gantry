import { loadDefinition, artefactModuleIds } from './definition.js'
import {
  readInstance,
  readModule,
...
import { createBitbucketPullRequestsClient } from './bitbucketPullRequestsClient.js'
import { createBitbucketIdentityClient } from './bitbucketIdentityClient.js'
import { createBitbucketClient } from './bitbucketClient.js'
import { listWorkspaces } from './workspaceRegistry.js'
import { summarizePullRequest, summarizeGitHubPullRequest, summarizeGitLabPullRequest, summarizeBitbucketPullRequest } from './stageStatus.js'
...
import { artefactFileUrl as gitlabArtefactFileUrl, mergeRequestUrl as gitlabPullRequestUrl } from './gitlabFileUrl.js'
import { artefactFileUrl as atlassianArtefactFileUrl, pullRequestUrl as atlassianPullRequestUrl } from './atlassianFileUrl.js'
import { renderStageArtefacts } from './render.js'

// #87 (ADR-0045): an artefact's `filename:` pattern resolves against this instance's own
// module data, the same as at render time (lib/render.js's compileArtefact) — so checking
// whether a gate-satisfying artefact's rendered document is actually on the branch means
// resolving the same pattern here, not just the unpatterned "<Instance name> - <Artefact
// title>" path. Loaded once per verify pass, for only the modules the target artefacts'
// `requires` lists actually touch (mirrors renderArtefact's own `artefactModuleIds` scoping).
async function loadModuleDataForArtefacts(definition, slug, artefacts, providerOptions) {
  const moduleIds = new Set(artefacts.flatMap((artefact) => artefactModuleIds(artefact)))
  const modules = {}
  for (const moduleId of moduleIds) {
    // eslint-disable-next-line no-await-in-loop
    modules[moduleId] = (await readModule(definition, slug, moduleId, providerOptions)).fields
  }
  return modules
}
...
async function verifyRenderedArtefacts(definition, stage, slug, instanceName, azureDevOps, checkResult) {
  ...
  const modules = await loadModuleDataForArtefacts(definition, slug, artefacts, { azureDevOps })
  const client = createAzureDevOpsClient(azureDevOps)
  const results = await Promise.all(
    artefacts.map(async (artefact) => {
      const path = azureDevOpsOutPath(slug, instanceName, artefact.title, 'docx', filenamePatternContext(artefact, modules, slug))
...
export async function requestStageApproval(slug, options = {}) {
  ...
  if (options.gitlab) {
    return requestGitLabStageApproval(slug, options)
  }
  if (options.atlassian) {
    return requestAtlassianStageApproval(slug, options)
  }
...
async function verifyRenderedGitHubArtefacts(definition, stage, slug, instanceName, github, checkResult) {
  ...
  const modules = await loadModuleDataForArtefacts(definition, slug, artefacts, { github })
  const client = createGitHubClient(github)
  const results = await Promise.all(
    artefacts.map(async (artefact) => {
      const path = githubOutPath(slug, instanceName, artefact.title, 'docx', filenamePatternContext(artefact, modules, slug))
...
// The GitLab twin of verifyRenderedGitHubArtefacts above (#33) — same "every gate-satisfying
// artefact must already be rendered onto this branch before a Merge Request opens" check, over the
// GitLab content store and gitlabOutPath's own naming convention.
async function verifyRenderedGitLabArtefacts(definition, stage, slug, instanceName, gitlab, checkResult) {
  const completeArtefactIds = new Set(
    checkResult.artefacts.filter((artefact) => artefact.complete).map((artefact) => artefact.id)
  )
  const artefacts = definition.artefacts.filter(
  ...
  const modules = await loadModuleDataForArtefacts(definition, slug, artefacts, { gitlab })
  const client = createGitLabClient(gitlab)
  const results = await Promise.all(
    artefacts.map(async (artefact) => {
      const path = gitlabOutPath(slug, instanceName, artefact.title, 'docx', filenamePatternContext(artefact, modules, slug))
      if (await client.fileExists(path, gitlab.branch)) return null
      return { artefact, path }
    })
  )
  const missing = results.filter(Boolean)
  ...
  if (missing.length > 0) {
    ...
    throw new Error(
      `Cannot request approval: required rendered artefact${missing.length === 1 ? '' : 's'} ` +
        `${missingSummary} missing from GitLab branch "${gitlab.branch}"` +
        `${missing.length === 1 ? '' : `: ${details}`}`
    )
  }
  ...
  return artefacts.map((artefact) => {
    const path = gitlabOutPath(slug, instanceName, artefact.title, 'docx', filenamePatternContext(artefact, modules, slug))
    return { artefact, path, url: gitlabArtefactFileUrl(gitlab, path, gitlab.branch) }
  })
}

/**
 * The GitLab twin of `requestGitHubStageApproval` above (#33, ADR-0041: "full capability parity ...
 * Merge-Request-gated sign-off"). Same preconditions, same auto-render-then-verify sequence, a GitLab
 * Merge Request in place of a Pull Request:
 *
 * - throws if the gate hasn't passed, if the stage has no branch yet, if a gate-satisfying artefact
 *   isn't yet rendered onto that branch, or if this stage already has an open Merge Request recorded
 *   (unless the stage was re-opened, mirroring the GitHub path's own `isReopened` bypass);
 * - resolves the effective required reviewer the same way (instance override, else workspace Owner)
 *   and attaches them as a requested reviewer if resolved (`reviewer_ids`) — GitLab has no
 *   API-level "required reviewer" any more than GitHub does (only enforced Approval Rules on
 *   Premium/Ultimate, honoured at Check-status time, ADR-0041), so this only ever *requests* review.
 *
 * Deliberately does not implement the Azure DevOps path's stale-approval-reset branch (ADR-0018): that
 * is Azure DevOps's own reviewer-vote-reset mechanism and has no GitLab client support, same scope note
 * `lib/stageStatus.js`'s `checkGitLabStageApprovalStatus` doc comment carries for the GitHub path.
 */
async function requestGitLabStageApproval(slug, options = {}) {
  const definitionsDir = options.definitionsDir ?? 'definitions'
  ...
  if (!branch) {
    throw new Error(
      `Instance "${slug}" has no branch for stage "${stage.id}" yet — nothing to open a Merge Request from`
    )
  }
  const gitlab = { ...gitlabBase, branch }

  const checkResult = await checkGate(slug, { gitlab, definitionsDir, gate: options.gate })
  if (!checkResult.pass) {
    throw new Error(
      `Gate "${checkResult.gate}" (stage "${checkResult.stage.id}") has not passed for instance "${slug}" — ` +
        `outstanding: ${formatGateOutstanding(checkResult)}`
...

  {
    ...
    if (targetArtefacts.length > 0) {
      ...
      const missingArtefacts = []
      for (const artefact of targetArtefacts) {
        const path = gitlabOutPath(slug, instanceName, artefact.title)
        // eslint-disable-next-line no-await-in-loop
        if (!(await client.fileExists(path, gitlab.branch))) missingArtefacts.push(artefact)
      }
      if (missingArtefacts.length > 0) {
        const filteredDefinition = { ...definition, artefacts: missingArtefacts }
        const results = await renderStageArtefacts(slug, filteredDefinition, stage, { gitlab, definitionsDir })
        for (const result of results) {
          if (!result.rendered) {
            if (result.error) throw new Error(result.error)
            if (result.skipped) throw new Error(result.reason ?? `Artefact ${result.artefactId} could not be rendered`)
  ...
  const renderedArtefacts = await verifyRenderedGitLabArtefacts(
    definition,
    stage,
    slug,
    instanceDisplayName(instance),
  ...
  // Effective reviewer = per-instance override if set, else the workspace's Owner — same resolution
  // order as the Azure DevOps/GitHub paths.
  const requiredReviewerOverride = instance.requiredReviewer ?? ''
  let effectiveReviewerValue = requiredReviewerOverride
  let reviewerSource = 'instance override'
  ...
  if (!effectiveReviewerValue) {
    ...
    const workspace = workspaces.find(
      (ws) =>
        ws.provider === 'gitlab' &&
        ws.location.namespace === gitlabBase.namespace &&
        ws.location.repository === gitlabBase.repository
    )
    effectiveReviewerValue = workspace?.owner ?? ''
    reviewerSource = 'workspace Owner'
  }
  ...
  if (effectiveReviewerValue && effectiveReviewerValue.trim()) {
    const identityClient = createGitLabIdentityClient({
      namespace: gitlabBase.namespace,
      repository: gitlabBase.repository,
      baseUrl: gitlabBase.baseUrl,
      pat: gitlabBase.pat,
    })
    ...
    if (!resolvedReviewer) {
      throw new Error(
        `Cannot request approval: the ${reviewerSource} ("${effectiveReviewerValue}") could not be resolved ` +
          `to a known GitLab identity. If the person has left the project/group, update the ` +
          `${reviewerSource} ${reviewerSource === 'instance override' ? 'in Instance Settings' : 'in Workspace Settings'}.`
      )
    }
  }
  ...
  const pullRequest = await client.createPullRequest({
    ...
    description:
      `Requests approval for the "${stage.title}" stage of gantry instance "${slug}" (gate "${stage.gate}"). ` +
      'Merging this Merge Request completes the stage.\n\n' +
      'Rendered artefacts for review:\n' +
      renderedArtefacts.map(({ artefact, url }) => `- [${artefact.title}](${url})`).join('\n'),
  })

  if (resolvedReviewer) {
    await client.addReviewers(pullRequest.pullRequestId, [{ id: resolvedReviewer.id, required: true }])
  }
  ...
  const commits = await client.getPullRequestCommits(pullRequest.pullRequestId)
  const pullRequestSummary = summarizeGitLabPullRequest(await client.getPullRequest(pullRequest.pullRequestId), commits)

  const webUrl = gitlabPullRequestUrl(gitlabBase, pullRequest.pullRequestId)
...
// The Atlassian (Bitbucket-backed) twin of verifyRenderedGitLabArtefacts above (#46, ADR-0042) — same
// "every gate-satisfying artefact must already be rendered onto this branch before a pull request
// opens" check, over the Bitbucket content store and atlassianOutPath's own naming convention.
async function verifyRenderedAtlassianArtefacts(definition, stage, slug, instanceName, atlassian, checkResult) {
  const completeArtefactIds = new Set(
    checkResult.artefacts.filter((artefact) => artefact.complete).map((artefact) => artefact.id)
  )
  const artefacts = definition.artefacts.filter(
  ...
  const modules = await loadModuleDataForArtefacts(definition, slug, artefacts, { atlassian })
  const client = createBitbucketClient(atlassian)
  const results = await Promise.all(
    artefacts.map(async (artefact) => {
      const path = atlassianOutPath(slug, instanceName, artefact.title, 'docx', filenamePatternContext(artefact, modules, slug))
      if (await client.fileExists(path, atlassian.branch)) return null
      return { artefact, path }
    })
  )
  const missing = results.filter(Boolean)
  ...
  if (missing.length > 0) {
    ...
    throw new Error(
      `Cannot request approval: required rendered artefact${missing.length === 1 ? '' : 's'} ` +
        `${missingSummary} missing from Bitbucket branch "${atlassian.branch}"` +
        `${missing.length === 1 ? '' : `: ${details}`}`
    )
  }
  ...
  return artefacts.map((artefact) => {
    const path = atlassianOutPath(slug, instanceName, artefact.title, 'docx', filenamePatternContext(artefact, modules, slug))
    return { artefact, path, url: atlassianArtefactFileUrl(atlassian, path, atlassian.branch) }
  })
}

/**
 * The Atlassian (Bitbucket-backed) twin of `requestGitLabStageApproval`/`requestGitHubStageApproval`
 * above (#46, ADR-0042: "full capability parity ... pull requests with sign-off gating"). Same
 * preconditions, same auto-render-then-verify sequence, a Bitbucket pull request in place of a Merge
 * Request/Pull Request:
 *
 * - throws if the gate hasn't passed, if the stage has no branch yet, if a gate-satisfying artefact
 *   isn't yet rendered onto that branch, or if this stage already has an open pull request recorded
 *   (unless the stage was re-opened, mirroring the GitHub/GitLab paths' own `isReopened` bypass — this
 *   ticket (#46) has no re-open path of its own to actually set that marker: no ticket in the current
 *   Atlassian batch adds one yet, unlike GitHub's #13/GitLab's #33, which each bundled re-open into
 *   their own sign-off ticket, so `isReopened` is always `false` for an Atlassian-backed instance
 *   today and this bypass simply never fires — kept here anyway so a later re-open ticket needs no
 *   change to this function at all, mirroring the exact shape every other provider's path already has);
 * - resolves the effective required reviewer the same way (instance override, else workspace Owner)
 *   and attaches them via `addReviewers` if resolved — Bitbucket has no API-level "required reviewer"
 *   any more than GitHub/GitLab do, so this only ever *requests* review, exactly the same limitation
 *   `lib/bitbucketPullRequestsClient.js`'s own `addReviewers` doc comment states.
 *
 * Deliberately does not implement the Azure DevOps path's stale-approval-reset branch (ADR-0018): that
 * is Azure DevOps's own reviewer-vote-reset mechanism and has no Bitbucket client support, same scope
 * note `lib/stageStatus.js`'s `checkBitbucketStageApprovalStatus` doc comment carries for the
 * GitHub/GitLab paths.
 */
async function requestAtlassianStageApproval(slug, options = {}) {
  const definitionsDir = options.definitionsDir ?? 'definitions'
  ...
  const atlassian = { ...atlassianBase, branch }

  const checkResult = await checkGate(slug, { atlassian, definitionsDir, gate: options.gate })
  if (!checkResult.pass) {
    throw new Error(
      `Gate "${checkResult.gate}" (stage "${checkResult.stage.id}") has not passed for instance "${slug}" — ` +
        `outstanding: ${formatGateOutstanding(checkResult)}`
...

  {
    ...
    if (targetArtefacts.length > 0) {
      ...
      const missingArtefacts = []
      for (const artefact of targetArtefacts) {
        const path = atlassianOutPath(slug, instanceName, artefact.title)
        // eslint-disable-next-line no-await-in-loop
        if (!(await client.fileExists(path, atlassian.branch))) missingArtefacts.push(artefact)
      }
      if (missingArtefacts.length > 0) {
        const filteredDefinition = { ...definition, artefacts: missingArtefacts }
        const results = await renderStageArtefacts(slug, filteredDefinition, stage, { atlassian, definitionsDir })
        for (const result of results) {
          if (!result.rendered) {
            if (result.error) throw new Error(result.error)
            if (result.skipped) throw new Error(result.reason ?? `Artefact ${result.artefactId} could not be rendered`)
  ...
  const renderedArtefacts = await verifyRenderedAtlassianArtefacts(
    definition,
    stage,
    slug,
    instanceDisplayName(instance),
  ...
  // Effective reviewer = per-instance override if set, else the workspace's Owner — same resolution
  // order as the Azure DevOps/GitHub/GitLab paths.
  const requiredReviewerOverride = instance.requiredReviewer ?? ''
  let effectiveReviewerValue = requiredReviewerOverride
  let reviewerSource = 'instance override'
  ...
  if (!effectiveReviewerValue) {
    ...
    const workspace = workspaces.find(
      (ws) =>
        ws.provider === 'atlassian' &&
        ws.location.owner === atlassianBase.owner &&
        ws.location.repository === atlassianBase.repository
    )
    effectiveReviewerValue = workspace?.owner ?? ''
    reviewerSource = 'workspace Owner'
  }
  ...
  let resolvedReviewer = null
  if (effectiveReviewerValue && effectiveReviewerValue.trim()) {
    const identityClient = createBitbucketIdentityClient({
      owner: atlassianBase.owner,
      repository: atlassianBase.repository,
      baseUrl: atlassianBase.baseUrl,
      pat: atlassianBase.pat,
    })
    ...
    if (!resolvedReviewer) {
      throw new Error(
        `Cannot request approval: the ${reviewerSource} ("${effectiveReviewerValue}") could not be resolved ` +
          `to a known Bitbucket account. If the person has left the account/repository, update the ` +
          `${reviewerSource} ${reviewerSource === 'instance override' ? 'in Instance Settings' : 'in Workspace Settings'}.`
      )
    }
  }
  ...
  const commits = await client.getPullRequestCommits(pullRequest.pullRequestId)
  const pullRequestSummary = summarizeBitbucketPullRequest(await client.getPullRequest(pullRequest.pullRequestId), commits)

  const webUrl = atlassianPullRequestUrl(atlassianBase, pullRequest.pullRequestId)
