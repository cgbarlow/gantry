...
import { readInstance, recordInstancePullRequest, recordInstanceApprovalState, azureDevOpsOutPath } from './instance.js'
...
function artefactFileUrl(azureDevOps, path, branch) {
  const url = new URL(
...
async function verifyRenderedArtefacts(definition, stage, slug, azureDevOps) {
  const artefacts = definition.artefacts.filter((artefact) => artefact.gate === stage.gate)
 ...
 * - any gate-matching rendered artefact is missing from that branch — checked before any Pull Request is opened, with every missing artefact named in the error;
...
export async function requestStageApproval(slug, options = {}) {
  ...
  const renderedArtefacts = await verifyRenderedArtefacts(definition, stage, slug, azureDevOps)
