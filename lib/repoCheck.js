 ...
 */
export async function migrateLegacyAzureDevOpsInstance(client, slug, instanceYamlText) {
  ...
  const legacyModuleItems = await client.listFolder(LEGACY_AZURE_DEVOPS_MODULES_DIR)
  ...
  const legacyInstanceYamlStillPresent = await client
    .getFileContent(LEGACY_AZURE_DEVOPS_INSTANCE_PATH)
    .then(() => true)
    .catch((err) => {
 ...
 *     instances), and `owner` is that stage's first non-empty module
 *     owner, or ''.
...
export async function checkAzureDevOpsRepo({ organization, project, repository, baseUrl, pat, definitionsDir }) {
  ...
  let legacyText
  try {
    legacyText = await client.getFileContent(LEGACY_AZURE_DEVOPS_INSTANCE_PATH)
  } catch (err) {
  ...
  if (legacyText !== undefined) {
    ...
    await migrateLegacyAzureDevOpsInstance(client, slug, legacyText)
  } else {
    const entries = await client.listFolder(AZURE_DEVOPS_WORKSPACE_ROOT)
    const slugs = entries
  ...
  const owner = await stageOwner(definition, stage, slug, azureDevOps)
