...
/**
 * Thrown when Azure DevOps rejects the supplied PAT (401/403). Kept distinct from AzureDevOpsRequestError so a caller (e.g. gantry's own server, per #82) can fold "PAT rejected" and "no PAT supplied" into the same structured "authentication required" response without string-matching a generic error's message.
 *
 * Extends the provider-neutral AuthenticationError (docs/adr/0039, #2) so a call site can catch either this vendor-named class or the neutral one — nothing here changes for the ~60 existing `instanceof AzureDevOps*Error` catch sites across the codebase.
 */
export class AzureDevOpsAuthenticationError extends AuthenticationError {
...
/**
 * Thrown when the repository itself doesn't exist in Azure DevOps — distinct from a missing file/folder within an existing repo (`AzureDevOpsNotFoundError`) and from a rejected PAT (`AzureDevOpsAuthenticationError`), so callers (e.g. repoCheck.js, server.js) can report "create the repo first" rather than a raw REST error or an ambiguous "empty" result.
 */
export class AzureDevOpsRepoNotFoundError extends RepoNotFoundError {
  constructor(message, { status } = {}) {
    super(message, { status, provider: PROVIDER })
    this.name = 'AzureDevOpsRepoNotFoundError'
  }
}

/**
 * Thrown when the requested item (file/path) doesn't exist in the repo — distinct from a rejected PAT or any other request failure, so callers can e.g. treat "file not found" as "create it" rather than an error.
 */
export class AzureDevOpsNotFoundError extends NotFoundError {
  constructor(message, { status } = {}) {
    super(message, { status, provider: PROVIDER })
...
/**
 * Catch-all for any other failed call to the Azure DevOps API: a non-2xx response this client doesn't otherwise distinguish, or a network-level failure reaching the server at all (DNS, connection refused, etc.).
 *
 * `body` is usually the raw response text (`res.text()`) for a non-2xx response. For an API that instead reports failure *inline* inside an HTTP 200 (e.g. the Update Refs endpoint `createBranch` calls, which reports a rejected ref update as `{ success: false, ... }` rather than a failing status code), `body` is that per-item result object instead — callers should not assume `body` is always a string.
 */
export class AzureDevOpsRequestError extends RequestError {
...
export function createAzureDevOpsClient({
  ...
  /**
   * Fetches an existing file's raw text content by repo-relative path. Throws AzureDevOpsNotFoundError if no item exists at that path (on the given branch, default "main").
   */
