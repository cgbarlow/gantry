 ...
 */
export class AzureDevOpsRepoNotFoundError extends Error {
  constructor(message, { status } = {}) {
    super(message)
...
export function createAzureDevOpsClient({
   ...
   */
  async function writeFiles(files, { branch = 'main', message } = {}) {
    const normalized = files.map((file) => ({ ...file, path: normalizePath(file.path) }))
    const [oldObjectId, ...existing] = await Promise.all([
      getBranchObjectId(branch),
      ...normalized.map((file) => fileExists(file.path, branch)),
    ])
    const changes = normalized.map((file, i) => ({ path: file.path, changeType: existing[i] ? 'edit' : 'add' }))
