...
export function createAzureDevOpsClient({
  ...
  /**
   * Explicit, merge-base-aware ahead/behind comparison between two versions (branches, tags, or
   * commits) via Azure DevOps's own Diffs "commit-diffs" endpoint (`GET .../diffs/commits`) — #140's
   * replacement for reasoning about direction from `listBranchCommits`'s `itemVersion`/`compareVersion`
   * filter, whose own REST reference (learn.microsoft.com/.../git/commits/get-commits) documents
   * neither parameter's comparison direction. This endpoint's reference (.../git/diffs/get), by
   * contrast, spells the two count fields out unambiguously: `aheadCount` is how many commits
   * `targetVersion` has that `baseVersion` lacks, `behindCount` the reverse (`baseVersion` has that
   * `targetVersion` lacks) — verified against that documentation, and against the matching
   * `GitBranchStats.aheadCount`/`behindCount` ("ahead and behind counts for a particular ref"), during
   * #140's investigation. `commonCommit` is the merge base Azure DevOps found between the two.
   *
   * `diffCommonCommit: true` makes the returned `changes` the diff from that merge base to
   * `targetVersion`, rather than the (potentially much larger, unrelated-history-included) diff from
   * `baseVersion` straight to `targetVersion` — what a caller actually wants when the question is "what
   * changed on target since these two diverged" (e.g. #140's stage-sync advisory: what main changed
   * since a stage branch forked from it), not "how do these two differ overall".
   */
  async function getCommitDiffs(baseVersion, targetVersion, { baseVersionType = 'branch', targetVersionType = 'branch', diffCommonCommit } = {}) {
    const url = new URL(`${repoUrl}/diffs/commits`)
    url.searchParams.set('baseVersion', baseVersion)
    url.searchParams.set('baseVersionType', baseVersionType)
    url.searchParams.set('targetVersion', targetVersion)
    url.searchParams.set('targetVersionType', targetVersionType)
    if (diffCommonCommit !== undefined) url.searchParams.set('diffCommonCommit', String(diffCommonCommit))
    // Generous over the API's own 100-item default: the one caller today (WI256/#140's stage-sync
    // advisory) scopes to a single instance's own workspace folder, never realistically thousands of
    // files, and a truncated `changes` page would silently under-report the advisory's file list.
    url.searchParams.set('$top', '2000')
    url.searchParams.set('api-version', apiVersion)

    const res = await request(url.toString(), { headers: { Accept: 'application/json' } })
    const data = await res.json()
    // The one guard against this file's own `$top=2000` comment above going stale in practice: Azure
    // DevOps sets `allChangesIncluded: false` when `changes` is a truncated page (more than `$top`
    // changed paths), which would otherwise silently understate a caller's advisory (#140's stage-sync
    // "what's behind" list) with nothing anywhere to say so. This is the one diagnostic breadcrumb for
    // that case — see lib/registry.js's own note on this codebase having no other server-side logging.
    if (data.allChangesIncluded === false) {
      console.warn(
        `lib/azureDevOpsClient.js: getCommitDiffs("${baseVersion}" -> "${targetVersion}") — Azure DevOps reports allChangesIncluded: false; "changes" is a truncated page (more than $top=2000 changed paths) and callers relying on it may under-report`
      )
    }
    return data
  }
  ...
  /**
   * Fetches the repository's own metadata — `{ id, project: { id }, name, defaultBranch, ... }`
   * (Azure DevOps's own Git Repositories "Get Repository" response). A dedicated function (not folded
   * into `repoExists`, which only ever needed the boolean) so #126's `hasWriteAccess` below can read
   * this repository's and its project's own GUIDs off the same request `repoExists` already made,
   * rather than adding a second round trip just to get them.
   */
  ...
  /**
   * Whether the repository itself exists in Azure DevOps — `getRepo` reduced to a boolean, used by lib/repoCheck.js to distinguish "repository doesn't exist" from "empty repository" before listing any folders. Without this, both cases look identical (listFolder returns `[]` for a missing repo), so workspace registration would succeed for a nonexistent repo and only fail later with an opaque REST error when instance creation tries to write.
   */
  ...
  // Azure DevOps's own "Git Repositories" security namespace — a stable, Microsoft-documented id
  // (https://learn.microsoft.com/azure/devops/organizations/security/namespace-reference#git-repositories),
  // not something this client derives or looks up.
  const GIT_REPOSITORIES_SECURITY_NAMESPACE_ID = '2e9eb7ed-3c0a-47d4-87c1-0ffdd275fd87'
  // "GenericContribute" within that namespace — "push to a non-protected branch", the write-access
  // question #126 needs answered (documented on the same namespace-reference page above; the
  // namespace's other bits, e.g. ForcePush or ManagePermissions, ask a stronger or unrelated question).
  const GENERIC_CONTRIBUTE_PERMISSION_BIT = 4

  /**
   * #126 (docs/adr/0047): whether this PAT can actually push to this repository — Azure DevOps's own
   * Permissions "Has Permissions" endpoint, asked about GenericContribute against this repository's
   * own security token (`repoV2/{projectId}/{repositoryId}`, Azure DevOps's documented token shape
   * for a single Git repository within the Git Repositories namespace). Read-only in effect: this is
   * a permission *query* Azure DevOps already exposes for exactly this purpose, not an action that
   * changes anything about the repository by asking.
   */
  async function hasWriteAccess() {
    const repo = await getRepo()
    const projectId = repo.project?.id ?? project
    const token = `repoV2/${projectId}/${repo.id}`
    const url = new URL(
      `${baseUrl.replace(/\/+$/, '')}/${encodeURIComponent(organization)}/_apis/permissions/${GIT_REPOSITORIES_SECURITY_NAMESPACE_ID}/${GENERIC_CONTRIBUTE_PERMISSION_BIT}`
    )
    url.searchParams.set('tokens', token)
    url.searchParams.set('api-version', apiVersion)
    const res = await request(url.toString(), { headers: { Accept: 'application/json' } })
    const data = await res.json()
    // Azure DevOps's documented response for this endpoint is a bare boolean body; handled
    // defensively in case a proxy or an older API version wraps it as `{ value: true }` instead.
    return typeof data === 'boolean' ? data : Boolean(data.value)
  }
