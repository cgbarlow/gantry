...
import { buildDefinitionYamlObject, buildModuleYamlObject, findDefinitionProblemsInStructure, TEMPLATE_NAME_RE } from './definition.js'
...
export const LOCAL_WORKSPACE_LIMITS = Object.freeze({
  ...
  maxModuleFiles: 200,
  maxAssets: 200,
  // WI #384 — an inline `definition` payload's own module-spec count and
  // template-file count, checked separately from `maxModuleFiles` above
  // (that one caps the *instance*'s saved module content files).
  maxDefinitionModules: 200,
  maxDefinitionTemplates: 200,
})
...
function validatePayload(operation, payload) {
  ...
  const definition = validateInlineDefinition(payload.definition, definitionId, definitionVersion)

  return { definitionId, definitionVersion, moduleFiles, assets, instanceYaml, definition }
}

// Validates an optional inline `definition` payload — `{ structure, templates?,
// referenceDocs? }` — a local-workspace *definition*'s own in-memory content
// (WI #384), supplied so `status`/`check`/`render`/`compile` can run against a
// definition that lives only in the caller's browser, never in this server's
// bundled `definitionsDir`. `structure` is the definition-version-projection
// shape `lib/definition.js`'s `buildDefinitionYamlObject`/`buildModuleYamlObject`
// already know how to turn back into `definition.yaml`/`modules/*.yaml` text —
// the same shape `writeDefinitionVersion` (disk-based) and
// `findDefinitionProblemsInStructure` (the stateless `/api/local/definition/validate`
// route) both already accept, so a caller on either side of the wire reuses one
// shape throughout. Returns `undefined` when no `definition` was supplied
// (the pre-existing "resolve from this server's own library" behaviour).
function validateInlineDefinition(rawDefinition, definitionId, definitionVersion) {
  if (rawDefinition === undefined || rawDefinition === null) return undefined
  assertObject(rawDefinition, 'definition')

  const structure = rawDefinition.structure
  assertObject(structure, 'definition.structure')
  if (structure.id !== definitionId) {
    fail(`definition.structure.id "${structure.id}" must match definitionId "${definitionId}"`)
  }
  const version = definitionVersion ?? Number(structure.version)
  if (!Number.isInteger(version) || version < 1) {
    fail('definitionVersion (or definition.structure.version) must be a positive integer when an inline "definition" is supplied')
  }
  const modules = structure.modules ?? []
  if (!Array.isArray(modules)) fail('definition.structure.modules must be an array')
  if (modules.length > LOCAL_WORKSPACE_LIMITS.maxDefinitionModules) {
    fail(`Too many definition.structure.modules: ${modules.length} (limit ${LOCAL_WORKSPACE_LIMITS.maxDefinitionModules})`, 413)
  }
  for (const mod of modules) {
    assertObject(mod, 'each definition.structure.modules entry')
    if (!isSafeFilename(mod.id)) fail(`Unsafe module id "${mod?.id}" in definition.structure.modules`)
  }

  const templates = rawDefinition.templates ?? {}
  assertObject(templates, 'definition.templates')
  const templateNames = Object.keys(templates)
  if (templateNames.length > LOCAL_WORKSPACE_LIMITS.maxDefinitionTemplates) {
    fail(`Too many definition.templates: ${templateNames.length} (limit ${LOCAL_WORKSPACE_LIMITS.maxDefinitionTemplates})`, 413)
  }
  for (const name of templateNames) {
    if (!TEMPLATE_NAME_RE.test(name)) fail(`Invalid template name "${name}" in definition.templates`)
    if (typeof templates[name] !== 'string') fail(`definition.templates["${name}"] must be a string`)
  }

  const referenceDocs = rawDefinition.referenceDocs ?? {}
  assertObject(referenceDocs, 'definition.referenceDocs')
  for (const artefactId of Object.keys(referenceDocs)) {
    if (!isSafeFilename(artefactId)) fail(`Unsafe referenceDocs key "${artefactId}" — must be a plain artefact id`)
    if (typeof referenceDocs[artefactId] !== 'string') fail(`definition.referenceDocs["${artefactId}"] must carry a base64 string`)
  }

  return { structure, templates, referenceDocs, version }
}

// Materializes an inline `definition` (already validated above) into
// `<root>/definitions/<definitionId>/<version>/` — `definition.yaml`,
// `modules/*.yaml`, `templates/*` — reusing the exact same builders the
// disk-based editor save path (`lib/definition.js`'s `writeDefinitionVersion`)
// does, so a request-supplied definition is indistinguishable, once written,
// from one that started life on disk. Returns the `definitions/` dir to pass
// as `definitionsDir` to the `lib/` compute functions below.
function materializeInlineDefinition(root, definitionId, definition) {
  const definitionsDir = join(root, 'definitions')
  const versionDir = join(definitionsDir, definitionId, String(definition.version))
  mkdirSync(join(versionDir, 'modules'), { recursive: true })

  const defObj = buildDefinitionYamlObject(definitionId, definition.version, definition.structure.status ?? 'draft', definition.structure, {})
  writeFileSync(join(versionDir, 'definition.yaml'), stringifyYAML(defObj))

  for (const mod of definition.structure.modules ?? []) {
    writeFileSync(join(versionDir, 'modules', `${mod.id}.yaml`), stringifyYAML(buildModuleYamlObject(mod)))
  }

  if (Object.keys(definition.templates).length) {
    mkdirSync(join(versionDir, 'templates'), { recursive: true })
    for (const [name, source] of Object.entries(definition.templates)) {
      writeFileSync(join(versionDir, 'templates', name), source)
    }
  }
  if (Object.keys(definition.referenceDocs).length) {
    mkdirSync(join(versionDir, 'templates'), { recursive: true })
    for (const [artefactId, base64] of Object.entries(definition.referenceDocs)) {
      writeFileSync(join(versionDir, 'templates', `reference-${artefactId}.docx`), Buffer.from(base64, 'base64'))
    }
  }
...
export async function runLocalWorkspaceCompute(operation, payload, options = {}) {
  ...
  const { definitionId, definitionVersion, moduleFiles, assets, instanceYaml, definition } = validatePayload(
    operation,
    payload
  )
  const definitionsDir = options.definitionsDir ?? 'definitions'

  // `validate` never touches the caller's file tree. An inline `definition`
  // (WI #384 — a local-workspace definition, not one this server's own
  // `definitionsDir` has ever seen) is checked with the same in-memory rule
  // set `POST /api/local/definition/validate` uses, no disk read at all; the
  // pre-existing id-lookup path (a *library* definition) still checks a
  // bundled one — either way, no sandbox is needed for this operation.
  if (operation === 'validate') {
    if (definition) {
      const problems = findDefinitionProblemsInStructure(definition.structure)
  ...
  const record = buildInstanceRecord(instanceYaml, {
    definitionId,
    definitionVersion: definition ? definition.version : definitionVersion,
    slug,
  })
  ...
  try {
    ...
    // WI #384 — a local-workspace-pinned instance's definition never lives in
    // this server's bundled `definitionsDir`; the caller ships its content
    // instead (`definition`, validated above) and this materializes it into
    // the same throwaway sandbox the instance files just landed in, so every
    // `lib/` compute function below reads it exactly as it would any other
    // `definitionsDir` entry — no branching in `getStatus`/`checkGate`/
    // `renderArtefact` themselves.
    const effectiveDefinitionsDir = definition ? materializeInlineDefinition(root, definitionId, definition) : definitionsDir

    if (operation === 'status') {
      return getStatus(slug, { instancesDir, definitionsDir: effectiveDefinitionsDir })
    }

    if (operation === 'check') {
      return checkGate(slug, { instancesDir, definitionsDir: effectiveDefinitionsDir, gate: payload.gate })
    }
    ...
    if (operation === 'compile') {
      ...
      const result = renderArtefact(slug, payload.artefact, {
        instancesDir,
        definitionsDir: effectiveDefinitionsDir,
    ...
    // WI #359: a docx-format render (this operation's only mode — a markdown-only local
    // render goes through the 'compile' operation above instead, never 'render') no longer
    // writes its intermediate `.md` to disk at all, so `result.markdown` (already in memory)
    // is used directly rather than re-reading a file that, by design, was never created.
    return {
      artefact: payload.artefact,
      basename: result.basename,
      markdown: result.markdown,
