...
const OPERATIONS = new Set(['status', 'check', 'validate', 'render'])
...
function validatePayload(operation, payload) {
  ...
  if (operation === 'render' && (typeof payload.artefact !== 'string' || payload.artefact.trim() === '')) {
    fail('artefact is required for /api/local/render')
  }
 ...
 *
 * @param {'status'|'check'|'validate'|'render'} operation
 ...
 *   `markdown` plus the produced `.docx` as `docxBase64`.
 */
export async function runLocalWorkspaceCompute(operation, payload, options = {}) {
  ...
  if ((operation === 'status' || operation === 'render') && !record.stage) {
