 ...
 * When the asset carries a `source` URL, also emits a caption-styled citation immediately below the image — `*Source: [<source>](<<source>>)*` — as an italic paragraph Pandoc preserves into the .docx. No citation is emitted for assets lacking a source.
 */
export function resolveAssetFileRefs(markdown, slug, options = {}) {
  return markdown.replace(ASSET_REF_RE, (match, assetId) => {
    ...
    const label = asset.source.replaceAll(']', '\\]')
    return `${base}\n\n*Source: [${label}](<${asset.source}>)*`
  })
}
