...
function missingLibraryPatMessage(provider) {
...
async function readAzureDevOpsDefinitions(repo, pat) {
  const { organization, project, repository, baseUrl } = repo.location
  const options = { azureDevOps: { organization, project, repository, baseUrl, pat } }
...
export async function refreshLibraryRepo(repo, { instancesDir = 'instances', pat } = {}) {
  const defs = repo.provider === 'github' ? await readGitHubDefinitions(repo, pat) : await readAzureDevOpsDefinitions(repo, pat)
