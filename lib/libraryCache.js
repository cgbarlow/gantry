...
import { listAtlassianDefinitions, loadAtlassianDefinition } from './definitionAtlassian.js'
...
const LIBRARY_PAT_ENV_VAR_NAME = {
  ...
  // #49 (ADR-0042): a single token, not a `{bitbucket, jira}` pair — a Library repo needs only its
  // Bitbucket environment credential, since reading `definitions/` and opening a Promote pull request
  // are both purely content-store operations that never touch Jira.
  atlassian: 'GANTRY_LIBRARY_PAT_ATLASSIAN',
}
...
// #49 (ADR-0042): the Atlassian (Bitbucket-backed) counterpart of readGitLabDefinitions above — same
// "only a definition's current latest published version is cached" contract, over
// lib/definitionAtlassian.js instead. `pat` here is always the workspace's *Bitbucket* token — a
// library repo never has a Jira token in the first place (ADR-0042: "an Atlassian library repo needs
// only its Bitbucket environment credential"), and `lib/definitionAtlassian.js`'s own `options.atlassian`
// shape is Bitbucket-only regardless (never `{bitbucket, jira}`). `jiraSite`/`jiraProjectKey` (present
// on every Atlassian location, `lib/provider.js`'s own full schema — a library repo's location shape
// is unchanged from a workspace's, only its credential is narrower) are never read here — Bitbucket's
// content-store API needs only `owner`/`repository`. `location.baseUrl` is passed through even though
// no production Atlassian location ever carries one (ADR-0042: Cloud-only, no self-hosted override) —
// it only ever rides along in a test that hand-builds a `repo` object to point at
// `tests/helpers/fakeBitbucketServer.js`, the same "exercise the module directly with an explicit
// baseUrl" convention `tests/definitionAtlassian.test.js`/`tests/renderAtlassian.test.js` already use.
async function readAtlassianDefinitions(repo, pat) {
  const { owner, repository, baseUrl } = repo.location
  const options = { atlassian: { owner, repository, baseUrl, pat } }
  const rows = await listAtlassianDefinitions(options)
  const defs = []
  for (const row of rows) {
    if (!row.latestPublished) continue // draft-only in this repo — nothing a read-only caller can use yet.
    defs.push(await loadAtlassianDefinition(row.id, row.latestPublished, options))
  }
  return defs
}
...
const readDefinitionsByProvider = {
  ...
  atlassian: readAtlassianDefinitions,
}
