...
export function createGitHubWorkItemsClient({ owner, repository, pat, baseUrl = DEFAULT_BASE_URL } = {}) {
  ...
  /**
   * Creates a new issue. `assignees` (logins) and `labels` (names) are both optional and passed
   * straight through to GitHub's own create-issue payload — this is what lets a caller create an
   * already-assigned, already-labelled issue in one round trip (#15's Request Review: one reviewer
   * assigned, one `gantry:review/*` status label, from the moment the issue exists) rather than a
   * create-then-patch pair. Returns the created issue (`number`, `id`, `title`, `body`, `state`,
   * `html_url`, `labels`, ...), the raw shape GitHub itself returns.
   */
  async function createIssue({ title, body, assignees, labels }) {
    const res = await request(`${repoUrl}/issues`, {
      method: 'POST',
      body: { title, body, ...(assignees ? { assignees } : {}), ...(labels ? { labels } : {}) },
    })
    return res.json()
  }
   ...
   * `allGitHubReviewLabels()`) exists on the repo, creating any that don't — #15's "Gantry creates
   * those labels on demand" (docs/adr/0040), so a repo needs no setup before Request Review works.
   * Tolerates GitHub's 422 "already_exists" for one a prior request (or another gantry instance
   * against the same repo) already created; any other failure propagates.
   */
  async function ensureLabelsExist(labels) {
    for (const label of labels) {
      try {
        await request(`${repoUrl}/labels`, { method: 'POST', body: label })
      } catch (err) {
        if (err instanceof GitHubRequestError && err.status === 422) continue
        throw err
      }
    }
  }

  return { owner, repository, baseUrl, createIssue, getIssue, updateIssue, createChildIssue, ensureLabelsExist }
}
