...
// The instance-data read options (local, Azure-DevOps-backed or GitHub-backed) implied by
// `options.azureDevOps`/`options.github` — this is the instance's own *data storage* location, wholly
// independent of `link.organization`/`link.project` (or `link.owner`/`link.repository`) below (the
// work-items API's own location, see linkInstanceToWorkItem's own doc comment on
...
export async function linkInstanceToWorkItem(slug, link, options = {}) {
  ...
  // 'github'/'azure-devops' is new territory — a provider #14's own binary `github`-or-else check
  // would have silently run the Azure DevOps branch against, against fields it doesn't understand
  // (e.g. a GitLab-shaped `link` with no `organization`/`project` at all). Reported clearly instead,
  // exactly like `getProviderCapabilities`'s own "not registered" error, until that provider's own
  // linker exists.
  ...
  if (instance.workItem) {
    const existingRef =
      instance.workItem.provider === 'github'
        ? `GitHub issue #${instance.workItem.parentNumber} (${instance.workItem.owner}/${instance.workItem.repository})`
        : `Azure DevOps work item ${instance.workItem.parentId} (${instance.workItem.organization}/${instance.workItem.project})`
    throw new Error(`Instance "${slug}" is already linked to ${existingRef} — unlinking/re-linking isn't supported yet.`)
  }
