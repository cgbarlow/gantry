...
export function parseSharedWorkspacePats(raw) {
  ...
  try {
    ...
    // #130's rule 1 ("names only, never values") applies to this message too, and this is the one
    // parser in this file whose variable holds credentials. `JSON.parse`'s own error text embeds the
    // first ~10 characters of its input (`Unexpected token 'g', "ghp_ABCDEF"... is not valid JSON`),
    // so interpolating `err.message` here printed a prefix of the operator's PAT straight into the
    // deploy log — and the likeliest way to reach this branch at all is pasting a bare PAT where the
    // map was expected. The expected shape is what an operator actually needs; the value they set is
    // something they already have.
    throw new Error(
      `${PATS_ENV_VAR_NAME} must be valid JSON: a JSON object mapping workspace id to PAT, {"<workspaceId>":"<pat>"}. ` +
