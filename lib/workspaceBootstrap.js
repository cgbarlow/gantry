...
//
// GANTRY_BOOTSTRAP_PATS (#113, parent #109, docs/adr/0046) extends this same boot step: registering a
// workspace is not the same as its instances being *visible* — with only #111 and #112, a restored
// Provider-backed workspace's instances stay invisible until the first authenticated browser request
// arrives, which for an unattended deployment (the hosted demo) can look indistinguishable from
// "still broken". `parseBootstrapPats` and `discoverBootstrapPatInstances` below are opt-in and off by
// default: unset, this env var changes nothing, and discovery keeps running only on the
// request-credential path #112 already built. This is a deliberate, narrow departure from ADR-0038's
// "no server-side credential" — recorded consciously in docs/adr/0046, not introduced as a side effect
// of this fix — so read that ADR before touching either function below.
...
const PATS_ENV_VAR_NAME = 'GANTRY_BOOTSTRAP_PATS'
...
/**
 * Parses and validates `GANTRY_BOOTSTRAP_PATS`'s raw string value into a `{ workspaceId: pat }` map
 * (#113, parent #109, docs/adr/0046) — the same shape and the same fail-loud validation style as
 * `mcp-server/src/credentials.js`'s `parseWorkspacePats` for its own, differently-scoped env var
 * (`GANTRY_WORKSPACE_PATS`, read by the separate MCP server process, docs/adr/0043). This is a
 ...
 * what keeps `GANTRY_BOOTSTRAP_PATS` off by default: an operator who never sets it gets back an empty
 * map, `discoverBootstrapPatInstances` below has nothing to iterate, and boot-time discovery simply
 * never runs.
 */
export function parseBootstrapPats(raw) {
  if (raw === undefined || raw === null || raw.trim() === '') return {}
 ...
 * has a matching entry in `pats` (`parseBootstrapPats`'s own return shape), runs
 ...
 * already populated for that workspace before any browser request arrives, closing the gap #112 alone
 * leaves at the request-credential path (#113, parent #109, docs/adr/0046).
 *
