 ...
 *
 * Covers exactly the three capabilities ADR-0014 calls for: create a pull
 * request, read its reviewers' votes, and complete (merge) it. Casting a
 * vote is deliberately not exposed here — that's the Owner's own action,
 * performed directly in Azure DevOps's UI (ADR-0014), never something
 * gantry itself does.
 *
...
export function createAzureDevOpsPullRequestsClient({
   ...
   * `[{ id: 'guid', vote: 0, required: true }]` — we mirror that shape
   * but expose only `id` and `required` (vote is the reviewer's own action
   * in the Azure DevOps UI, never something gantry sets).
   */
