...
function reviewerSummary(reviewers) {
  return (reviewers ?? []).map((r) => ({ displayName: r.displayName ?? r.id, vote: r.vote ?? 0 }))
}
 ...
 *   completed;
 * - `review` — `{ state, reviewers }`, state from `interpretReviewerVotes`;
...
export async function checkStageApprovalStatus(slug, options = {}) {
  ...
  const review = {
    state: interpretReviewerVotes(pullRequest.reviewers),
    reviewers: reviewerSummary(pullRequest.reviewers),
  }
