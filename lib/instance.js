...
// client's own `'main'` default when the caller didn't supply one. There is no per-stage branch yet
// (#12 — GitHub stage branches is a later ticket), so every #11 caller currently gets `undefined` here
// and lands on `main`, same as an Azure-DevOps-backed instance did before #122.
 ...
 * separate ticket), and no stage-branch targeting (#12): every write here lands on `main`.
 */
