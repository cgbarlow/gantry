...
import { NotFoundError, RepoNotFoundError } from './providerErrors.js'
import { resolveContentStore } from './providerRegistry.js'
import { PROVIDER } from './azureDevOpsClient.js'
import { PROVIDER as GITHUB_PROVIDER, repoNotFoundMessage } from './githubClient.js'
import { isValidSlug } from './slug.js'
import { parseFieldBody, serialiseFieldBody } from './fieldShape.js'
...
// #188: an instance's own reference number (the `i<N>` of `w<W>i<N>`), recorded in instance.yaml at
// creation so it survives the server's number registry being rebuilt (lib/numberRegistry.js).
function persistedNumberField(options) {
  return Number.isInteger(options.number) ? { number: options.number } : {}
}
...
// #87 (ADR-0045 §6): caps a rendered artefact's basename so `out/<basename>.docx`
// stays a valid path component on every mainstream filesystem (ext4/NTFS/APFS all
// cap a single path segment at 255 bytes) with generous headroom left for the
// extension and whatever prefix a Provider push adds ahead of it. 200 is
// conservative rather than exact — ADR-0045 doesn't pin a number, and there's no
// reason to cut it any closer than that. Re-trims after slicing so a cut mid-
// separator doesn't leave a dangling "-" or space, matching sanitiseRenderFilename's
// own trimmed-edges contract.
const MAX_BASENAME_LENGTH = 200
function capBasenameLength(text) {
  return text.length <= MAX_BASENAME_LENGTH ? text : text.slice(0, MAX_BASENAME_LENGTH).replace(/[\s-]+$/, '')
}

// #87 (ADR-0045): every `{token}` a filename: pattern can contain — `{today}`
// and `{instance.name}`/`{instance.slug}` resolve without touching module data;
// anything else is `{module.field}`, looked up in the already-fetched `modules`
// (the same `{ [moduleId]: { [fieldId]: value } }` shape compileArtefact builds
// for template interpolation). An unresolvable/blank token is dropped (ADR-0045
// §5) rather than left as a literal gap or thrown — findDefinitionProblems/
// findLocalDefinitionProblems reject an invalid *token* at publish time, so by
// the time this runs the only reason a reference resolves empty is the field
// itself being genuinely blank on this instance, which is allowed mid-stage.
function resolveFilenameToken(token, { instanceName, instanceSlug, modules, today }) {
  const ref = token.trim()
  if (ref === 'today') return today ?? new Date().toISOString().slice(0, 10)
  if (ref === 'instance.name') return instanceName ?? ''
  if (ref === 'instance.slug') return instanceSlug ?? ''
  const dot = ref.indexOf('.')
  if (dot === -1) return ''
  const moduleId = ref.slice(0, dot)
  const fieldId = ref.slice(dot + 1)
  const value = modules?.[moduleId]?.[fieldId]
  return typeof value === 'string' ? value.trim() : ''
}

const FILENAME_TOKEN_RE = /\{([^{}]+)\}/g

// #87 (ADR-0045 §5): substitutes every `{token}` and lets sanitiseRenderFilename's
// own whitespace/dash collapsing (called by renderedArtefactBasename right after
// this) tidy the separators an empty token leaves behind — e.g.
// "{selection.candidate-name} - Offer Pack" with a blank candidate-name resolves
// to " - Offer Pack" here, then "Offer Pack" once sanitised.
function resolveFilenamePattern(pattern, context) {
  return pattern.replace(FILENAME_TOKEN_RE, (_, token) => resolveFilenameToken(token, context))
}

// #87: the small bundle renderedArtefactBasename's pattern branch needs beyond
// the instance name/artefact title it already takes — an artefact's own
// filename: pattern (`undefined` for the overwhelming majority that declare
// none, which is exactly what keeps every existing render byte-for-byte
// unchanged) plus the instance's already-fetched module data and slug.
// Exported so lib/render.js and lib/stageApproval.js build this identically
// rather than each assembling its own shape.
export function filenamePatternContext(artefact, modules, instanceSlug) {
  return { pattern: artefact.filename, modules, instanceSlug }
}

// The basename (no extension) every rendered artefact is written under
// locally and pushed to every Provider as (WI226): "<Instance name> - <Full
// artefact title>" — e.g. "ATLAS Reference Design - High Level Design" — for
// any artefact that declares no `filename:` pattern (still the default, and
// unchanged for every artefact that doesn't opt in).
//
// #87 (ADR-0045): an artefact that *does* declare `filename:` (carried on
// `patternContext.pattern`) instead names itself from the instance's own
// content — `{module.field}`/`{instance.name}`/`{instance.slug}`/`{today}`
// tokens resolved against `patternContext.modules`/`.instanceSlug`/`.today`
// (the last defaulting to the date of *this* call when omitted, per §3). This
// stays the one place a rendered file's name is decided — no second seam.
export function renderedArtefactBasename(instanceName, artefactTitle, patternContext = {}) {
  const { pattern, modules, instanceSlug, today } = patternContext
  const raw =
    typeof pattern === 'string' && pattern.trim() !== ''
      ? resolveFilenamePattern(pattern, { instanceName, instanceSlug, modules, today })
      : `${instanceName} - ${artefactTitle}`
  return capBasenameLength(sanitiseRenderFilename(raw))
}

// #88 (ADR-0045 §7): the filename a rendered artefact's manifest lives at, sibling of the
// artefact file itself inside the same `out/` directory — local disk and every Provider push
// alike, since the manifest travels with whichever "out/" the artefact's own file just landed
// in rather than needing a separate location convention of its own. Leading dot: bookkeeping,
// not a document anyone renders or opens, the same convention `lib/assets.js`'s
// `manifest.yaml` avoids only because assets already have their own dedicated directory.
export const RENDER_MANIFEST_FILENAME = '.gantry-render-manifest.json'

// #88: turns whatever was read from a manifest file — real JSON, missing (`undefined`),
// truncated, hand-edited into garbage, or simply not an object — into a plain `{ [artefactId]:
// filename }` map, defaulting to `{}` for every failure case. This is the one place "a manifest
// missing or unreadable degrades to current (pre-#88) behaviour rather than failing the render"
// is decided; every caller below treats an empty map exactly like "no artefact has ever been
// recorded here before", which is byte-for-byte the render behaviour that existed before this
// ticket (write the new file, never look for or delete an old one).
export function parseRenderManifest(raw) {
...
// #88: every filename `renderedArtefactBasename` (+ extension) can actually produce is a single
// path segment — `sanitiseRenderFilename` already strips `/`/`\` from anything derived from
// content. A hand-edited manifest is untrusted input, though: this rejects anything that isn't a
// plain segment (a path separator, or a bare `.`/`..`) before it can reach `resolveRenderManifestUpdate`'s
// caller as a "delete this" instruction, so a doctored manifest entry can never walk `rmSync`/a
// Provider `deletePaths` entry outside the `out/` directory it belongs in.
function isSafeManifestFilename(value) {
  return typeof value === 'string' && value !== '' && value !== '.' && value !== '..' && !/[/\\]/.test(value)
}

// #88 (ADR-0045 §7): the one decision every local and Provider render path shares — given the
// manifest as last read (`manifestRaw`, tolerant per `parseRenderManifest` above) and the
// filename (basename + extension, no directory) this render just produced for `artefactId`,
// works out whether anything actually needs to change. Pure — no I/O, so this can't itself be
// the thing that fails a render, and both the local-disk caller and every Provider's
// `writeFiles` call make the exact same call rather than each re-deriving it.
//
// `changed: false` (previous filename already matches) is what lets a render that doesn't
// affect the name skip writing the manifest at all — "must not touch the manifest file or
// create a no-op commit" (#88's own acceptance criteria) — since every render already pushes
// its own content regardless of whether the name moved.
export function resolveRenderManifestUpdate(manifestRaw, artefactId, newFilename) {
  const manifest = parseRenderManifest(manifestRaw)
  const previous = manifest[artefactId]
  if (previous === newFilename) {
    return { manifest, changed: false, staleFilename: null }
  }
  return {
    manifest: { ...manifest, [artefactId]: newFilename },
    changed: true,
    staleFilename: isSafeManifestFilename(previous) && previous !== newFilename ? previous : null,
  }
}
...
// `patternContext` (#87): forwarded verbatim to renderedArtefactBasename — see
// `filenamePatternContext` above. Omitted by every caller that predates #87, which
// keeps this an unpatterned "<Instance name> - <Artefact title>" path exactly as before.
export function azureDevOpsOutPath(slug, instanceName, artefactTitle, extension = 'docx', patternContext = {}) {
  assertValidAzureDevOpsSlug(slug)
  return `${AZURE_DEVOPS_WORKSPACE_ROOT}/${slug}/out/${renderedArtefactBasename(instanceName, artefactTitle, patternContext)}.${extension}`
}
...
// #11: a GitHub-backed instance uses the exact same `gantry-workspace/<slug>/` on-disk convention as
// Azure DevOps (ADR-0037's "Remote workspace" rename covers the *concept* of which suite a workspace
// names, not this physical repo layout) — no GitHub-specific root, no legacy-migration story (a
// GitHub workspace is new, #8, so there is no pre-#100-shaped repo to migrate off of).
export function githubInstancePath(slug) {
  assertValidAzureDevOpsSlug(slug)
  return `${AZURE_DEVOPS_WORKSPACE_ROOT}/${slug}/instance.yaml`
}

export function githubModulePath(slug, moduleId) {
  assertValidAzureDevOpsSlug(slug)
  return `${AZURE_DEVOPS_WORKSPACE_ROOT}/${slug}/modules/${moduleId}.md`
}

// #16: the GitHub twin of azureDevOpsAssetsDir/azureDevOpsAssetPath — same WI260 repo-as-asset-store
// convention (`gantry-workspace/<slug>/assets/<name>`, sibling of `modules/`), same root, because #11
// already established that a GitHub-backed instance uses Azure DevOps's exact on-disk layout.
export function githubAssetsDir(slug) {
  assertValidAzureDevOpsSlug(slug)
  return `${AZURE_DEVOPS_WORKSPACE_ROOT}/${slug}/assets`
}

export function githubAssetPath(slug, filename) {
  assertValidAzureDevOpsSlug(slug)
  return `${githubAssetsDir(slug)}/${filename}`
}

// #16: the GitHub twin of azureDevOpsOutPath above — same "<Instance name> - <Full artefact title>"
// basename convention, same `out/` sibling directory.
export function githubOutPath(slug, instanceName, artefactTitle, extension = 'docx', patternContext = {}) {
  assertValidAzureDevOpsSlug(slug)
  return `${AZURE_DEVOPS_WORKSPACE_ROOT}/${slug}/out/${renderedArtefactBasename(instanceName, artefactTitle, patternContext)}.${extension}`
}

// #31: the GitLab twin of the GitHub path builders above — same `gantry-workspace/<slug>/` on-disk
// convention (ADR-0041 changes GitLab's *location* shape, not this physical repo layout), same
// WI260 repo-as-asset-store `assets/` sibling directory, same "<Instance name> - <Full artefact
// title>" `out/` basename convention. `gitlabInstancePath` is only what this ticket's own
// render-provenance read needs (`readInstanceFromGitLab` below) — creating/writing a fresh
// GitLab-backed instance.yaml (the `createInstanceInGitHub` twin) is #25's own job.
export function gitlabInstancePath(slug) {
  assertValidAzureDevOpsSlug(slug)
  return `${AZURE_DEVOPS_WORKSPACE_ROOT}/${slug}/instance.yaml`
}

export function gitlabModulePath(slug, moduleId) {
  assertValidAzureDevOpsSlug(slug)
  return `${AZURE_DEVOPS_WORKSPACE_ROOT}/${slug}/modules/${moduleId}.md`
}

export function gitlabAssetsDir(slug) {
  assertValidAzureDevOpsSlug(slug)
  return `${AZURE_DEVOPS_WORKSPACE_ROOT}/${slug}/assets`
}

export function gitlabAssetPath(slug, filename) {
  assertValidAzureDevOpsSlug(slug)
  return `${gitlabAssetsDir(slug)}/${filename}`
}

export function gitlabOutPath(slug, instanceName, artefactTitle, extension = 'docx', patternContext = {}) {
  assertValidAzureDevOpsSlug(slug)
  return `${AZURE_DEVOPS_WORKSPACE_ROOT}/${slug}/out/${renderedArtefactBasename(instanceName, artefactTitle, patternContext)}.${extension}`
}

// #43: the Atlassian (Bitbucket-backed) twin of the GitHub/GitLab path builders above — same
// `gantry-workspace/<slug>/` on-disk convention (ADR-0042 changes Atlassian's *location* shape, not
// this physical repo layout), same WI260 repo-as-asset-store `assets/` sibling directory, same
// "<Instance name> - <Full artefact title>" `out/` basename convention. Every path here addresses the
// Bitbucket half of the suite only — an instance's own content never touches Jira.
export function atlassianInstancePath(slug) {
  assertValidAzureDevOpsSlug(slug)
  return `${AZURE_DEVOPS_WORKSPACE_ROOT}/${slug}/instance.yaml`
}

export function atlassianModulePath(slug, moduleId) {
  assertValidAzureDevOpsSlug(slug)
  return `${AZURE_DEVOPS_WORKSPACE_ROOT}/${slug}/modules/${moduleId}.md`
}

export function atlassianAssetsDir(slug) {
  assertValidAzureDevOpsSlug(slug)
  return `${AZURE_DEVOPS_WORKSPACE_ROOT}/${slug}/assets`
}

export function atlassianAssetPath(slug, filename) {
  assertValidAzureDevOpsSlug(slug)
  return `${atlassianAssetsDir(slug)}/${filename}`
}

export function atlassianOutPath(slug, instanceName, artefactTitle, extension = 'docx', patternContext = {}) {
  assertValidAzureDevOpsSlug(slug)
  return `${AZURE_DEVOPS_WORKSPACE_ROOT}/${slug}/out/${renderedArtefactBasename(instanceName, artefactTitle, patternContext)}.${extension}`
}
 ...
 */
// #127 (parent #109, docs/adr/0047): `sharedCache`, when `options.azureDevOps` carries one (set by
// `lib/server.js`'s `withAzureDevOpsCredential` — see that function's own doc comment), rides straight
// through to `resolveContentStore`, which is the one place that actually wraps the client for caching.
// This function itself neither knows nor cares whether caching is happening — it just forwards
// whatever it was given, exactly like `pat`/`baseUrl` always have been.
function azureDevOpsClientFor(options) {
  const { organization, project, repository, pat, baseUrl, branch, sharedCache } = options.azureDevOps
  return { client: resolveContentStore('azure-devops', { organization, project, repository, pat, baseUrl, sharedCache }), branch }
}

// The GitHub twin of azureDevOpsClientFor above (#11) — same contract: `options.github`'s presence is
// what routes a call through GitHub instead of the local filesystem, and `branch` falls through to the
// client's own `'main'` default when the caller didn't supply one. `lib/server.js`'s GitHub routes
// resolve a real per-stage branch via `lib/githubStageBranch.js` (#12) the same way its Azure DevOps
// routes already do via `lib/stageBranch.js`; a caller that supplies no branch (e.g. creating a brand
// new instance) still lands on `main`, same as an Azure-DevOps-backed instance did before #122.
// #127: same `sharedCache` pass-through as azureDevOpsClientFor above — see its doc comment.
function githubClientFor(options) {
  const { owner, repository, pat, baseUrl, branch, sharedCache } = options.github
  return { client: resolveContentStore('github', { owner, repository, pat, baseUrl, sharedCache }), branch }
}

// The GitLab twin of azureDevOpsClientFor/githubClientFor above (#31): `options.gitlab`'s presence
// routes a call through GitLab instead of the local filesystem, and `branch` falls through to the
// client's own `'main'` default (`lib/gitlabClient.js`) when the caller didn't supply one — no
// per-stage branch lifecycle yet (#29, mirroring GitHub's own #12), same "every write defaults to
// main" contract GitHub's own client followed before its stage branches existed.
// #127: same `sharedCache` pass-through as azureDevOpsClientFor above — see its doc comment.
function gitlabClientFor(options) {
  const { namespace, repository, pat, baseUrl, branch, sharedCache } = options.gitlab
  return { client: resolveContentStore('gitlab', { namespace, repository, pat, baseUrl, sharedCache }), branch }
}

// #43 — the Atlassian (Bitbucket-backed) twin of gitlabClientFor above: `options.atlassian`'s presence
// routes a call through Bitbucket instead of the local filesystem, and `branch` falls through to the
// client's own `'main'` default (`lib/bitbucketClient.js`) when the caller didn't supply one — no
// per-stage branch lifecycle wired into this read/write path yet (`lib/bitbucketStageBranch.js`
// exists, but wiring it into every instance read/write is a later ticket's job, mirroring GitLab's own
// #33/#34). `options.atlassian` is always the *Bitbucket* half of an Atlassian location — `{ owner,
// repository, pat, baseUrl?, branch? }`, never `{ bitbucket, jira }` — since an instance's own content
// (instance.yaml, modules, assets, rendered artefacts) is content-store data, per ADR-0042.
function atlassianClientFor(options) {
  const { owner, repository, pat, baseUrl, branch } = options.atlassian
  return { client: resolveContentStore('atlassian', { owner, repository, pat, baseUrl }), branch }
}
...
export function createInstance(definitionId, slug, options = {}) {
  ...
  if (options.github) {
    return createInstanceInGitHub(definitionId, slug, definition, firstStage, { ...options, definitionVersion: pinnedVersion })
  }
  ...
  storage.writeText(
    ...
    stringifyInstanceYAML({ definition: definitionId, slug, stage: firstStage.id, assignee: options.assignee ?? '', definitionVersion: pinnedVersion, ...persistedNumberField(options) })
  )
...
async function ensureWorkspaceReadme(client, branch, { slug, displayTitle, definitionTitle, definitionDescription, location, providerLabel = 'Azure DevOps' }) {
  try {
    ...
    console.error(
      `lib/instance.js: could not write repo-root README.md for workspace instance "${slug}" at ${providerLabel} ${location} — ${err.message}`
    )
  }
}
...
/**
 * The GitHub twin of createInstanceInAzureDevOps above (#11): same blank first-stage template, same
 * repo-existence guard, same "instance.yaml already exists at this slug" 409-mapped rejection, same
 * best-effort first-instance README, same non-atomic-per-file partial-failure reporting — only the
 * client and the repo-relative path builders differ. There is no import-a-real-instance counterpart
 * yet (`importInstanceToAzureDevOps`'s job — #18, "Adopt an existing GitHub repo as a workspace", is a
 * separate ticket). Instance creation always lands on `main` regardless of provider — creating the
 * skeleton is not itself a stage "Save" (CONTEXT.md), so there is no stage branch to target yet; the
 * first real write via `writeModule`/`writeModules` is what starts one (#12, `lib/githubStageBranch.js`).
 */
async function createInstanceInGitHub(definitionId, slug, definition, firstStage, options) {
  const { client, branch } = githubClientFor(options)
  const { owner, repository } = options.github
  const location = `${owner}/${repository}`

  if (!(await client.repoExists())) {
    throw new RepoNotFoundError(repoNotFoundMessage(owner, repository), { provider: GITHUB_PROVIDER })
  }

  const alreadyExists = await client
    .getFileContent(githubInstancePath(slug), { branch })
    .then(() => true)
    .catch((err) => {
      if (err instanceof NotFoundError) return false
      throw err
  ...
  if (alreadyExists) {
    throw new Error(`Instance "${slug}" already exists at GitHub ${location}`)
  }

  const pinnedVersionForGitHub = options.definitionVersion ?? resolvePinnedVersion(definitionId, options)
  await client.writeFile(
    githubInstancePath(slug),
    stringifyInstanceYAML({
      definition: definitionId,
      slug,
      stage: firstStage.id,
      assignee: options.assignee ?? '',
      definitionVersion: pinnedVersionForGitHub,
      ...persistedNumberField(options),
  ...
  try {
    for (const moduleId of firstStage.modules) {
      const moduleSpec = definition.modules.get(moduleId)
      await client.writeFile(
        githubModulePath(slug, moduleId),
        renderModuleFile(moduleSpec, { owner: options.owner ?? '' }),
        { message: `Add module "${moduleId}"`, branch }
      )
      writtenModules.push(moduleId)
    ...
    const remaining = firstStage.modules.filter((moduleId) => !writtenModules.includes(moduleId))
    throw new Error(
      `Instance "${slug}" was only partially created at GitHub ${location}: instance.yaml and module(s) ` +
        `${writtenModules.join(', ') || '(none)'} were written, but module "${remaining[0]}" failed (${err.message}). ` +
        `instance.yaml now exists there, so re-running createInstance for this instance will reject as "already ` +
        `exists" — call writeModule directly for the remaining module(s) (${remaining.join(', ')}) to finish it.`
    )
...
export function readInstance(slug, options = {}) {
  ...
  if (options.github) {
    return readInstanceFromGitHub(slug, options)
  }
  if (options.gitlab) {
    return readInstanceFromGitLab(slug, options)
  }
  if (options.atlassian) {
    return readInstanceFromAtlassian(slug, options)
  }
...
// The GitHub twin of readInstanceFromAzureDevOps above (#11) — same parseYAML call, same merge-
// conflict-marker guard, over content fetched from a GitHub repo instead.
async function readInstanceFromGitHub(slug, options) {
  const { client, branch } = githubClientFor(options)
  let text
  try {
    text = await client.getFileContent(githubInstancePath(slug), { branch })
  } catch (err) {
    if (err instanceof NotFoundError) {
      const { owner, repository } = options.github
      throw new Error(`No instance "${slug}" at GitHub ${owner}/${repository}.`)
    }
    throw err
  }
  assertNoMergeConflictMarkers(text, { slug, source: `GitHub${options.github.branch ? ` branch "${options.github.branch}"` : ''}` })
  return withDefaultAssignee(parseYAML(text))
}

// #31: the GitLab twin of readInstanceFromGitHub above — same parseYAML call, same merge-conflict-
// marker guard, over content fetched from a GitLab project instead.
async function readInstanceFromGitLab(slug, options) {
  const { client, branch } = gitlabClientFor(options)
  let text
  try {
    text = await client.getFileContent(gitlabInstancePath(slug), { branch })
  } catch (err) {
    if (err instanceof NotFoundError) {
      const { namespace, repository } = options.gitlab
      throw new Error(`No instance "${slug}" at GitLab ${namespace}/${repository}.`)
    }
    throw err
  }
  assertNoMergeConflictMarkers(text, { slug, source: `GitLab${options.gitlab.branch ? ` branch "${options.gitlab.branch}"` : ''}` })
  return withDefaultAssignee(parseYAML(text))
}

// #43 — the Atlassian (Bitbucket-backed) twin of readInstanceFromGitLab above — same parseYAML call,
// same merge-conflict-marker guard, over content fetched from a Bitbucket Cloud repo instead.
async function readInstanceFromAtlassian(slug, options) {
  const { client, branch } = atlassianClientFor(options)
  let text
  try {
    text = await client.getFileContent(atlassianInstancePath(slug), { branch })
  } catch (err) {
    if (err instanceof NotFoundError) {
      const { owner, repository } = options.atlassian
      throw new Error(`No instance "${slug}" at Bitbucket ${owner}/${repository}.`)
    }
    throw err
  }
  assertNoMergeConflictMarkers(text, { slug, source: `Bitbucket${options.atlassian.branch ? ` branch "${options.atlassian.branch}"` : ''}` })
  return withDefaultAssignee(parseYAML(text))
}
 ...
 *
 * With `options.azureDevOps` / `options.github` / `options.gitlab` supplied, updates `instance.yaml` in
 * that repo instead of the local filesystem and returns a Promise the caller must `await` (#124 —
 * previously Azure-DevOps-or-local only, so updating a GitHub- or GitLab-backed instance's assignee
 * silently wrote onto a local directory its data never actually lived in). Without any of them — every
 * pre-existing local caller — this stays synchronous.
 */
export function updateInstanceAssignee(slug, assignee, options = {}) {
  ...
  if (options.github) {
    return updateInstanceAssigneeInGitHub(slug, assignee, options)
  }
  if (options.gitlab) {
    return updateInstanceAssigneeInGitLab(slug, assignee, options)
  }
...
// #124: the GitHub twin of updateInstanceAssigneeInAzureDevOps above — same read-modify-write shape,
// over a GitHub-backed instance.yaml instead.
async function updateInstanceAssigneeInGitHub(slug, assignee, options) {
  const { client, branch } = githubClientFor(options)
  const current = await readInstanceFromGitHub(slug, options)
  const next = { ...current, assignee }
  await client.writeFile(githubInstancePath(slug), stringifyInstanceYAML(next), {
    message: `Update assignee for instance "${slug}"`,
    branch,
  })
  return next
...
// #124: the GitLab twin of updateInstanceAssigneeInGitHub above — same read-modify-write shape, over a
// GitLab-backed instance.yaml instead.
async function updateInstanceAssigneeInGitLab(slug, assignee, options) {
  const { client, branch } = gitlabClientFor(options)
  const current = await readInstanceFromGitLab(slug, options)
  const next = { ...current, assignee }
  await client.writeFile(gitlabInstancePath(slug), stringifyInstanceYAML(next), {
    message: `Update assignee for instance "${slug}"`,
    branch,
  })
  return next
...
export function writeInstanceStage(slug, stageId, options = {}) {
  ...
  if (options.github) {
    return writeInstanceStageInGitHub(slug, stageId, options)
  }
  if (options.gitlab) {
    return writeInstanceStageInGitLab(slug, stageId, options)
  }
  if (options.atlassian) {
    return writeInstanceStageInAtlassian(slug, stageId, options)
  }
...
// The GitHub twin of writeInstanceStageInAzureDevOps above (#13): the same "Check status" advance,
// over a GitHub-backed instance. Targets `main` for the identical reason — the just-merged Pull
// Request already put the stage's approved content there.
async function writeInstanceStageInGitHub(slug, stageId, options) {
  const { client, branch } = githubClientFor(options)
  const current = await readInstanceFromGitHub(slug, options)
  const next = { ...current, stage: stageId }
  await client.writeFile(githubInstancePath(slug), stringifyInstanceYAML(next), {
...
// The GitLab twin of writeInstanceStageInGitHub above (#33, ADR-0041): the same "Check status"
// advance, over a GitLab-backed instance. Targets `main` for the identical reason — the just-merged
// Merge Request already put the stage's approved content there.
async function writeInstanceStageInGitLab(slug, stageId, options) {
  const { client, branch } = gitlabClientFor(options)
  const current = await readInstanceFromGitLab(slug, options)
  const next = { ...current, stage: stageId }
  await client.writeFile(gitlabInstancePath(slug), stringifyInstanceYAML(next), {
    message: `Advance instance "${slug}" to stage "${stageId}" (Merge Request merged)`,
    branch,
  })
  return next
}

// The Atlassian (Bitbucket-backed) twin of writeInstanceStageInGitLab above (#46, ADR-0042): the same
// "Check status" advance, over an Atlassian-backed instance. Targets `main` for the identical reason —
// the just-merged pull request already put the stage's approved content there.
async function writeInstanceStageInAtlassian(slug, stageId, options) {
  const { client, branch } = atlassianClientFor(options)
  const current = await readInstanceFromAtlassian(slug, options)
  const next = { ...current, stage: stageId }
  await client.writeFile(atlassianInstancePath(slug), stringifyInstanceYAML(next), {
    message: `Advance instance "${slug}" to stage "${stageId}" (Pull Request merged)`,
    branch,
  })
  return next
...
export function recordInstanceWorkItemLink(slug, workItem, options = {}) {
  ...
  if (options.github) {
    return recordInstanceWorkItemLinkInGitHub(slug, workItem, options)
  }
  if (options.gitlab) {
    return recordInstanceWorkItemLinkInGitLab(slug, workItem, options)
  }
...
// The GitHub twin of recordInstanceWorkItemLinkInAzureDevOps above (#14) — same read-modify-write
// shape, over a GitHub-backed instance.yaml instead. `workItem` here always carries
// `provider: 'github'` (`lib/workItemLink.js`'s `linkInstanceToWorkItem`), which is what lets a later
// `readInstance` tell a GitHub-linked issue apart from an (implicitly `provider`-less) Azure DevOps
// work item link without a second field.
async function recordInstanceWorkItemLinkInGitHub(slug, workItem, options) {
  const { client, branch } = githubClientFor(options)
  const current = await readInstanceFromGitHub(slug, options)
  const next = { ...current, workItem }
  await client.writeFile(githubInstancePath(slug), stringifyInstanceYAML(next), {
    message: `Link instance "${slug}" to GitHub issue #${workItem.parentNumber}`,
    branch,
  })
  return next
}

// #124: the GitLab twin of recordInstanceWorkItemLinkInGitHub above — same read-modify-write shape,
// over a GitLab-backed instance.yaml instead. Previously missing entirely (this codepath fell through
// to the local branch above, writing the link onto whatever local directory happened to share this
// GitLab-backed instance's slug — POST /api/instance/work-items/link's own `gitlab` branch, lib/server.js).
async function recordInstanceWorkItemLinkInGitLab(slug, workItem, options) {
  const { client, branch } = gitlabClientFor(options)
  const current = await readInstanceFromGitLab(slug, options)
  const next = { ...current, workItem }
  await client.writeFile(gitlabInstancePath(slug), stringifyInstanceYAML(next), {
    message: `Link instance "${slug}" to GitLab issue #${workItem.parentIid}`,
    branch,
  })
  return next
}
...
export async function recordInstancePullRequest(slug, stageId, pullRequestId, options = {}) {
  if (!options.azureDevOps && !options.github && !options.gitlab && !options.atlassian) {
    throw new Error('recordInstancePullRequest is for Workspace-backed instances only — pass options.azureDevOps, options.github, options.gitlab or options.atlassian')
  }
  const { client, branch } = options.azureDevOps
    ? azureDevOpsClientFor(options)
    : options.github
      ? githubClientFor(options)
      : options.gitlab
        ? gitlabClientFor(options)
        : atlassianClientFor(options)
  const current = options.azureDevOps
    ? await readInstanceFromAzureDevOps(slug, options)
    : options.github
      ? await readInstanceFromGitHub(slug, options)
      : options.gitlab
        ? await readInstanceFromGitLab(slug, options)
        : await readInstanceFromAtlassian(slug, options)
  ...
  const path = options.azureDevOps
    ? azureDevOpsInstancePath(slug)
    : options.github
      ? githubInstancePath(slug)
      : options.gitlab
        ? gitlabInstancePath(slug)
        : atlassianInstancePath(slug)
  await client.writeFile(path, stringifyInstanceYAML(next), {
...
export async function recordInstancePullRequestStatus(slug, stageId, status, options = {}) {
  if (!options.azureDevOps && !options.github && !options.gitlab && !options.atlassian) {
    throw new Error('recordInstancePullRequestStatus is for Workspace-backed instances only — pass options.azureDevOps, options.github, options.gitlab or options.atlassian')
  }
  if (status === undefined) {
...
export async function recordInstanceReviewRequest(slug, stageId, review, options = {}) {
  if (options.github) {
    return recordInstanceReviewRequestInGitHub(slug, stageId, review, options)
  }
  if (options.gitlab) {
    return recordInstanceReviewRequestInGitLab(slug, stageId, review, options)
  }
  if (options.atlassian) {
    return recordInstanceReviewRequestInAtlassian(slug, stageId, review, options)
  }
  if (!options.azureDevOps) {
    throw new Error('recordInstanceReviewRequest is for Workspace-backed instances only — pass options.azureDevOps, options.github, options.gitlab or options.atlassian')
  }
  return withReviewWriteLock(slug, async () => {
    ...
    // Routed through stringifyInstanceYAML (not a bare stringifyYAML) like every other instance.yaml
    // writer in this file — canonical key ordering is what lets two stacked stages' independent review
    // writes 3-way-merge cleanly (see stringifyInstanceYAML's own doc comment above).
    await client.writeFile(azureDevOpsInstancePath(slug), stringifyInstanceYAML(next), {
...
// The GitHub twin of the Azure DevOps branch above (#15) — same read-modify-write shape, over a
// GitHub-backed instance.yaml instead. `review.workItemId` here is a GitHub issue number, not an
// Azure DevOps work item id.
async function recordInstanceReviewRequestInGitHub(slug, stageId, review, options) {
  return withReviewWriteLock(slug, async () => {
    const { client, branch } = githubClientFor(options)
    const current = await readInstanceFromGitHub(slug, options)
    const reviewRequests = { ...(current.reviewRequests ?? {}) }
    ...
    const next = { ...current, reviewRequests }
    await client.writeFile(githubInstancePath(slug), stringifyInstanceYAML(next), {
      message: `Request review for stage "${stageId}" of instance "${slug}" (issue #${review.workItemId})`,
      branch,
    })
    return next
  })
...
// The GitLab twin of the GitHub branch above (#34) — same read-modify-write shape, over a
// GitLab-backed instance.yaml instead. `review.workItemId` here is a GitLab issue iid.
async function recordInstanceReviewRequestInGitLab(slug, stageId, review, options) {
  return withReviewWriteLock(slug, async () => {
    const { client, branch } = gitlabClientFor(options)
    const current = await readInstanceFromGitLab(slug, options)
    const reviewRequests = { ...(current.reviewRequests ?? {}) }
...
// The Atlassian twin of the GitLab branch above (#47) — same read-modify-write shape, over a
// Bitbucket-backed instance.yaml instead (Bitbucket is the content-store half of the Atlassian suite
// instance.yaml itself lives in, ADR-0042 — this never touches Jira). `review.workItemId` here is a
// Jira issue's own globally-unique numeric id, not its human-facing key (see lib/stageReview.js's own
// `requestStageReviewAtlassian` doc comment for why).
async function recordInstanceReviewRequestInAtlassian(slug, stageId, review, options) {
  return withReviewWriteLock(slug, async () => {
    const { client, branch } = atlassianClientFor(options)
    const current = await readInstanceFromAtlassian(slug, options)
    const reviewRequests = { ...(current.reviewRequests ?? {}) }
    ...
    const next = { ...current, reviewRequests }
    await client.writeFile(atlassianInstancePath(slug), stringifyInstanceYAML(next), {
      message: `Request review for stage "${stageId}" of instance "${slug}" (Jira issue ${review.workItemId})`,
      branch,
    })
    return next
  })
...
/**
 * Updates the cached native review status for one review request — a
 * `Custom.GantryReviewStatus` value for Azure DevOps, a `gantry:review/*`
 * label's mapped value for GitHub. The state is intentionally persisted only
 * after an explicit status check; a page load never polls review work items.
 */
export async function recordInstanceReviewStatus(slug, stageId, workItemId, status, options = {}) {
  if (options.github) {
    return recordInstanceReviewStatusInGitHub(slug, stageId, workItemId, status, options)
  }
  if (options.gitlab) {
    return recordInstanceReviewStatusInGitLab(slug, stageId, workItemId, status, options)
  }
  if (options.atlassian) {
    return recordInstanceReviewStatusInAtlassian(slug, stageId, workItemId, status, options)
  }
  if (!options.azureDevOps) {
    throw new Error('recordInstanceReviewStatus is for Workspace-backed instances only — pass options.azureDevOps, options.github, options.gitlab or options.atlassian')
  }
  return withReviewWriteLock(slug, async () => {
    const { client, branch } = azureDevOpsClientFor(options)
    const current = await readInstanceFromAzureDevOps(slug, options)
...
// The GitHub twin of the Azure DevOps branch above (#15) — same read-modify-write shape and the same
// "no matching review request" guard, over a GitHub-backed instance.yaml instead.
async function recordInstanceReviewStatusInGitHub(slug, stageId, workItemId, status, options) {
  return withReviewWriteLock(slug, async () => {
    const { client, branch } = githubClientFor(options)
    const current = await readInstanceFromGitHub(slug, options)
    const reviewRequests = { ...(current.reviewRequests ?? {}) }
    ...
    const next = { ...current, reviewRequests }
    await client.writeFile(githubInstancePath(slug), stringifyInstanceYAML(next), {
      message: `Update review status for stage "${stageId}" of instance "${slug}" (issue #${workItemId})`,
      branch,
    })
    return next
  })
...
// The GitLab twin of the GitHub branch above (#34) — same read-modify-write shape and the same
// "no matching review request" guard, over a GitLab-backed instance.yaml instead.
async function recordInstanceReviewStatusInGitLab(slug, stageId, workItemId, status, options) {
  return withReviewWriteLock(slug, async () => {
    const { client, branch } = gitlabClientFor(options)
    const current = await readInstanceFromGitLab(slug, options)
    const reviewRequests = { ...(current.reviewRequests ?? {}) }
...
// The Atlassian twin of the GitLab branch above (#47) — same read-modify-write shape and the same
// "no matching review request" guard, over a Bitbucket-backed instance.yaml instead. `workItemId` here
// is a Jira issue's own globally-unique numeric id (see recordInstanceReviewRequestInAtlassian above).
async function recordInstanceReviewStatusInAtlassian(slug, stageId, workItemId, status, options) {
  return withReviewWriteLock(slug, async () => {
    const { client, branch } = atlassianClientFor(options)
    const current = await readInstanceFromAtlassian(slug, options)
    const reviewRequests = { ...(current.reviewRequests ?? {}) }
    ...
    const next = { ...current, reviewRequests }
    await client.writeFile(atlassianInstancePath(slug), stringifyInstanceYAML(next), {
      message: `Update review status for stage "${stageId}" of instance "${slug}" (Jira issue ${workItemId})`,
      branch,
    })
    return next
  })
 ...
 *
 * Dual-backend like every other instance.yaml writer, extended to every Workspace-backed provider
 * (#124 — previously Azure-DevOps-or-local only, silently writing a Provider-backed instance's
 * override onto whatever *local* directory happened to share its slug): with `options.azureDevOps` /
 * `options.github` / `options.gitlab` supplied, updates `instance.yaml` in that repo (on whatever
 * branch the caller resolved — lib/server.js's PUT route passes the viewed stage's own branch via
 * resolveStageBranch/resolveGitHubStageBranch/resolveGitLabStageBranch, matching #122's write
 * convention) instead of the local filesystem and returns a Promise the caller must `await`. Without
 * any of them — the local path — this stays synchronous.
 */
export function recordSyncedFieldOverrides(slug, stageId, updates, options = {}) {
  if (options.azureDevOps || options.github || options.gitlab) {
    return recordSyncedFieldOverridesInProvider(slug, stageId, updates, options)
  }
...
// The GitHub/GitLab twin of the Azure-DevOps-only implementation this replaced (#124) — same
// read-modify-write shape, dispatched by whichever provider's own base options the caller supplied.
// Mirrors recordInstanceReopened/clearInstanceReopened's own ternary-ladder dispatch above rather than
// three near-identical named functions, since this body genuinely is just "pick the client/read/path
// for the one provider that's actually present".
async function recordSyncedFieldOverridesInProvider(slug, stageId, updates, options) {
  const { client, branch } = options.azureDevOps
    ? azureDevOpsClientFor(options)
    : options.github
      ? githubClientFor(options)
  ...
  const current = options.azureDevOps
    ...
    : options.github
      ...
      : await readInstanceFromGitLab(slug, options)
  const next = applySyncedFieldOverrides(current, stageId, updates)
  const path = options.azureDevOps ? azureDevOpsInstancePath(slug) : options.github ? githubInstancePath(slug) : gitlabInstancePath(slug)
  await client.writeFile(path, stringifyInstanceYAML(next), {
...
export async function recordInstanceReopened(slug, stageId, previousStage, options = {}) {
  if (!options.azureDevOps && !options.github && !options.gitlab) {
    throw new Error('recordInstanceReopened is for Workspace-backed instances only — pass options.azureDevOps, options.github or options.gitlab')
  }
  const { client, branch } = options.azureDevOps
    ? azureDevOpsClientFor(options)
    : options.github
...
export async function clearInstanceReopened(slug, stageId, options = {}) {
  if (!options.azureDevOps && !options.github && !options.gitlab && !options.atlassian) {
    throw new Error('clearInstanceReopened is for Workspace-backed instances only — pass options.azureDevOps, options.github, options.gitlab or options.atlassian')
  }
  const { client, branch } = options.azureDevOps
    ? azureDevOpsClientFor(options)
    : options.github
...
// #109 (docs/adr/0047): a self-healing write triggered from inside a read — the ADR-0016
// heading-scale migration below is the one this guards, and lib/syncedFields.js's own
// work-item-link recovery uses the identical check — must never persist using a credential this
// read resolved to nothing but a workspace's shared, read-only-intended one. `sharedCache.cacheReads`
// (lib/server.js's `sharedCacheContext`) is true exactly when that happened: a request that carried
// no credential of its own, answered only by the workspace's own `GANTRY_SHARED_WORKSPACE_PATS`
// entry — the one case docs/adr/0047 says must never reach a write. Before #125 gave a read call site
// this fallback at all, every credential reaching this function was already a real requester's own,
// so this migration write was always safe; skipping the persistence here is what keeps that true now
// that the same read call sites can also be answered with no requester credential at all. The
// migrated text is still parsed and returned either way — a shared-credential read sees corrected
// content immediately, it just isn't the read that commits the fix; the next real-credentialed read
// (an architect's own, or a future visitor who happens to bring one) does that instead.
function readIsSharedCredentialOnly(options, providerKey) {
  return Boolean(options[providerKey]?.sharedCache?.cacheReads)
}
...
export function writeModule(definition, slug, moduleId, data, options = {}) {
  ...
  if (options.github) {
    return writeModuleToGitHub(slug, moduleId, text, options)
  }
...
export function writeModules(definition, slug, modules, options = {}) {
  ...
  if (options.github) {
    // #11's own acceptance criterion — "one Save produces one commit covering every changed module" —
    // is exactly why this goes through the client's own single-commit `writeFiles`, not a loop of
    // per-module `writeFile` calls.
    const { client, branch } = githubClientFor(options)
    const files = texts.map(({ moduleId, text }) => ({ path: githubModulePath(slug, moduleId), content: text }))
    return client
      .writeFiles(files, { branch, message: options.message ?? `Update modules ${moduleIds.join(', ')}` })
      .then(({ push }) => ({ modules: moduleIds, commit: push?.commits?.[0]?.commitId ?? null }))
  }
...
async function writeModuleToGitHub(slug, moduleId, text, options) {
  const { client, branch } = githubClientFor(options)
  const path = githubModulePath(slug, moduleId)
  await client.writeFile(path, text, { message: `Update module "${moduleId}"`, branch })
  return { module: moduleId, path }
}
...
export function readModule(definition, slug, moduleId, options = {}) {
  ...
  if (options.github) {
    return readModuleFromGitHub(moduleSpec, slug, moduleId, options, strict)
  }
  if (options.gitlab) {
    return readModuleFromGitLab(moduleSpec, slug, moduleId, options, strict)
  }
  if (options.atlassian) {
    return readModuleFromAtlassian(moduleSpec, slug, moduleId, options, strict)
  }
...
async function readModuleFromAzureDevOps(moduleSpec, slug, moduleId, options, strict) {
  ...
  // The same lazy heading-scale migration as the local path above (ADR-0016), pushed back as its own commit so the remote repo's file is new-scale too — queued behind any other in-flight migration push so concurrent readers can't race each other's commits (see queueMigrationWrite). Never with a shared-only credential (see readIsSharedCredentialOnly's own doc comment) — docs/adr/0047's "never usable for a write" line applies to this write exactly as it does to any other.
  const migrated = migrateModuleHeadingScale(text, moduleSpec)
  if (migrated !== text && !readIsSharedCredentialOnly(options, 'azureDevOps')) {
...
// The GitHub twin of readModuleFromAzureDevOps above (#11) — same "no saved data yet" miss, same
// lazy heading-scale migration pushed back as its own commit.
async function readModuleFromGitHub(moduleSpec, slug, moduleId, options, strict) {
  const { client, branch } = githubClientFor(options)
  const path = githubModulePath(slug, moduleId)
  let text
  try {
    ...
    if (err instanceof NotFoundError) {
      throw new Error(
        `Module "${moduleId}" has no saved data for instance "${slug}" (expected ${path} in GitHub) — fill in its fields and save before rendering`
      )
    }
    throw err
  }
  // Never with a shared-only credential — see readModuleFromAzureDevOps's identical guard above.
  const migrated = migrateModuleHeadingScale(text, moduleSpec)
  if (migrated !== text && !readIsSharedCredentialOnly(options, 'github')) {
    await queueMigrationWrite(() =>
      client.writeFile(path, migrated, {
        message: `Migrate module "${moduleId}" headings to the # / ## scale (ADR-0016)`,
        branch,
...
// #31 — the GitLab twin of readModuleFromGitHub above: same "no saved data yet" miss, same lazy
// heading-scale migration pushed back as its own commit.
async function readModuleFromGitLab(moduleSpec, slug, moduleId, options, strict) {
  const { client, branch } = gitlabClientFor(options)
  const path = gitlabModulePath(slug, moduleId)
  let text
  try {
    text = await client.getFileContent(path, { branch })
  } catch (err) {
    if (err instanceof NotFoundError) {
      throw new Error(
        `Module "${moduleId}" has no saved data for instance "${slug}" (expected ${path} in GitLab) — fill in its fields and save before rendering`
      )
    }
    throw err
  }
  // Never with a shared-only credential — see readModuleFromAzureDevOps's identical guard above.
  const migrated = migrateModuleHeadingScale(text, moduleSpec)
  if (migrated !== text && !readIsSharedCredentialOnly(options, 'gitlab')) {
    await queueMigrationWrite(() =>
      client.writeFile(path, migrated, {
        message: `Migrate module "${moduleId}" headings to the # / ## scale (ADR-0016)`,
        branch,
...
// #43 — the Atlassian (Bitbucket-backed) twin of readModuleFromGitLab above: same "no saved data yet"
// miss, same lazy heading-scale migration pushed back as its own commit.
async function readModuleFromAtlassian(moduleSpec, slug, moduleId, options, strict) {
  const { client, branch } = atlassianClientFor(options)
  const path = atlassianModulePath(slug, moduleId)
  let text
  try {
    text = await client.getFileContent(path, { branch })
  } catch (err) {
    if (err instanceof NotFoundError) {
      throw new Error(
        `Module "${moduleId}" has no saved data for instance "${slug}" (expected ${path} in Bitbucket) — fill in its fields and save before rendering`
      )
    }
    throw err
  }
  // Never with a shared-only credential — see readModuleFromAzureDevOps's identical guard above.
  // (Atlassian has no shared-read surface yet, per lib/providerRegistry.js's resolveContentStore doc
  // comment, so `options.atlassian?.sharedCache` never carries `cacheReads: true` today — this guard
  // is here so that stays true by construction rather than by omission the moment it does.)
  const migrated = migrateModuleHeadingScale(text, moduleSpec)
  if (migrated !== text && !readIsSharedCredentialOnly(options, 'atlassian')) {
    await queueMigrationWrite(() =>
      client.writeFile(path, migrated, {
        message: `Migrate module "${moduleId}" headings to the # / ## scale (ADR-0016)`,
        branch,
