...
export function parseModuleFile(text, moduleSpec, options = {}) {
  ...
  for (let i = 0; i < headings.length; i++) {
    ...
    if (!fieldDef) {
      // A heading matching no defined field is a preserved custom field (#132), not a warning — see this function's doc comment.
      const id = uniqueCustomFieldId(title, usedCustomIds)
      const custom = { id, title: rawTitle.trim(), value }
...
function renderModuleInstanceFile(moduleId, moduleSpec, data) {
  ...
  for (const entry of data.layout ?? []) {
    if (entry.field !== undefined) {
      ...
      sections.push(sectionText(entry.custom.title ?? '', entry.custom.value ?? ''))
    }
  }
  for (const field of moduleSpec.fields) {
