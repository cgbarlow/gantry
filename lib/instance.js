...
export function azureDevOpsOutPath(slug, instanceName, artefactTitle) {
  assertValidAzureDevOpsSlug(slug)
  return `${AZURE_DEVOPS_WORKSPACE_ROOT}/${slug}/out/${renderedArtefactBasename(instanceName, artefactTitle)}.docx`
}
...
async function createInstanceInAzureDevOps(definitionId, slug, definition, firstStage, options) {
  ...
  // README untouched). Best-effort, in the same spirit as the module loop's
  // partial-failure reporting below: instance.yaml has already been committed at
  // this point, so a README read/write failure is logged and skipped rather than
