...
// Defaults `assignee` to `''` on top of a parsed instance.yaml (#97) —
// but only when parsing actually produced an object. A blank/malformed
// instance.yaml parses to `null` (yaml's own documented behavior for empty
// content), and spreading `null` into an object literal is a silent no-op
// in JS — naively doing `{ assignee: '', ...parseYAML(text) }` would turn
// that `null` into `{ assignee: '' }`, masking the read as if it had
// succeeded and deferring the failure to a later, less clear error deep
// inside whatever the caller does next (e.g. `loadDefinition` rejecting an
// `undefined` id) instead of surfacing it immediately at the read itself,
// the same way every other malformed-instance.yaml case already does.
function withDefaultAssignee(parsed) {
  if (parsed === null || parsed === undefined) return parsed
  return { assignee: '', ...parsed }
}

// Repo-relative paths for an Azure-DevOps-backed instance, under the
// per-slug `gantry-workspace/<slug>/` subdirectory (docs/adr/0009 and
// docs/adr/0005-instance-data-in-external-ado-repo.md) rather than repo
// root — this is what lets one Azure DevOps repo ("workspace",
// lib/workspaceRegistry.js, #96) host more than one instance, each in its
// own slug-named subdirectory, instead of the whole repo being exactly one
// instance's data as it was before #100. Exported so lib/render.js
// (rendered-artefact output) and lib/repoCheck.js (the wizard's "check
// repo" probe, and its legacy-root migration routine) build the exact
// same paths rather than each duplicating this prefix convention.
export const AZURE_DEVOPS_WORKSPACE_ROOT = 'gantry-workspace'

// Every caller of the three path builders below is expected to have
// already validated `slug` (every client-supplied slug is checked at the
// HTTP layer, lib/server.js's isValidSlug; every slug discovered from a
// remote repo's own instance.yaml is checked in lib/repoCheck.js before
// it's ever used to build a path) — but asserting it again here too, at
// the one place `slug` actually becomes part of a remote Azure DevOps
// path, means that protection holds even for a future or overlooked
// caller that doesn't happen to validate upstream, rather than relying
// entirely on every call site remembering to.
...
// The pre-#100 repo-root paths (no `gantry-workspace/<slug>/` prefix at
// all) an Azure-DevOps-backed instance's data used to live at, back when
// the whole repo was necessarily exactly one instance. Kept only for
// lib/repoCheck.js's one-time migration routine to detect and move off of
// — nothing else should ever read or write these once a repo has been
// migrated.
 ...
 * branch? }`) — the credential/location "accepted as a plain parameter at
 * this layer" that #82's spec calls for (wiring it from a real HTTP
 * request, or from a lookup keyed on `slug`, is later work). Its
 * *presence* on `options` — never anything read off a fetched
 * `instance.yaml` — is what routes a call through Azure DevOps instead of
 * the local filesystem: for a real Azure-DevOps-backed instance,
 * `instance.yaml` itself lives only in that repo, so there is nothing to
 * read locally first that could tell us where to look.
 *
 * Also returns `branch` — the specific branch every read/write below
 * should target — straight off `options.azureDevOps.branch`, left
 * `undefined` when the caller didn't supply one so every downstream
 * `client.getFileContent`/`writeFile`/`listFolder`/`deleteFile` call falls
 * through to that client's own `'main'` default (#118) rather than this
 * function silently picking a value the caller never asked for. A real
 * per-stage branch (#122) is chosen by the caller, not decided here.
 */
...
export function createInstance(definitionId, slug, options = {}) {
  ...
  // `assignee` (#97) — the instance record's own single named person,
  // optional and empty by default — is a field on the instance itself, not
  // derived from any module. Distinct from `options.owner` below (unaffected
  // by #97): that seeds each first-stage module file's own frontmatter
  // `owner`, the separate Design Authority sign-off convention docs/adr/0001
  // and CONTEXT.md describe, which #97 explicitly leaves untouched.
  storage.writeText(
...
async function createInstanceInAzureDevOps(definitionId, slug, definition, firstStage, options) {
  ...
  await client.writeFile(
    ...
    stringifyYAML({
      ...
      slug,
      stage: firstStage.id,
      // See createInstance's own comment on `assignee` vs. `options.owner`
      // (#97) — the same explicit, module-independent field, just written
      // to the Azure-DevOps-backed instance.yaml instead of a local one.
...
// Exported (not just used internally by listInstances below) so
// lib/instanceRegistry.js's auto-backfill can find every instance actually
// on disk without re-implementing this same directory-scan-plus-instance.yaml-
// check — the registry's "any instance.yaml found on disk with no existing
// registry entry gets one added automatically" behavior (#89) is exactly
// this same scan.
 ...
 *
 * `assignee` defaults to `''` when the on-disk instance.yaml predates #97
 * (every instance created before this ticket, including the
 * examples/demo-cli/demo-web fixtures) — the same "default, not a
 * migration" treatment ADR-0008 gave the instance registry itself.
 */
...
/**
 * Updates the instance record's own stored `assignee` (#97) — a single
 * named person, or `''` to clear it — preserving every other field already
 * on `instance.yaml` (`definition`, `slug`, `stage`, and, for an
 * Azure-DevOps-backed instance, its descriptive `azureDevOps` block): this
 * reads the current instance first and only overwrites `assignee` on top of
 * it, rather than reconstructing the file from scratch. Stage transitions
 * never touch this field (there is no code path that changes `stage` at
 * all yet — see #97's own investigation), so an assignee set here stays put
 * across whichever stage the instance is later viewed/moved to.
 *
 * With `options.azureDevOps` supplied, updates `instance.yaml` in that
...
/**
 * Writes the instance record's own persisted `stage` pointer — the same
 * field `evaluateStage`/`getStatus`/`checkGate` default to reading when no
 * `stageId`/`gate` is otherwise given. The same read-modify-write shape as
 * `updateInstanceAssignee` above: preserves every other field already on
 * `instance.yaml` (`definition`, `slug`, `assignee`, `workItem`, etc.),
 * rather than reconstructing the file from scratch.
 *
 * Doesn't validate `stageId` against the instance's own definition, or
 * check any gate — that's the caller's job. `lib/stageAdvancement.js`'s
 * `advanceStage` (#115, the local instance "Advance to next stage"
 * self-serve action, ADR-0012) is this codebase's one caller: it only ever
 * calls this with a stage id it already confirmed is the definition's
 * genuine next stage, once that stage's own gate has genuinely passed.
 *
 * Local instances only (`options.instancesDir`) — unlike every other
 * instance.yaml writer in this file, this has no `options.azureDevOps`
 * branch. A Workspace-backed instance never advances this way at all: it
 * only moves to its next stage once that stage's own Pull Request is
 * merged (ADR-0014, "PR-based stage approval" — #122-#125's own
 * mechanism; not yet in this repo as of this writing, see #106), a
 * mechanism entirely separate from this direct instance.yaml write.
 */
...
/**
 * Records `workItem` — the Azure DevOps work-item link `lib/workItemLink.js`'s
 * `linkInstanceToWorkItem` (#103) creates — on the instance record:
 * `{ organization, project, workItemType, parentId, baseUrl?, stages:
 * { [stageId]: childWorkItemId } }`. Follows the exact same read-modify-write
 * shape as `instance.yaml`'s own pre-existing descriptive `azureDevOps`
 * field (ADR-0008: "purely descriptive... never reconciled"): every other
 * field already on `instance.yaml` (`definition`, `slug`, `stage`, and, for
 * an Azure-DevOps-backed instance, its own `azureDevOps` block) is preserved
 * untouched. An instance with no `workItem` field is simply unlinked —
 * behaves exactly as it did before this ticket (#103's first acceptance
 * criterion) — so this is never called implicitly, only from an explicit
 * "link this instance" action.
 *
 * With `options.azureDevOps` supplied, updates `instance.yaml` in that
 * Azure DevOps repo instead of the local filesystem and returns a Promise
 * the caller must `await`. Without it — every existing caller — this stays
 * synchronous. Note this `options.azureDevOps` is the instance's own *data*
 * storage location (git-backed), independent of `workItem.organization`/
 * `workItem.project` (the Work Items API's own org/project) — an instance
 * can be locally stored yet linked to a work item in some Azure DevOps
 * project, or Azure-DevOps-backed for its data yet linked to a work item in
 * a different project entirely; this function never assumes the two match.
 */
 ...
 *
 * `options.strict` is forwarded to `parseModuleFile` on both paths — a
 * parser anomaly (non-matching or duplicate heading) throws instead of
 * warning, matching `evaluateStage`'s (`lib/status.js`) `check`-mode
 * contract regardless of which storage backend a module is read from.
 */
