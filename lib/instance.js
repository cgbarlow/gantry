...
function listInstanceSlugs(instancesDir) {
  return existsSync(instancesDir)
    ? readdirSync(instancesDir).filter((name) => existsSync(join(instancesDir, name, 'instance.yaml')))
    : []
}
 ...
 * its definition and current stage — a lightweight listing distinct from
 * `readInstance`, which returns one instance's full data.
 */
export function listInstances(options = {}) {
  ...
  return listInstanceSlugs(instancesDir)
    ...
    .map((slug) => {
      ...
      return { slug, definition: instance.definition, stage: instance.stage }
    })
}
