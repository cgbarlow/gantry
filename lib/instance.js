...
export async function recordInstancePullRequest(slug, stageId, pullRequestId, options = {}) {
  if (!options.azureDevOps && !options.github) {
    throw new Error('recordInstancePullRequest is for Workspace-backed instances only — pass options.azureDevOps or options.github')
  }
  const { client, branch } = options.azureDevOps ? azureDevOpsClientFor(options) : githubClientFor(options)
  ...
  const path = options.azureDevOps ? azureDevOpsInstancePath(slug) : githubInstancePath(slug)
...
export async function recordInstancePullRequestStatus(slug, stageId, status, options = {}) {
  if (!options.azureDevOps && !options.github) {
    throw new Error('recordInstancePullRequestStatus is for Workspace-backed instances only — pass options.azureDevOps or options.github')
  }
  if (status === undefined) {
...
export async function recordInstanceReopened(slug, stageId, previousStage, options = {}) {
  if (!options.azureDevOps && !options.github) {
    throw new Error('recordInstanceReopened is for Workspace-backed instances only — pass options.azureDevOps or options.github')
  }
  const { client, branch } = options.azureDevOps ? azureDevOpsClientFor(options) : githubClientFor(options)
  const current = options.azureDevOps
    ? await readInstanceFromAzureDevOps(slug, options)
...
export async function clearInstanceReopened(slug, stageId, options = {}) {
  if (!options.azureDevOps && !options.github) {
    throw new Error('clearInstanceReopened is for Workspace-backed instances only — pass options.azureDevOps or options.github')
  }
  const { client, branch } = options.azureDevOps ? azureDevOpsClientFor(options) : githubClientFor(options)
  const current = options.azureDevOps
    ? await readInstanceFromAzureDevOps(slug, options)
