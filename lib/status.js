...
// #48 — the Atlassian (Bitbucket-backed) twin of evaluateStageFromGitLab above: identical shape, over
// readModule's `{ atlassian }` option instead (`lib/instance.js`'s own atlassian-backed read, landed
// with #43's content-store wiring). Needed for `lib/repoCheck.js`'s `checkAtlassianRepo` to report a
// real `status: 'complete' | 'incomplete'` for an already-discovered instance the same way every other
// provider's own repo check already does — without this, `evaluateStage(definition, stage, slug,
// { atlassian })` would silently fall through to this function's own local-filesystem branch instead
// (no `options.atlassian` case to dispatch to), reading a directory that's never actually there rather
// than throwing — exactly the kind of quietly-wrong-instead-of-loud gap #48 was warned not to repeat.
