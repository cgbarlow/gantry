...
/**
 * For `stage`, reports each of its modules' presence and whether its fields required at that stage's gate are non-empty. Shared by `status` (always the instance's current stage, non-strict parsing) and `check` (any gate's stage, strict parsing — a parser anomaly must fail the gate, not pass silently on data the parser wasn't confident about).
 *
...
/**
 * `gantry status <slug>`: for the instance's current stage (or `options.stageId`, for callers browsing a stage other than the instance's current one — e.g. the web form), reports each module's presence and whether its fields required at that stage's gate are non-empty. Presence/non-emptiness only — no gate-validation logic.
 *
