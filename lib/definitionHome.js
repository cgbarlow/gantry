...
export function libraryRepoRowsWithProblems(existingIds, { instancesDir = 'instances' } = {}) {
  ...
  for (const repo of listLibraryRepos({ instancesDir })) {
    ...
    // #19 (ADR-0037): `repo.location` is always the current, provider-discriminated shape (see
    // lib/librarySettings.js's own doc comment) — used here directly rather than the pre-#19 flat
    // aliases, which only exist for Azure DevOps.
    const repoName =
