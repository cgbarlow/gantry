...
/**
 * Extracts the caller's Azure DevOps Personal Access Token from `req`'s Authorization header. Returns the PAT string, or `null` if the request carries no usable credential (header absent, not the "Basic" scheme, not valid base64, or an empty password half). Never throws — a missing/malformed credential is reported by returning `null`, for callers to turn into a structured "authentication required" response, not an exception.
 */
