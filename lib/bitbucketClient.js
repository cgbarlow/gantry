...
export function createBitbucketClient({ owner, repository, pat, baseUrl = DEFAULT_BASE_URL } = {}) {
   ...
   *
   * `#48`: a repository that doesn't exist *at all* also 404s this same filtered-branches-list route
   * (there's no repo for it to list anything under) — genuinely no different, from this function's own
   * "has no ref at all" contract, than a real repository whose named branch just isn't there (which
   * Bitbucket instead reports as a 200 with an empty `values` array, the case already handled above).
   * Both are "nothing to report a tip for", so a 404 here is folded into the same `null` rather than
   * left to propagate as `BitbucketNotFoundError` — every caller built on this (`getBranchObjectId`,
   * `branchExists`, `listFolder`, every content read, `createBranch`'s own "`from` doesn't exist"
   * check) already only tests for `null`/falsy, so this is what makes each of those already-documented
   * "absence is not exceptional" contracts (this file's own `listFolder` doc comment, mirroring
   * `lib/gitlabClient.js`'s identical one) actually hold for a nonexistent *repository*, not only a
   * nonexistent branch on an otherwise-real one — the distinction `checkAtlassianRepo`
   * (`lib/repoCheck.js`, #48) depends on to tell "empty repo" from "repo doesn't exist" via its own
   * subsequent `repoExists()` call, the same way `checkGitLabRepo`'s identical `listFolder`-then-
   * `repoExists()` pattern already relies on `lib/gitlabClient.js`'s own `listFolder` never throwing
   * for a missing project either.
   */
