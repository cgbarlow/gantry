 ...
 * handling (ADR-0040) — is `completePullRequest` below (#13, `lib/stageStatus.js`'s
 * `checkStageApprovalStatus`); `getPullRequestCommits` is its sibling, feeding the same commit panel
 * Azure DevOps's own `getPullRequestCommits` does.
 *
...
export function createGitHubPullRequestsClient({ owner, repository, pat, baseUrl = DEFAULT_BASE_URL } = {}) {
  ...
  /**
   * Fetches the commits on a pull request (`GET .../pulls/{id}/commits`) — GitHub's own per-PR commit
   * list, used the same way `lib/azureDevOpsPullRequestsClient.js`'s `getPullRequestCommits` is: to
   * populate the Work item details card's commit panel (`lib/stageStatus.js`'s `summarizePullRequest`)
   * so it means the same thing on both providers (CONTEXT.md's own "Work item details card" entry).
   * Returns GitHub's raw shape (`[{ sha, commit: { message, author, committer } }]`) — mapping it onto
   * gantry's `{ commitId, message, timestamp }` summary shape is the caller's job, exactly as the
   * Azure DevOps twin leaves its own raw `commitId`/`comment`/`committer.date` fields for
   * `lib/stageStatus.js`'s `commitSummary` to normalize.
   */
  async function getPullRequestCommits(pullRequestId) {
    const res = await request(`${repoUrl}/pulls/${pullRequestId}/commits`)
    return res.json()
  }

  /**
   * Merges a pull request (`PUT .../pulls/{id}/merge`) — always `merge_method: 'merge'`, never
   * `'squash'` or `'rebase'`: ADR-0040 and the spec's own Out of Scope ("Per-workspace merge method")
   * are explicit that stage branches are stacked on the preceding stage's branch while its Pull
   * Request is open, and a squash or rebase merge would rewrite history a stacked successor branch is
   * already built on. `completionOptions.mergeCommitMessage`, if given, becomes GitHub's own
   * `commit_message` — otherwise GitHub picks its own default message.
   *
   * GitHub refuses a merge blocked by branch protection or a required status check with a non-2xx
   * response (typically 405 or 409) whose `message` names the reason — this function makes no attempt
   * to interpret, retry, or fall back to a different merge method on that refusal (ADR-0040: "never
   * retried, never downgraded to another merge method"); the thrown `RequestError`'s `.body` carries
   * GitHub's own response verbatim for the caller (`lib/stageStatus.js`) to surface as a blocked
   * sign-off.
   */
  async function completePullRequest(pullRequestId, completionOptions = {}) {
    const res = await request(`${repoUrl}/pulls/${pullRequestId}/merge`, {
      method: 'PUT',
      body: {
        merge_method: 'merge',
        ...(completionOptions.mergeCommitMessage !== undefined ? { commit_message: completionOptions.mergeCommitMessage } : {}),
      },
    })
    return res.json()
  }
