...
/**
 * The message every "this GitHub repository doesn't exist" error surfaces (`getRepo` below, plus
 * `lib/repoCheck.js`'s `checkGitHubRepo` and `lib/instance.js`'s own belt-and-braces repo check) — one
 * wording shared across all three call sites rather than each restating it slightly differently.
 *
 * Deliberately does not say only "does not exist": per `docs/adr/0040`'s own "Consequences", GitHub
 * returns this exact 404 both for a repository that is genuinely missing and for a fine-grained PAT
 * that can't see an otherwise-real one because it's missing a required permission — the two are
 * indistinguishable from the HTTP response alone. Naming the scope explanation here, at the one place
 * that constructs the message, is what satisfies #21's acceptance criterion that an insufficient-scope
 * failure is explained rather than surfaced as a bare not-found; a caller that already knows more
 * (`checkGitHubRepo`'s own AuthenticationError branch, for instance) still short-circuits before
 * reaching this message.
 */
function repoNotFoundMessage(owner, repository) {
  return (
    `GitHub repository ${owner}/${repository} was not found, or this Personal Access Token cannot see it. ` +
    'GitHub returns the same "not found" response both when a repository genuinely does not exist and when a ' +
    'fine-grained PAT is missing a required permission (Contents, Issues, Pull requests or Metadata) — check the ' +
    "PAT's permissions before assuming the repository name or owner is wrong. Create the repository on GitHub " +
    'first if it truly does not exist yet.'
  )
}
...
export function createGitHubClient({ owner, repository, pat, baseUrl = DEFAULT_BASE_URL } = {}) {
   ...
   * `lib/azureDevOpsClient.js`'s `branchExists`, needed by the per-stage branch lifecycle (#12,
   * `lib/githubStageBranch.js`) for two reasons — read-only callers ask "has work begun on this
   * stage yet" without creating anything, and `resolveGitHubStageBranch` asks whether an earlier
   * stage's branch is still open (stack on it) or already merged/gone (fork fresh from `main`
   * instead) — and by `lib/definitionPromote.js` (#20) to refuse re-promoting onto a branch a
   * previous promotion already opened.
   */
