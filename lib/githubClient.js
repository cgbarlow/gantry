...
export function createGitHubClient({ owner, repository, pat, baseUrl = DEFAULT_BASE_URL } = {}) {
  ...
  /**
   * Whether `branch` currently has a ref at all — `getBranchTip` reduced to a boolean, mirroring
   * `lib/azureDevOpsClient.js`'s own `branchExists`. Used by `lib/definitionPromote.js` (#20) to
   * refuse re-promoting onto a branch a previous promotion already opened, and by the per-stage
   * branch lifecycle once #12 builds it.
   */
  async function branchExists(branch) {
    return (await getBranchTip(branch)) !== null
  }

  /**
   * Creates a new branch named `name`, pointing at the current tip of `from` (default `"main"`) —
   * mirrors `lib/azureDevOpsClient.js`'s own `createBranch`: no commit of its own, the new ref simply
   * starts out pointing at whatever commit `from` currently points at. Built on the same
   * `POST /git/refs` call `writeFiles` already uses for a brand-new branch's very first push.
   *
   * Throws NotFoundError if `from` itself doesn't exist yet (no commit to branch from). GitHub
   * itself rejects creating a ref that already exists (422), which `request()`'s shared error
   * handling surfaces as a RequestError — the same "already exists" failure Azure DevOps's own
   * `createBranch` reports for the same case.
   */
  async function createBranch(name, { from = 'main' } = {}) {
    const tip = await getBranchTip(from)
    if (!tip) {
    ...
    await request(`${repoUrl}/git/refs`, {
      method: 'POST',
      body: { ref: `refs/heads/${name}`, sha: tip.commitSha },
    })
    return { name, from, objectId: tip.commitSha }
  }
  ...
  async function writeFiles(files, { branch = 'main', message, deletePaths = [] } = {}) {
    ...
    return { changes: [...changes, ...deleteChanges], push: { commits: [{ commitId: newCommit.sha }] } }
  }
  ...
  return { owner, repository, baseUrl, getRepo, repoExists, getFileContent, fileExists, listFolder, writeFile, writeFiles, branchExists, createBranch }
}
