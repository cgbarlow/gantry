 ...
 * Covers exactly what #30 needs: creating an instance's per-stage child issues under an existing
 * parent issue, and pushing a gate-passed "state" (closed) to a stage's own issue.
 *
...
export function createGitLabWorkItemsClient({ namespace, repository, pat, baseUrl = DEFAULT_BASE_URL } = {}) {
   ...
   * `web_url`, ...), the raw shape GitLab itself returns — `iid` (the project-scoped "internal id") is
   * what every other method here, and `lib/workItemLink.js`'s own recorded `stages` map, addresses an
   * issue by; GitLab's separate, globally-unique `id` is never used.
   */
  ...
  return { namespace, repository, baseUrl, createIssue, getIssue, updateIssue, createChildIssue }
}
