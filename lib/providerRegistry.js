...
const providers = {
  ...
  // Atlassian joins the registry the same incremental, one-capability-per-ticket way GitHub and
  // GitLab did — #42 registers only `workItems` (Jira Cloud), the first Atlassian capability to
  // land (#40 registered the provider identifier and its two-token credential schema only, per its
  // own scope, shipping no capability at all). `contentStore`/`pullRequests`/`identity` — all three
  // backed by Bitbucket Cloud, not Jira, per ADR-0042's split-suite model — join in later tickets the
  // same way GitHub's and GitLab's own remaining three capabilities each did.
  atlassian: {
