...
const providers = {
  ...
  // content store; #28 — identity; work items/pull requests land in later tickets per ADR-0041's own
  // scope list). `getProviderCapabilities('gitlab')` only exposes what's registered so far — a caller
  // reaching for `pullRequests`/`workItems` today gets `undefined`, same as GitHub did before its own
  // remaining tickets landed.
  gitlab: {
