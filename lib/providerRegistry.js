...
const providers = {
  ...
  // Cloud; #42 landed its workItems, `lib/jiraWorkItemsClient.js`, backed by Jira Cloud. Unlike the
  // other three providers (each one product bundling all four capabilities), Atlassian is
  // split-suite: `pullRequests`/`identity` (later tickets, #44 for Bitbucket's half, #45 for Jira's)
  // stay to be registered — one entry here per capability regardless of which of the suite's two
  // products actually implements it, the same as every other provider's own entry.
  atlassian: {
