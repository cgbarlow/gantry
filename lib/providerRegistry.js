...
const providers = {
  ...
  // #11: content store only so far — pullRequests/workItems/identity join once #12-#15/#10 build
  // them, the same incremental registration this file's own doc comment describes.
  github: {
    contentStore: createGitHubClient,
  },
}

/** Provider identifiers with a registered implementation — currently just Azure DevOps; GitHub joins once its four capabilities are built. Distinct from the full `provider` enum (docs/adr/0037), which also lists `'atlassian'` as known-but-unavailable. */
