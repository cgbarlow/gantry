...
const providers = {
  ...
  // content store; #28 — identity; #30 — work items; pull requests lands in a later ticket per
  // ADR-0041's own scope list). `getProviderCapabilities('gitlab')` only exposes what's registered so
  // far — a caller reaching for `pullRequests` today gets `undefined`, same as GitHub did before its
  // own remaining tickets landed.
  gitlab: {
