...
// Atlassian's `identity` capability is split across two tickets (#44 Bitbucket, #45 Jira; see the
// `atlassian` entry's own comment below) — this dispatches a single `(config) => capability` call to
// whichever of the two the caller's config shape names, mirroring `web/lib/identityPicker.js`'s own
// discriminated-by-which-fields-are-present convention for telling providers' location shapes apart.
function createAtlassianIdentityClient(config = {}) {
  const { jiraSite, jiraProjectKey, owner, repository } = config
  if (jiraSite || jiraProjectKey) return createJiraIdentityClient(config)
  if (owner || repository) return createBitbucketIdentityClient(config)
  throw new Error('createAtlassianIdentityClient: config must be Jira-shaped ("jiraSite"/"jiraProjectKey") or Bitbucket-shaped ("owner"/"repository")')
}
...
const providers = {
  ...
  // itself split in two, unlike every other capability here: #44 lands the Bitbucket-backed half —
  // PR reviewers, resolved against Bitbucket's own repo permissions, `lib/bitbucketIdentityClient.js`
  // — and #45 lands the Jira-backed half — work-item assignees, resolved against the Jira project's
  // own "Assignable User" permission, `lib/jiraIdentityClient.js`. Per ADR-0042's "identity stays
  // per-product" decision the two are never unified into one call; since both halves land under this
  // single `identity` key, this file (not either ticket alone) owns dispatching between them —
  // Jira-shaped config (`jiraSite`/`jiraProjectKey`) routes to the Jira client, Bitbucket-shaped config
  // (`owner`/`repository`) to the Bitbucket client, the same discriminated-by-which-fields-are-present
  // convention `web/lib/identityPicker.js` already uses to tell providers' own location shapes apart.
  // `pullRequests` (#49) is Bitbucket-backed, `lib/bitbucketPullRequestsClient.js`, registered with
  // Promote's own open/read/request-reviewer scope (mirroring GitHub's own #20-then-#13 staging) —
  // sign-off's fuller surface (merge, wired into `lib/stageApproval.js`) is #46's job, extending the
  // same file and needing no change here.
  atlassian: {
    contentStore: createBitbucketClient,
    workItems: createJiraWorkItemsClient,
    identity: createAtlassianIdentityClient,
    pullRequests: createBitbucketPullRequestsClient,
  },
}

/** Provider identifiers with at least one registered capability — Azure DevOps, GitHub and GitLab (full four each), and Atlassian (content store, work items, identity and pull requests — full four too, as of #49). Distinct from the full `provider` enum (docs/adr/0037), which only lists identifiers `lib/provider.js` accepts as a workspace's own provider — a provider can appear there before every one of its capabilities is registered here. */
...
/**
 * Resolves `providerId`'s content-store capability and instantiates it with `config`. Shorthand for
 * `getProviderCapabilities(providerId).contentStore(config)` — except for one opt-in extra: a
 * caller-supplied `config.sharedCache` (`{ cache, storeKey, cacheReads }` — `lib/sharedWorkspaceCache.js`'s
 * own shape, threaded here by `lib/instance.js`'s `azureDevOpsClientFor`/`githubClientFor`/
 * `gitlabClientFor` — Azure DevOps, GitHub and GitLab are docs/adr/0047's own shared-read surface;
 * `atlassianClientFor` doesn't thread it, so an Atlassian-backed workspace's reads are unaffected by
 * this ticket — in turn set by `lib/server.js`'s credential-gating helpers) wraps the freshly built
 * client through `cache.wrapContentStore` before returning it (#127, parent
 * #109, docs/adr/0047 "Cache design") instead of returning it bare. `sharedCache` is stripped out of
 * what actually reaches the provider client's own factory — none of `createGitHubClient` and its
 * siblings know or need to know this key exists. Every existing caller that never sets it (every call
 * site before #127, and any call against an unshared workspace after it) gets back exactly the bare
 * client this function always returned.
 */
export function resolveContentStore(providerId, config = {}) {
  const { sharedCache, ...clientConfig } = config
  const client = getProviderCapabilities(providerId).contentStore(clientConfig)
  if (!sharedCache?.cache) return client
  return sharedCache.cache.wrapContentStore(client, { storeKey: sharedCache.storeKey, cacheReads: Boolean(sharedCache.cacheReads) })
}
