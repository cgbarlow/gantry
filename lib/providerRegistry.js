...
const providers = {
  ...
  // other provider did — starting with #41's content store, `lib/bitbucketClient.js`, backed by
  // Bitbucket Cloud. Unlike the other three (each one product bundling both halves), Atlassian is
  // split-suite: its `workItems` capability (a later ticket, #42) is backed by Jira Cloud instead, and
  // its `pullRequests`/`identity` (later tickets, #44/#46) stay Bitbucket-backed — one entry here per
  // capability regardless of which of the suite's two products actually implements it, the same as
  // every other provider's own entry.
  atlassian: {
    contentStore: createBitbucketClient,
  },
}

/** Provider identifiers with at least one registered capability — Azure DevOps (full four), GitHub and GitLab (full four), and Atlassian (content store so far, more capabilities landing per-ticket). Distinct from the full `provider` enum (docs/adr/0037), which only lists identifiers `lib/provider.js` accepts as a workspace's own provider — a provider can appear there before every one of its capabilities is registered here. */
