...
const providers = {
  ...
  // content store; identity/work items/pull requests land in later tickets per ADR-0041's own scope
  // list). `getProviderCapabilities('gitlab')` only exposes what's registered so far — a caller
  // reaching for `pullRequests`/`workItems`/`identity` today gets `undefined`, same as GitHub did
  // before its own remaining three tickets landed.
  gitlab: {
