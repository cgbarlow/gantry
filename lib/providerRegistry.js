...
const providers = {
  ...
  // content store; #30 — work items; identity/pull requests land in later tickets per ADR-0041's own
  // scope list). `getProviderCapabilities('gitlab')` only exposes what's registered so far — a caller
  // reaching for `pullRequests`/`identity` today gets `undefined`, same as GitHub did before its own
