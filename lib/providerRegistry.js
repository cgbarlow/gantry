...
const providers = {
  ...
  // Cloud; #42 landed its workItems, `lib/jiraWorkItemsClient.js`, backed by Jira Cloud; #44 lands
  // `identity`'s Bitbucket-backed half (pull-request reviewers), `lib/bitbucketIdentityClient.js` —
  // per ADR-0042's "identity stays per-product" decision, this is only ever consulted for a
  // pull-request reviewer, never a work-item assignee. #45 (Jira's own `lib/jiraIdentityClient.js`,
  // work-item assignees) still needs to land alongside this entry — how the two combine under this
  // single `identity` key is that ticket's own concern, same as every capability here being scoped to
  // exactly what's landed "so far" until a provider reaches full parity. `pullRequests` (#46) is still
  // to be registered.
  atlassian: {
    contentStore: createBitbucketClient,
    workItems: createJiraWorkItemsClient,
    identity: createBitbucketIdentityClient,
  },
}

/** Provider identifiers with at least one registered capability — Azure DevOps (full four), GitHub and GitLab (full four), and Atlassian (content store + work items so far, pull requests/identity landing per-ticket). Distinct from the full `provider` enum (docs/adr/0037), which only lists identifiers `lib/provider.js` accepts as a workspace's own provider — a provider can appear there before every one of its capabilities is registered here. */
