...
export function createAzureDevOpsWorkItemsClient({
   ...
   * rev, fields, relations, url }` shape `createWorkItem`/`updateWorkItem`
   * already return, so callers don't need a second response shape to
   * handle.
   *
   ...
   * shape.
   */
  async function getWorkItem(id, { fields } = {}) {
    const url = new URL(`${projectWitUrl}/workitems/${id}`)
    url.searchParams.set('api-version', apiVersion)
