 ...
 * definition/stage/status/owner" listing). This module answers a narrower,
 ...
 *
 * Since #96: an Azure-DevOps-backed entry no longer duplicates its own
 * organization/project/repository — it references a workspace id
 * (`lib/workspaceRegistry.js`) instead. That's purely a *storage-shape*
 * change, not a contract change for this module's own callers:
 * `resolveInstanceLocation`/`listRegisteredInstances` still hand back the
 * familiar `{ kind: 'azureDevOps', organization, project, repository,
 * baseUrl? }` shape (denormalized from the referenced workspace on read),
 * and `registerInstance` still accepts that same shape (looking up — or
 * auto-creating — the matching workspace under the hood) alongside the new,
 * more direct `{ kind: 'azureDevOps', workspaceId }` shape. A registry file
 * written before #96 (raw entries still carrying organization/project/
 * repository directly, with no `workspaceId`) is migrated in place the
 * first time it's read — see `migrateLegacyAzureDevOpsEntries` — so an
 * existing deployment's already-registered instances keep resolving
 * correctly with no manual step, backed by one auto-created workspace per
 * distinct organization/project/repository(/baseUrl) tuple found.
 */
...
function assertValidLocation(location) {
  ...
  if (location.kind === 'azureDevOps') {
    // Either shape is accepted here — see normalizeAzureDevOpsLocation,
    // which does the actual field-by-field validation appropriate to
    // whichever one was given (a `workspaceId` that must resolve, or an
    // organization/project/repository trio that doesn't yet).
    return
  }
  throw new Error(`Unknown registry location kind "${location?.kind}" — expected "local" or "azureDevOps"`)
}

// Normalizes any caller-supplied Azure DevOps location into the one shape
// this registry actually persists: `{ kind: 'azureDevOps', workspaceId }`.
// Accepts either an already-resolved `{ workspaceId }` (validated to
// reference a real, registered workspace — never persisted unchecked) or
// the legacy/convenience `{ organization, project, repository, baseUrl? }`
// shape (looked up — or, if this is the first instance ever registered
// against that repo, created — via `getOrCreateWorkspace`, so registering
// several instances against the same repo shares one workspace rather than
// fragmenting it across several).
...
// The read-side inverse of normalizeAzureDevOpsLocation: expands a
// persisted `{ kind: 'azureDevOps', workspaceId }` entry back out to the
// `{ kind: 'azureDevOps', organization, project, repository, baseUrl? }`
// shape every existing caller (server.js's resolveAzureDevOpsLocation,
// lib/registry.js's buildAzureDevOpsRow, every pre-#96 test) already
// expects — the workspace reference is this module's own storage detail,
// not something every consumer needs to learn to dereference itself.
...
// Migrates any raw entry still in the pre-#96 shape (an `azureDevOps` kind
// carrying `organization`/`project`/`repository` directly, no
// `workspaceId` yet) to the current `{ kind: 'azureDevOps', workspaceId }`
// shape — auto-creating (or reusing, for a second such entry pointing at
// the same repo) the workspace that backs it via `getOrCreateWorkspace`.
// Returns whether anything changed, so `loadWithBackfill` only re-persists
// the file when a migration actually happened.
...
//
// Also runs the pre-#96-shape migration above, for the same "every read,
// not just once" reason — a registry file from before workspaces existed
// must keep working with zero manual step, exactly as the local-instance
 ...
 * 'local' }`, `{ kind: 'azureDevOps', organization, project, repository,
 * baseUrl? }`, or `{ kind: 'azureDevOps', workspaceId }` — anything else
 * throws rather than silently persisting a location no other part of the
 * registry knows how to interpret. An `organization`/`project`/`repository`
 * location is resolved to (or, the first time this exact repo is
 * registered, creates) a workspace via `lib/workspaceRegistry.js` before
 * being persisted — this registry only ever stores a workspace reference,
 * never a duplicated organization/project/repository.
 */
...
/**
 * The raw workspace id a slug's Azure-DevOps-backed entry references, or
 * `null` for a local slug (or one the registry has never seen at all).
 * Deliberately a separate, additive function rather than a new field on
 * `resolveInstanceLocation`'s own return value (#104) — that function's
 * `{ kind, organization, project, repository, baseUrl? }` shape is already
 * relied on verbatim by existing callers/tests (e.g. `lib/registry.js`'s
 * `buildAzureDevOpsRow`, `GET /api/instances`'s response body), so adding a
 * key there would be a breaking shape change for no benefit to them. This
 * is the one place a caller that specifically needs "which workspace does
 * this slug belong to" (the client-side PAT/ticketing-system override
 * resolution `GET /api/instance/workspace` route builds on) asks that
 * question directly.
 */
export function resolveInstanceWorkspaceId(slug, options = {}) {
  ...
  // `loadWithBackfill` has already run `migrateLegacyAzureDevOpsEntries` by
  // this point, so every azureDevOps entry is guaranteed to carry a
  // `workspaceId` — no legacy organization/project/repository shape can
