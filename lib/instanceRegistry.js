...
function denormalizeEntry(entry, options) {
  // A local entry may now also carry `archived` (#223) — an internal storage detail, exactly like
  // an azureDevOps entry's `workspaceId`, so it's stripped here rather than leaked into the
  // `location` shape every caller relies on. `archived` is surfaced separately, as its own field.
  if (entry?.kind === 'local') return { kind: 'local' }
 ...
 *
 * Archived instances (#223 — an entry carrying `archived: true`) are left out by default, so the
 * dashboard listing (`lib/registry.js`'s `listRegistry`, this function's main consumer) drops an
 * archived instance with no code change of its own. Pass `options.includeArchived` for the
 * "show archived / restore" view: it then returns every entry, each additionally carrying
 * `archived` (a plain boolean) so that view can tell the two apart. The `{ slug, location }` shape
 * every existing caller relies on is unchanged when the option is absent.
 */
export function listRegisteredInstances(options = {}) {
  const { entries } = loadWithBackfill(options)
  return Object.keys(entries)
    .sort()
    .filter((slug) => options.includeArchived || !entries[slug].archived)
    .map((slug) => {
      const row = { slug, location: denormalizeEntry(entries[slug], options) }
      if (options.includeArchived) row.archived = Boolean(entries[slug].archived)
      return row
    })
}

/**
 * Whether a slug's instance is currently archived (#223). `false` for a slug the registry has
 * never seen — an instance that doesn't exist isn't "archived".
 */
export function isInstanceArchived(slug, options = {}) {
  const { entries } = loadWithBackfill(options)
  return Boolean(entries[slug]?.archived)
}

/**
 * Archives (`archived: true`) or restores (the key removed) a slug's instance — the write half of
 * #223's archive/restore for an instance. Nothing is deleted: the entry's `kind`/`workspaceId`,
 * the instance's own data on disk (or in its Azure DevOps repo), and its assigned instance number
 * (lib/numberRegistry.js) are all untouched, so `restoreInstance` brings it back to exactly the
 * state it was archived from. The entry is rebuilt in the same canonical shape `registerInstance`
 * persists (`{ kind: 'local' }` or `{ kind: 'azureDevOps', workspaceId }`), with `archived` — when
 * set — always last.
 *
 * Both are idempotent: archiving an already-archived instance, or restoring one that isn't
 * archived, is a no-op success. Throws if `slug` is unknown (neither registered nor on disk).
 *
 * Archiving an instance is independent of its workspace — it never touches the workspace registry,
 * and archiving every instance in a workspace does not archive the workspace itself (#223).
 */
...
function setInstanceArchived(slug, archived, options) {
  ...
  const entry = entries[slug]
  const rebuilt =
    entry.kind === 'azureDevOps' ? { kind: 'azureDevOps', workspaceId: entry.workspaceId } : { kind: 'local' }
  if (archived) rebuilt.archived = true
  entries[slug] = rebuilt
  writeRegistryFile(registryPath, entries)
  return { slug, location: denormalizeEntry(entries[slug], options), archived: Boolean(archived) }
}
