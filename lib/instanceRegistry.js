  ...
  for (const slug of legacySlugs) {
    ...
    if (legacy.kind === 'local') {
      ...
      if (legacy.workspaceId !== undefined) {
        ...
        const workspace = getOrCreateWorkspace(
          { organization: legacy.organization, project: legacy.project, repository: legacy.repository, baseUrl: legacy.baseUrl },
          options
        )
...
function candidateLabel(scopeId, slug, entry, options) {
  ...
  const workspace = resolveWorkspace(scopeId, options)
  return workspace ? `${workspace.organization}/${workspace.project}/${workspace.repository}:${slug}` : `${scopeId}:${slug}`
}
...
/**
 * Resolves a slug's location: `{ kind: 'directory', workspace }` or `{ kind: 'azureDevOps', organization, project, repository, baseUrl? }`. `undefined` if unknown. Pass `options.workspace` (a directory workspace's folder name) to resolve within that one scope directly; omit it to search every scope, per this module's bare-slug backward-compatibility rule (throws if ambiguous).
 */
...
export function listRegisteredInstances(options = {}) {
  ...
  for (const scopeId of Object.keys(entries)) {
    for (const slug of Object.keys(entries[scopeId])) {
      ...
      const row = { slug, workspace: location.kind === 'directory' ? location.workspace : undefined, location }
