...
import { getOrCreateWorkspace, resolveWorkspace, listWorkspaces } from './workspaceRegistry.js'
...
import { discoverProviderInstances } from './repoCheck.js'
...
// #133 (parent #109): a workspace id -> `true` map, colocated next to instance-registry.json the same
// way workspace-registry.json/number-registry.json are — its own small file rather than a slug entry
// inside `entries[scopeId]`, since every reader of that map (`listRegisteredInstances`,
// `listRegisteredScopedSlugsInStorageOrder`, `denormalizeEntry`, …) already treats every key under a
// scope as a real, addressable instance slug; a sentinel slug there would have to be filtered out of
// every one of those, and would still show up in a directly-read registry file. This file instead
// records a fact about the *workspace*, not an instance, so it lives beside the instance registry
// without reshaping it.
//
// A workspace's discovery can end one of three ways: never attempted (no entry here, no registered
// instance either), attempted and found something (recorded as ordinary `entries[scopeId]` rows
// instead — no entry here), or attempted and found nothing (recorded here). Only the third case needs
// this file at all — see `hasUndiscoveredProviderWorkspaces`/`backfillProviderInstances` below, whose
// whole point is telling that third case apart from the first without re-listing the repo to check.
const DISCOVERED_EMPTY_FILENAME = 'discovered-empty-workspaces.json'

function discoveredEmptyPathFor(options) {
  const workspacesDir = options.instancesDir ?? 'instances'
  return join(workspacesDir, DISCOVERED_EMPTY_FILENAME)
}

function readDiscoveredEmptyFile(path) {
...
function isDiscoveredEmpty(workspaceId, options) {
  return Boolean(readDiscoveredEmptyFile(discoveredEmptyPathFor(options))[workspaceId])
}

// Recorded once a discovery attempt for `workspaceId` finds zero slugs — see the module-level comment
// on `DISCOVERED_EMPTY_FILENAME` above. Idempotent (re-marking an already-marked workspace is a no-op
// write of the same content).
function markDiscoveredEmpty(workspaceId, options) {
  const path = discoveredEmptyPathFor(options)
  const marks = readDiscoveredEmptyFile(path)
  marks[workspaceId] = true
  storage.writeText(path, JSON.stringify(marks, null, 2) + '\n')
}

// #133: the credential a discovery attempt against `workspace` should use — the request's own
// (`options.pat`) when it carries one, else that workspace's own `GANTRY_SHARED_WORKSPACE_PATS` entry
// (`options.sharedPats`, `{ workspaceId: pat }`, `{}` by default) — mirrors `lib/registry.js`'s
// `resolveRowCredential` exactly, so a shared credential is used only against the workspace it was
// declared for (never tried against a different undiscovered workspace) and a request's own credential
// always wins when both are present. `undefined` when neither exists, the same "nothing to try" signal
// `options.pat` alone used to be before this ticket.
function credentialFor(workspace, options) {
  return options.pat || options.sharedPats?.[workspace.id] || undefined
}

// True when this request could possibly have a credential to discover with — `options.pat`, or at
// least one entry in `options.sharedPats`. `lib/registry.js`'s `listRegistry` always passes a
// `sharedPats` object (`{}` by default, per its own comment on that default), so checking merely
// `!options.sharedPats` here would never be true for that caller and would defeat the early-exit guard
// below on every request. Checking for at least one key is what actually distinguishes "nothing set
// GANTRY_SHARED_WORKSPACE_PATS" (the common case) from "a shared credential exists, but maybe not for
// *this* workspace" (still worth the full `loadWithBackfill` + per-workspace `credentialFor` check).
function hasAnyCredential(options) {
  return Boolean(options.pat) || Boolean(options.sharedPats && Object.keys(options.sharedPats).length > 0)
}
...
function assertValidEntryKind(location) {
  ...
  if (location.kind === 'azureDevOps' || location.kind === 'github' || location.kind === 'gitlab') {
    return
  }
  throw new Error(`Unknown registry location kind "${location?.kind}" — expected "directory", "azureDevOps", "github" or "gitlab"`)
}
...
// The GitHub twin of normalizeAzureDevOpsLocation above (#11) — same "an already-known workspaceId
// resolves directly, otherwise a location gets-or-creates its workspace" shape, over GitHub's own
// `{owner, repository, baseUrl?}` fields instead of Azure DevOps's three-field location.
function normalizeGitHubLocation(location, options) {
  if (location.workspaceId !== undefined) {
    if (!resolveWorkspace(location.workspaceId, options)) {
      throw new Error(`Unknown workspace "${location.workspaceId}"`)
    }
    return { scopeId: location.workspaceId, entry: { kind: 'github' } }
  }

  const missing = ['owner', 'repository'].filter((key) => !location[key])
  if (missing.length) {
    throw new Error(`A "github" registry location is missing: ${missing.join(', ')}`)
  }
  ...
  return { scopeId: workspace.id, entry: { kind: 'github' } }
}

// #31 — the GitLab twin of normalizeGitHubLocation above (#26): same "an already-known workspaceId
// resolves directly, otherwise a location gets-or-creates its workspace" shape, over GitLab's own
// `{namespace, repository, baseUrl?}` fields (ADR-0041).
function normalizeGitLabLocation(location, options) {
  if (location.workspaceId !== undefined) {
    if (!resolveWorkspace(location.workspaceId, options)) {
      throw new Error(`Unknown workspace "${location.workspaceId}"`)
    }
    return { scopeId: location.workspaceId, entry: { kind: 'gitlab' } }
  }

  const missing = ['namespace', 'repository'].filter((key) => !location[key])
  if (missing.length) {
    throw new Error(`A "gitlab" registry location is missing: ${missing.join(', ')}`)
  }
  ...
  return { scopeId: workspace.id, entry: { kind: 'gitlab' } }
}
...
// #112 (parent #109): a Provider-backed workspace's registry entry `kind` — keyed by
// `lib/provider.js`'s own `provider` id, which is why this map exists at all: every entry kind here
// happens to equal its provider id verbatim *except* `azure-devops`, whose entry kind is the
// pre-existing camelCase `azureDevOps` (predating the Provider model itself — see
// `assertValidEntryKind` above). Deliberately has no `atlassian` entry: the instance registry has no
// entry kind for it yet (`assertValidEntryKind` only knows `directory`/`azureDevOps`/`github`/
// `gitlab`), so an Atlassian workspace is skipped by `backfillProviderInstances` below rather than
// attempted with a kind that would fail to register.
const PROVIDER_ENTRY_KIND = {
  'azure-devops': 'azureDevOps',
  github: 'github',
  gitlab: 'gitlab',
}

// Cheap, synchronous pre-check for whether `backfillProviderInstances` below would actually have
// anything to do — no network calls, just the already-loaded registry + workspace registry files.
// `lib/registry.js`'s `listRegistry` uses this to decide whether it needs to take the async
// discovery path at all, so a request that names no Provider-backed workspace with zero instances
// (the common case once every workspace has been discovered once) stays exactly as synchronous as it
// was before #112 — this is what satisfies "the common path stays cheap".
//
// `options.workspaceId` (#131), when supplied, narrows the question to that one workspace — the exact
// counterpart of `backfillProviderInstances`'s own scoping option below, so a caller that already
// knows which single workspace it is discovering for (the workspace-scoped listing route) pays
// nothing for, and asks nothing about, every other workspace in the registry.
//
// #133 (parent #109): a workspace with zero registered instances is only "undiscovered" when there's
// a credential to try it with (`options.pat` or that workspace's own `options.sharedPats` entry — see
// `credentialFor`) and it hasn't already been discovered-and-found-empty (`isDiscoveredEmpty` — see
// `DISCOVERED_EMPTY_FILENAME`'s own comment above). Without either check, a shared-credential
// workspace with a genuinely empty repo would report "undiscovered" forever, sending every request
// down `listRegistry`'s async backfill branch for no gain.
export function hasUndiscoveredProviderWorkspaces(options = {}) {
  if (!hasAnyCredential(options)) return false
  const { entries } = loadWithBackfill(options)
  return listWorkspaces(options).some((workspace) => {
    if (options.workspaceId && workspace.id !== options.workspaceId) return false
    if (!PROVIDER_ENTRY_KIND[workspace.provider]) return false
    if (Object.keys(entries[workspace.id] ?? {}).length > 0) return false
    if (!credentialFor(workspace, options)) return false
    return !isDiscoveredEmpty(workspace.id, options)
  })
}

// #122 (parent #109, docs/adr/0047): the smallest server-side signal that lets the dashboard tell
// "this workspace definitely has instances, none of which the current request can read" apart from
// "nothing has ever been registered here" — the two causes behind a workspace contributing zero rows
// to `GET /api/instances` that #122's own acceptance criteria require to read differently ("can't see
// in here" vs "nothing in here yet"). Read-only, synchronous, no PAT — exactly `hasUndiscoveredProviderWorkspaces`'s
// own "no network calls, just the already-loaded registry file" shape, but answering a different
// question: not "would discovery have anything to do" (which needs a credential to attempt), but "does
// this workspace's own registry entry already carry at least one slug" — a fact this server already
// knows for certain without ever touching the Provider, since every entry under `entries[workspaceId]`
// was put there either by an explicit create/adopt (itself credential-proven at the time, #93/#94) or by
// a past successful discovery run (`backfillProviderInstances` above) — never by guessing.
//
// This can't distinguish "confirmed empty" from "never yet discovered" when it returns `false` — a
// successful discovery that finds zero slugs leaves the registry untouched (`backfillProviderInstances`'s
// own `if (slugs.length === 0) continue`, above), so an actually-empty repo and a workspace nobody has
// ever had a working credential for look identical here. `false` is reported honestly as "nothing
// registered yet", not "confirmed empty", by whichever caller renders it (web/lib/dashboardWorkspaces.js).
//
// #187: archived instances don't count. A workspace whose instances are all archived was emptied on
// purpose, and must read as empty rather than as content this request can't see — the listing leaves
// archived instances out, so counting them here made the dashboard call such a workspace unreadable.
// `workspaceHasArchivedInstances` below reports them separately.
export function workspaceHasRegisteredInstances(workspaceId, options = {}) {
  const { entries } = loadWithBackfill(options)
  return Object.values(entries[workspaceId] ?? {}).some((entry) => !entry?.archived)
}

// #187: whether any of this workspace's registered instances is archived — what tells an emptied
// workspace (every instance archived) apart from one where nothing has been registered yet.
export function workspaceHasArchivedInstances(workspaceId, options = {}) {
  const { entries } = loadWithBackfill(options)
  return Object.values(entries[workspaceId] ?? {}).some((entry) => Boolean(entry?.archived))
}

/**
 * Discovers and registers a Provider-backed workspace's own instances directly from its repo (#112,
 * parent #109) — the Provider-backed twin of `scanDirectoryWorkspacesForInstances` above, run for
 * every workspace `lib/workspaceRegistry.js` knows about rather than a directory scan.
 *
 * Runs only for a workspace that currently has ZERO registered instances (of either archived state —
 * an archived instance still counts as "already discovered", so its workspace is never rescanned),
 * has not already been discovered-and-found-empty (`isDiscoveredEmpty`, #133 — see
 * `DISCOVERED_EMPTY_FILENAME`'s own comment), and only when `credentialFor` finds a usable credential
 * for it — the request's own `options.pat`, or (#133) that workspace's own `options.sharedPats` entry
 * when the request carries none — no server-side credential is introduced (ADR-0038 stays intact), and
 * a workspace with neither simply isn't attempted. Once at least one instance is discovered and
 * registered for a workspace, every later call sees a non-empty scope for it and skips it — this is
 * what keeps the common path cheap: an already-populated workspace never re-lists its repo again. A
 * workspace whose repo is discovered and genuinely empty gets the same treatment via the
 * discovered-empty marker instead of a registered instance (#133) — otherwise "zero registered
 * instances" would stay true forever and this function would re-list that repo on every single call.
 *
 * A workspace whose repo can't be listed at all — a rejected/expired PAT, a network failure, an
 * unreachable host, a rate limit — is left alone: caught and skipped here, exactly like every
 * provider row builder in `lib/registry.js` already does for a read failure, so one bad workspace
 * never fails the whole request. Only `azure-devops`/`github`/`gitlab` workspaces are attempted — see
 * `PROVIDER_ENTRY_KIND`'s own comment on why `atlassian` is skipped outright.
 *
 * `options.workspaceId`, when supplied, scopes the loop below to that one workspace only — every
 * other undiscovered workspace is left untouched by this call. The request-credential path (#112,
 * `lib/registry.js`'s `listRegistry`) never passes it: `options.pat` there is already the one
 * credential the whole request carries, so trying it against every undiscovered workspace is exactly
 * the intended breadth. `lib/workspaceBootstrap.js`'s `discoverBootstrapPatInstances` (#113) always
 * passes it: a boot-time `GANTRY_SHARED_WORKSPACE_PATS` entry is one PAT for one declared workspace, and
 * without this scope it would otherwise also be tried against every *other* still-undiscovered
 * workspace in the same registry — exactly the cross-workspace credential use docs/adr/0038 exists to
 * prevent. Scoping here, rather than filtering the workspace list before calling in, keeps that
 * boundary enforced in one place both callers share.
 *
 * `options.surfaceAuthenticationErrors` (#131) opts a caller out of the "one bad workspace never fails
 * the whole request" swallow above — for an `AuthenticationError` only, and only for that one error
 * class. A *listing* caller (`lib/registry.js`'s `listRegistry`, and the boot-time path) wants the
 * swallow: a rejected credential there must degrade to a shorter listing, never a 500. The
 * workspace-scoped route added by #131 wants the opposite: it exists to answer "can this credential
 * see this one workspace", so a credential the Provider rejects has to reach the caller in order to
 * come back as the same structured `authentication_required` response every other provider-backed
 * route gives it — swallowed, it would be indistinguishable from "this workspace is genuinely empty",
 * which is precisely the silent failure that ticket exists to remove. Every other failure (network,
 * rate limit, unreachable host, missing repo) is still caught and skipped exactly as before, for both
 * kinds of caller.
 *
 * Returns whether the registry file actually changed (any workspace's instances were discovered),
 * purely for callers that want to know whether it's worth re-reading the registry afterward.
 */
export async function backfillProviderInstances(options = {}) {
  if (!hasAnyCredential(options)) return false
  ...
  for (const workspace of listWorkspaces(options)) {
    ...
    const entryKind = PROVIDER_ENTRY_KIND[workspace.provider]
    if (!entryKind) continue
    if (Object.keys(entries[workspace.id] ?? {}).length > 0) continue
    if (isDiscoveredEmpty(workspace.id, options)) continue

    // #133: this workspace's own credential — the request's own `options.pat` when it has one, else
    // its `options.sharedPats` entry (never any other workspace's — see `credentialFor`). A workspace
    // with neither is left alone exactly as a missing `options.pat` always left every workspace alone
    // before this ticket.
    const pat = credentialFor(workspace, options)
    if (!pat) continue

    let slugs
    try {
      slugs = await discoverProviderInstances(workspace.provider, workspace.location, { pat })
    } catch (err) {
      // #131: see this function's own doc comment on `options.surfaceAuthenticationErrors` — a
      // rejected credential is the one failure a workspace-scoped caller needs back rather than
      // swallowed. The credential itself is never part of this (or any other) message here; only the
      // workspace id and its provider location are.
      if (options.surfaceAuthenticationErrors && err instanceof AuthenticationError) throw err
      console.error(
        `lib/instanceRegistry.js: skipping Provider-backed instance discovery for workspace "${workspace.id}" ` +
          `(${describeProviderLocation(workspace.provider, workspace.location)}) — ${err.message}`
      )
      continue
    }
    if (slugs.length === 0) {
      // #133: a genuinely empty repo — record it so this workspace's own request-time credential path
      // isn't re-listed on every future request (the trap #133 exists to close). A workspace already
      // registered another way, or one still lacking any usable credential, never reaches this line.
      markDiscoveredEmpty(workspace.id, options)
      continue
    }

    entries[workspace.id] = entries[workspace.id] ?? {}
    for (const slug of slugs) {
      entries[workspace.id][slug] = { kind: entryKind }
    }
    changed = true
  }
...
export function registerInstance(slug, location, options = {}) {
  ...
  if (location.kind === 'directory') {
    ...
    entry = { kind: 'directory' }
  } else if (location.kind === 'github') {
    ;({ scopeId, entry } = normalizeGitHubLocation(location, options))
  } else if (location.kind === 'gitlab') {
    ;({ scopeId, entry } = normalizeGitLabLocation(location, options))
  } else {
