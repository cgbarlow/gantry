...
export function hasUndiscoveredProviderWorkspaces(options = {}) {
  ...
  return listWorkspaces(options).some((workspace) => {
    ...
    return Object.keys(entries[workspace.id] ?? {}).length === 0
  })
}
 ...
 * an archived instance still counts as "already discovered", so its workspace is never rescanned) and
 * only when `options.pat` is a usable credential this request actually carries — no server-side
 * credential is introduced (ADR-0038 stays intact), and a workspace whose credential is missing
 * simply isn't attempted. Once at least one instance is discovered and registered for a workspace,
 * every later call sees a non-empty scope for it and skips it — this is what keeps the common path
 * cheap: an already-populated workspace never re-lists its repo again.
 *
...
export async function backfillProviderInstances(options = {}) {
  ...
  for (const workspace of listWorkspaces(options)) {
    ...
    let slugs
    try {
      slugs = await discoverProviderInstances(workspace.provider, workspace.location, { pat: options.pat })
    } catch (err) {
