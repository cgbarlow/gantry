...
function assertValidEntryKind(location) {
  ...
  if (location.kind === 'azureDevOps' || location.kind === 'github') {
    return
  }
  throw new Error(`Unknown registry location kind "${location?.kind}" — expected "directory", "azureDevOps" or "github"`)
}
