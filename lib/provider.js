 ...
 *
 * Modeled now so Atlassian support could be added later without a schema migration — but only
 ...
 * provider value gets set runs the exact same check rather than each inventing its own.
 */
 ...
 *
 * A Library repo (ADR-0036) uses this exact same schema, for every provider including Atlassian —
 * `lib/librarySettings.js`'s own doc comment (#40): "the credential exemption doesn't shrink the
 * location shape." ADR-0042's "a Library repo needs only its Bitbucket environment credential" is
 * scoped to the *credential* (`lib/libraryCache.js`'s single-token `GANTRY_LIBRARY_PAT_ATLASSIAN`
 * slot, never a `{bitbucket, jira}` pair) — a library repo's `jiraSite`/`jiraProjectKey` are still
 * recorded (the Jira project the repo's own team happens to use) even though no Atlassian library-repo
 * operation (reading `definitions/`, opening a Promote pull request — both content-store-only) ever
 * reads them.
 */
...
/**
 * A `describeProviderLocation` sibling built for hashing (`lib/workspaceRegistry.js`'s
 * `deriveWorkspaceId`, #110), not for display: an EXACT, machine-readable, canonical string key for
 * `provider` + `location` — never lower-cased, never truncated, and stable across processes so the
 * same inputs always hash to the same UUIDv5.
 *
 * `location` is routed through `normalizeProviderLocation` first, so the key covers exactly that
 * provider's required-then-optional fields in the one canonical order (validating along the way — an
 * invalid location throws here exactly as it would registering one) and a stray field for the wrong
 * provider never leaks in.
 *
 * Each field is encoded as `key=<byteLength>:<value>` and joined with `;`. The length prefix, not the
 * separator character, is what marks a value's end — so a value that itself contains `;`, `=`, or a
 * digit sequence that looks like another field's prefix can never be misread as a field boundary; the
 * parser (there isn't one — this key is write-only) would always consume exactly `byteLength` bytes
 * before expecting the next `;`. `provider` is folded in as its own leading field for the same reason
 * `providerLocationsMatch`'s caller must already have checked providers match before comparing
 * locations — this key is provider-scoped by construction, so the same repository name under two
 * different providers can never collide.
 */
export function canonicalLocationKey(provider, location) {
  assertValidProvider(provider)
  const normalized = normalizeProviderLocation(provider, location, { entityLabel: 'A location' })
  const schema = LOCATION_SCHEMA[provider]
  const fields = [['provider', provider], ...[...schema.required, ...schema.optional]
