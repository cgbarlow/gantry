...
/**
 * Bitbucket's pull-requests capability (#46, docs/adr/0039/0042) — the Bitbucket twin of
 * `lib/gitlabPullRequestsClient.js`/`lib/githubPullRequestsClient.js`: open a pull request, read it
 * back (status + the data `interpretBitbucketPullRequest` below needs) and merge it with a merge
 * commit. Same `request()`/error-mapping pattern as `lib/bitbucketClient.js` so a caller working
 * across the content-store and pull-requests clients reads them the same way.
 *
 ...
 * GitHub's location keys). `baseUrl` defaults to Bitbucket Cloud's own fixed API host but is
 * overridable in tests, to point at `tests/helpers/fakeBitbucketServer.js` instead of
...
export function createBitbucketPullRequestsClient({ owner, repository, pat, baseUrl = DEFAULT_BASE_URL } = {}) {
  ...
  // active/completed/abandoned vocabulary `lib/githubPullRequestsClient.js`/`lib/gitlabPullRequestsClient.js`
  // already return, so a caller displaying a sign-off's status never has to branch on provider for the
  // word itself. 'DECLINED' (closed without merging) and 'SUPERSEDED' (replaced by another pull
  // request targeting the same branches) both read as `'abandoned'` — neither is a decided approval
  // outcome any more than GitHub's own 'closed'-but-not-merged is.
   ...
   * own `source.branch.name`/`destination.branch.name` fields).
   */
   ...
   * unlike GitLab's separate approvals/discussions calls).
   */
  ...
  /**
   * Attaches the given reviewers — `[{ id, required }]`, `id` being a Bitbucket account's `uuid`
   * (`lib/bitbucketIdentityClient.js`'s `resolveIdentity` result carries one). Bitbucket Cloud has no
   * dedicated "add a reviewer to an existing pull request" endpoint — reviewers are set via `PUT
   * .../pullrequests/{id}` with a full-replace `reviewers` array, the same full-replace shape
   * `lib/gitlabPullRequestsClient.js`'s own `addReviewers` (GitLab's `reviewer_ids`) already uses, so
   * this is called once, right after `createPullRequest`, exactly the way every other provider's own
   * `addReviewers` is. Bitbucket has no API-level "required reviewer" any more than GitHub/GitLab do —
   * `required` is accepted for shape-parity with those clients but otherwise unused; this only ever
   * *requests* review, per ADR-0040/0041's own "gantry never invents an enforcement mechanism the
   * provider itself doesn't have" stance, carried over unchanged for Bitbucket.
   */
...
/**
 * Turns a pull request's raw `participants` array into gantry's own pending/approved/
 * changes-requested vocabulary, per ADR-0042's own mapping: each `role === 'REVIEWER'` participant's
 * `state` is already exactly `'approved'`, `'changes_requested'` or `null` (no action yet) — a genuine
 * tri-state read directly off the participant, unlike GitLab's own inferred not-approved-plus-
 * unresolved-thread reading (`interpretGitLabMergeRequest`). `'changes_requested'` wins over
 * `'approved'` when reviewers disagree, mirroring `interpretGitHubReviews`'s identical precedence — a
 * single "send this back" from any reviewer is never masked by another reviewer's approval.
 * `role === 'PARTICIPANT'` entries (the author, or anyone who commented without being asked to review)
 * never count towards this reading — only `REVIEWER`s carry a real, review-shaped vote.
 */
export function interpretBitbucketPullRequest(participants) {
