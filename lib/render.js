...
import { readInstance, readModule, azureDevOpsOutPath } from './instance.js'
...
// The traceability footer (#98) every rendered artefact — across every
// definition, not just "design" — gets appended to its compiled markdown
// (and, via pandoc, its .docx) before being written/pushed anywhere: a
// plain paragraph (never a heading, so it doesn't show up in a docx's
// table of contents) naming the short commit hash and date the render's
// source data came from. `commit` is `{ hash, date }` — see
// `localHeadCommitInfo` (local instances) and `commitInfoFromPush` (Azure-
// DevOps-backed instances) below for where those two values come from.
function appendFooter(markdown, commit) {
  if (!commit) return markdown
  const body = markdown.replace(/\n+$/, '')
  // Plain text wrapping the commit hash in an inline code span, deliberately
  // not *also* wrapped in emphasis — pandoc's docx round-trip splits an
  // emphasis run around a nested inline-code span into separate runs
  // ("*text* `code` *text*"), which would make a simple substring/regex
  // match against the round-tripped text needlessly fragile.
  return `${body}\n\n---\n\nRendered from commit \`${commit.hash}\` (${commit.date}).\n`
}

// Local-instance source of the footer's commit info (#98): the current
// repo's HEAD at render time, via a plain read-only `git` call — the
// simplest available source when there's no push response to read it
// from instead (see commitInfoFromPush, the Azure-DevOps-backed
// equivalent, below). `cwd` defaults to the process's own working
// directory — the gantry repo itself, since local instances live inside
// it — but is overridable so tests can point this at a scratch repo
// instead of asserting against whatever commit this repo's HEAD happens
// to be at test-run time.
//
// Rendering a local instance never depended on `git` being installed or
// on `cwd` being inside a working tree before this footer existed — a
// missing/uninitialised repo now turns every local render into a hard
// failure. Rather than let that surface as `git`'s own raw stderr (e.g.
// "fatal: not a git repository"), it's caught and re-thrown as one clear,
// actionable error naming what's actually missing. `stdio: ['ignore',
// 'pipe', 'pipe']` is deliberate, not just `encoding` — execFileSync's
// default stdio *inherits* the child's stderr straight to this process's
// own, so without overriding it, git's raw error text would still print
// directly to the terminal even though the thrown JS error's own message
// is clean.
...
// Azure-DevOps-backed source of the footer's commit info (#98): read
// straight off the push response Azure DevOps already returns when the
// artefact is pushed (see renderArtefactFromAzureDevOps below) — never a
// separate call — since that response already carries the exact commit
// the push just created, before this function's caller re-pushes the
// artefact a second time with the footer naming it.
function commitInfoFromPush(push) {
  ...
  // Real Azure DevOps push responses always carry at least one of these
  // (committer/author date on the commit, or the push's own date) — this
  // is a fail-loudly guard against a malformed/unexpected response, not a
  // path any real render is expected to hit. Reusing the commit hash as a
  // fake "date" here would be a strictly worse failure mode: a footer that
  // silently claims a date has no meaningful value.
...
function compileArtefact(slug, artefact, instance, modules, options) {
  ...
  // options.commit (#98) is only absent while the Azure-DevOps-backed path
  // (below) is still on its first, footer-less compile/push — the one it
  // uses solely to learn its own push's resulting commit — so appendFooter
  // is a no-op there rather than an error.
  const markdown = appendFooter(resolved, options.commit)
...
export function renderArtefact(slug, artefactId, options = {}) {
  ...
  // #98: sourced via a plain `git` call against this repo's own current
  // HEAD — never a network call — since a local instance's render never
  // pushes anything the footer could otherwise be sourced from.
...
// lives in, at `gantry-workspace/<slug>/out/<artefactId>.docx` (#100) on
// `options.azureDevOps.branch` (falling through to the client's own
// `'main'` default when omitted — #118), overwriting any previous render
// of that artefact on that same branch (the repo's own history is the
// audit trail — no reason to also keep every past render as a distinct
// file). A dry run never reaches this — `compileArtefact` returns before
// writing anything, local or remote.
//
// The footer's commit hash/date (#98) can only be known once Azure DevOps
// has actually assigned them — neither is predictable client-side before a
// push completes — so getting an accurate, self-consistent footer into the
// pushed artefact itself takes two pushes: an initial, footer-less push
// (identical to this function's pre-#98 behaviour) purely to learn its own
// resulting commit from the response Azure DevOps already returns (never a
// separate call/round-trip to look that commit back up some other way),
// followed by a second push of the same path that overwrites it with the
// footer naming that commit. The instance/module data itself is only read
// once — both pushes recompile the one already-fetched `instance`/`modules`
// data, not two separate reads.
//
// Known, accepted limitation: a commit can never truthfully name its own
// hash inside its own content (the hash is computed from the content), so
// the footer names the *first* (content-establishing) push's commit, not
// the file's own literal newest commit in the repo's history — that's
// always the second (footer-only) push, one commit later. Anyone reading
// this file's blame/history in Azure DevOps will see that second commit as
// the most recent touch to the path. This is also not atomic: if the
// second push fails after the first succeeds, the repo is left holding the
// first push's footer-less content with no automatic rollback (see the
// catch below) — a subsequent successful render corrects it, but nothing
...
async function renderArtefactFromAzureDevOps(slug, artefactId, options) {
  ...
  const azureDevOpsPath = azureDevOpsOutPath(slug, artefact.id)
  ...
  try {
    ...
    // The first (content) push has already landed at this point — this is
    // the same "footer-less content is now live" partial-failure shape as
    // the second push's own catch below, just triggered by a malformed
    // response instead of a failed request, so it gets the same "here's
    // what's actually there" framing rather than a bare parsing error.
  ...
  try {
    ...
    // The first push already succeeded and is live in Azure DevOps at this
    // point — this failure only affects adding the footer, not whether a
    // render happened at all. Surfaced distinctly so a caller (and whoever
    // reads the error) knows the repo is left holding footer-less content
    // at commit ${commit.hash} rather than assuming the render itself
    // failed outright.
    throw new Error(
