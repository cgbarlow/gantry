...
function compileArtefact(slug, artefact, instance, modules, options) {
  ...
  if (options.dryRun) {
    return { markdown, mdPath, docxPath, basename, referenceDocPath, dryRun: true }
  }
  ...
  return { markdown, mdPath, docxPath, basename, referenceDocPath, dryRun: false }
}
...
async function renderArtefactFromAzureDevOps(slug, artefactId, options) {
  ...
  const commit = await pushDraftAndLearnCommit(client, azureDevOpsPath, artefact.id, draft.docxPath, branch)
