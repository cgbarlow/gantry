...
// The traceability footer (#98) every rendered artefact — across every definition, not just "design" — gets appended to its compiled markdown (and, via pandoc, its .docx) before being written/pushed anywhere: a plain paragraph (never a heading, so it doesn't show up in a docx's table of contents) naming the short commit hash and date the render's source data came from. `commit` is `{ hash, date }` — see `localHeadCommitInfo` (local instances) and `commitInfoFromPush` (Azure-DevOps-backed instances) below for where those two values come from.
 ...
 *
 * Never throws: a module save that already landed must not be reported as failed just because rendering one of its stage's artefacts ran into trouble afterwards. Two distinct non-error outcomes are reported per artefact instead of one another:
