...
export function getOrAssignInstanceNumber(scopeKey, slug, options = {}) {
  ...
  // #188: `options.persistedNumber` is the number recorded in the instance's own instance.yaml at
  // creation. This registry file lives on the server's own disk — wiped on every redeploy of an
  // ephemeral host — so the persisted number is what keeps a ref stable across restarts: it's
  // claimed whenever it's free, even over a number this slug was lazily handed earlier.
  const { persistedNumber } = options
  const persistedIsFree =
    Number.isInteger(persistedNumber) &&
    persistedNumber > 0 &&
    !Object.entries(scope).some(([other, n]) => other !== slug && n === persistedNumber)
  if (Object.hasOwn(scope, slug) && (scope[slug] === persistedNumber || !persistedIsFree)) return scope[slug]
  const number = persistedIsFree ? persistedNumber : nextNumber(scope)
  scope[slug] = number
...
/** Whether anything in `scopeKey` has been numbered yet — false for a scope this process hasn't listed since its registry file was (re)created. */
export function scopeHasInstanceNumbers(scopeKey, options = {}) {
  return Object.keys(readRegistryFile(registryPathFor(options)).instances[scopeKey] ?? {}).length > 0
}
...
/**
 * Resolves a numeric reference the same way `resolveSlugForRef` does, but returns `{ scopeKey, slug }`
 * instead of the bare slug — WI #356's addressing layer (`lib/server.js`) needs the scope a ref
 * resolved *within* too, so it can go straight to that one workspace's concrete directory instead of
 * re-searching every workspace for the slug afterward (which, unlike a ref itself, can genuinely be
 * ambiguous if the same slug happens to exist in more than one workspace — a ref never has that
 * problem, since its workspace number already pins down exactly one scope). `undefined` under the
 * exact same conditions `resolveSlugForRef` returns `undefined`.
 */
export function resolveScopeAndSlugForRef(ref, options = {}) {
  ...
  const slug = resolveSlugByNumber(backfilledScopeKey, instanceNumber, options)
  return slug ? { scopeKey: backfilledScopeKey, slug } : undefined
}
...
export function resolveSlugForRef(ref, options = {}) {
  return resolveScopeAndSlugForRef(ref, options)?.slug
}
...
//
// WI #356: every LOCAL_SCOPE-scoped instance now physically lives under the reserved `default`
// server workspace folder (`MIGRATED_DEFAULT_WORKSPACE_FOLDER`, `lib/instanceRegistry.js`), not
// directly under `instancesDir` (the workspaces root) the way a genuinely pre-#356 bare instance
// used to — so the path this stats has to account for that one extra path segment.
function localInstanceOrderKey(instancesDir, slug) {
  try {
    const stat = statSync(join(instancesDir, MIGRATED_DEFAULT_WORKSPACE_FOLDER, slug))
...
// The workspace half of `backfillNumberRegistry` below: numbers every known workspace not yet
// numbered, in a fixed order (workspace registry order, then server workspace folders). Returns how
// many were newly numbered; the caller writes `data`.
function assignWorkspaceNumbers(data, instancesDir) {
  ...
  // WI #356: a server workspace directory is a numbering scope too, exactly like an Azure DevOps
  // workspace — except the one reserved for the pre-#356 local scope (lib/instanceRegistry.js's
  // `MIGRATED_DEFAULT_WORKSPACE_FOLDER`/`scopeIdForDirectoryFolder`), which keeps `LOCAL_SCOPE` and
  // never gets a sequential number here (`instanceNumbersFor` hardcodes it to `LOCAL_WORKSPACE_NUMBER`
  // directly, with no `data.workspaces` entry at all — see that function's own comment).
  for (const { id: folder } of listServerWorkspaces(instancesDir)) {
    const scopeId = scopeIdForDirectoryFolder(folder)
    if (scopeId === LOCAL_SCOPE) continue
    if (!Object.hasOwn(data.workspaces, scopeId)) {
      data.workspaces[scopeId] = nextNumber(data.workspaces)
      workspacesAssigned++
    }
  }
  return workspacesAssigned
}

/**
 * #188: numbers every known workspace up front, at boot. Numbers used to be handed out lazily in
 * whatever order requests first touched each workspace — so on a host whose disk (and this registry
 * file with it) is wiped on redeploy, the same workspace could come back as `w1` one day and `w2`
 * the next, and an old ref silently opened someone else's instance. Numbering them eagerly, in a fixed
 * order, before any request arrives makes a given set of workspaces always number the same way.
 */
export function backfillWorkspaceNumbers(options = {}) {
  const instancesDir = options.instancesDir ?? 'instances'
  const registryPath = registryPathFor(options)
  const data = readRegistryFile(registryPath)
  const assigned = assignWorkspaceNumbers(data, instancesDir)
  if (assigned > 0) writeRegistryFile(registryPath, data)
  return assigned
}
...
export function backfillNumberRegistry(options = {}) {
  ...
  const workspacesAssigned = assignWorkspaceNumbers(data, instancesDir)

  // WI #356: instance-registry.json is scope-nested (a directory workspace's scope id, or an Azure
  // DevOps workspace's uuid — see lib/instanceRegistry.js), so slugs are paired with their own scope
  // up front rather than classified into a single shared "local" bucket vs "remote" the way pre-#356
  // bare instances used to be. The `LOCAL_SCOPE` bucket still exists — it's just now one scope among
  // several, reached only by a slug whose scope id happens to be `LOCAL_SCOPE` (every pre-#356 local
  // instance, and everything migrated into the reserved `default` server workspace,
  // lib/instanceRegistry.js's `scopeIdForDirectoryFolder`).
  const pairs = listRegisteredScopedSlugsInStorageOrder({ instancesDir })
  const localScopePairs = pairs.filter((p) => p.workspace === LOCAL_SCOPE)
  const otherScopePairs = pairs.filter((p) => p.workspace !== LOCAL_SCOPE)

  let instancesAssigned = 0
  // The LOCAL_SCOPE bucket alone keeps its filesystem-timestamp creation-order proxy (a real,
  // per-entity signal) — every instance in it, whether a genuine pre-#356 local instance or one
  // migrated into the reserved `default` server workspace, still lives at a real path under
  // `instancesDir` a timestamp can be read from.
  const orderedLocalSlugs = [...localScopePairs].sort(
    (a, b) => localInstanceOrderKey(instancesDir, a.slug) - localInstanceOrderKey(instancesDir, b.slug)
  )
  ...
  // Every other scope (a real directory or Azure DevOps workspace) keeps instance-registry.json's
  // own raw key order within that one scope (its own best-effort creation-order proxy, same reasoning
  // as the workspace loop above) — the order slugs of different workspaces happen to interleave in
  // that one shared file has no bearing on any one workspace's own instance numbering.
  for (const { slug, workspace: scopeId } of otherScopePairs) {
    const scope = data.instances[scopeId] ?? {}
    if (!Object.hasOwn(scope, slug)) {
      scope[slug] = nextNumber(scope)
      data.instances[scopeId] = scope
      instancesAssigned++
    }
  }

  // WI #370: only write when something was actually assigned. This used to write unconditionally,
  // which meant a backfill that found nothing to do still tried to create `number-registry.json` —
  // and crashed with ENOENT when the directory it was pointed at doesn't exist yet, the ordinary case
  // for a workspaces root before anything has been created in it. A no-op backfill should leave the
  // filesystem exactly as it found it. Whenever there *is* something to assign, the directory
  // necessarily exists, because that's where the workspaces and instances being numbered were found.
  if (workspacesAssigned > 0 || instancesAssigned > 0) writeRegistryFile(registryPath, data)
