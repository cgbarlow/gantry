...
// Resolves which stage a gate check runs against: `options.gate` (any stage
// whose `gate` matches, not just the instance's current one), falling back
// to the instance's own current stage. Shared by the local and Azure-
// DevOps-backed halves of `checkGate` below so the two can never drift in
// how they pick a stage, only in how the instance/modules were read.
function resolveCheckStage(definition, instance, slug, options) {
  if (options.gate) {
...
function checkResultFor(slug, definition, stage, modules, complete) {
  return {
    slug,
 ...
 * checkGateFromAzureDevOps below) and returns a Promise the caller must
 * `await`. Without it — every existing caller — this stays exactly the
 * synchronous local-filesystem check it always was. Added for #103's
 * confirmed gate-pass-to-work-item sync, which must be able to check an
 * Azure-DevOps-backed instance's gate the same way it checks a local one —
 * a gap this ticket also closes in `GET /api/instance/check`
 * (lib/server.js), which previously only ever checked local instances.
 */
export function checkGate(slug, options = {}) {
  ...
  return checkResultFor(slug, definition, stage, modules, complete)
}

// The Azure-DevOps-backed half of checkGate (#103) — the exact same
// stage-resolution/strict-evaluation logic as the local path, over an
// instance/its modules read from Azure DevOps instead of disk.
async function checkGateFromAzureDevOps(slug, options) {
  ...
  const { modules, complete } = await evaluateStage(definition, stage, slug, {
