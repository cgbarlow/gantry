...
function checkResultFor(slug, definition, stage, modules, artefacts, complete, warnings) {
  return {
    slug,
...
// stage's union of modules as if every artefact had to be complete. Only an
// artefact that can pass the gate is a candidate (#151, ADR-0051): completing a
// `satisfies-gate: false` one would never pass it, however close it is.
...
export function checkGate(slug, options = {}) {
  ...
  if (options.github) {
    return checkGateFromGitHub(slug, options)
  }
  if (options.gitlab) {
    return checkGateFromGitLab(slug, options)
  }
  if (options.atlassian) {
    return checkGateFromAtlassian(slug, options)
  }
  ...
  const { modules, artefacts, complete, warnings } = evaluateStage(definition, stage, slug, { instancesDir, strict: true })

  return checkResultFor(slug, definition, stage, modules, artefacts, complete, warnings)
}

// The GitHub-backed half of checkGate (#14) — the exact same stage-resolution/strict-evaluation logic
// as the Azure DevOps half below, over an instance/its modules read from a GitHub repo instead. There
// is no per-stage GitHub branch yet (#12 is a later ticket), so `options.github` is passed straight
// through without any branch resolution — every read falls through to `main`, `lib/githubClient.js`'s
// own default.
async function checkGateFromGitHub(slug, options) {
  const definitionsDir = options.definitionsDir ?? 'definitions'
  ...
  const { modules, artefacts, complete, warnings } = await evaluateStage(definition, stage, slug, {
...
// The GitLab twin of checkGateFromGitHub above (#33, ADR-0041) — identical shape, over an
// instance/its modules read from a GitLab project instead. Stage branches exist for GitLab (#29,
// unlike GitHub when its own equivalent checkGate half first landed), but `options.gate`/the
// instance's own current stage is resolved the same way regardless — the caller (`lib/stageApproval.js`'s
// `requestGitLabStageApproval`) is responsible for supplying `options.gitlab.branch` when a specific
// stage's own branch should be read instead of `main`.
async function checkGateFromGitLab(slug, options) {
  const definitionsDir = options.definitionsDir ?? 'definitions'
...
// The Atlassian (Bitbucket-backed) twin of checkGateFromGitLab above (#46, ADR-0042) — identical
// shape, over an instance/its modules read from a Bitbucket repository instead. `options.gate`/the
// instance's own current stage is resolved the same way regardless — the caller
// (`lib/stageApproval.js`'s `requestAtlassianStageApproval`) is responsible for supplying
// `options.atlassian.branch` when a specific stage's own branch should be read instead of `main`.
async function checkGateFromAtlassian(slug, options) {
  const definitionsDir = options.definitionsDir ?? 'definitions'
