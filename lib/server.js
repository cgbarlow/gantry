...
import { readInstance, readModule, writeModule, writeModules, createInstance, importInstanceToAzureDevOps, updateInstanceAssignee, updateInstanceRequiredReviewer, recordInstanceWorkItemLink, instanceDefinitionVersion, AZURE_DEVOPS_WORKSPACE_ROOT, githubAssetsDir, githubAssetPath } from './instance.js'
import { getStatus } from './status.js'
import { checkGate, resolveCheckStage } from './check.js'
import { renderArtefact, prepareAzureDevOpsWasmRender, finishAzureDevOpsWasmRender, prepareGitHubWasmRender, finishGitHubWasmRender, externaliseImagesForWasm } from './render.js'
...
import { refreshAllLibraryRepos, readLibraryRepoCacheMeta } from './libraryCache.js'
import {
...
import { assertValidProvider, normalizeProviderLocation, DEFAULT_PROVIDER } from './provider.js'
import {
  instanceNumbersFor,
...
import { checkAzureDevOpsRepo, checkGitHubRepo } from './repoCheck.js'
import {
...
function buildReviewSummaryForArtefact(definition, artefact, instance, azureDevOpsLocation, commitInfo = null, provider = 'azure-devops') {
  ...
  // #16: `azureDevOpsLocation` doubles as "whichever provider's location this render is against" —
  // `provider` picks which provider's own commitUrl builder reads it, since the two shapes
  // (`{organization,project,repository}` vs `{owner,repository}`) aren't interchangeable.
    ...
    ? {
        ...
        url:
          azureDevOpsLocation && commitInfo.fullHash
            ? provider === 'github'
              ? githubCommitUrl(azureDevOpsLocation, commitInfo.fullHash)
              : commitUrl(azureDevOpsLocation, commitInfo.fullHash)
            : undefined,
...
async function collectOtherAzureDevOpsDefinitionIds({ instancesDir, pat, excludeWorkspaceId } = {}) {
  ...
  const workspaces = listWorkspaces({ instancesDir }).filter(
    (w) => w.ticketingSystem === 'azure-devops' && w.id !== excludeWorkspaceId
  )
  const ids = []
...
export function createServer(options = {}) {
  ...
  // counterpart, resolved the same "option first, env var fallback" way. `options.libraryPats`, if
  // supplied, overrides both at once — for a test (or embedder) that wants to hand the whole
  // per-provider map directly rather than one option per provider.
  const libraryPats =
  ...
  // Promote (WI #387, ADR-0036) is Azure-DevOps-only until #20 gives it a GitHub counterpart — every
  // existing Promote call site below keeps naming this single, Azure-DevOps-specific credential.
  const libraryPat = libraryPats['azure-devops']
  ...
  return createHttpServer(async (req, res) => {
    try {
      ...
      if (url.pathname === '/api/definitions' && req.method === 'POST') {
        ...
        // no-shadowing rule — a collision in any *other* home, including an Azure DevOps workspace's
        // (`collectOtherAzureDevOpsDefinitionIds`, above) — is the new 409, checked only when the
        // target itself is free, so the pre-existing 400 contract for a same-store collision never
        // changes underfoot.
        if (!definitionIdExistsIn(targetDefinitionsDir, newId)) {
          try {
            const azureDevOpsIds = await collectOtherAzureDevOpsDefinitionIds({ instancesDir, pat: getCredential(req) })
            assertDefinitionIdAvailable(newId, { definitionsDir, instancesDir, extraKnownIds: azureDevOpsIds })
          } catch (err) {
      ...
      if (promoteMatch && req.method === 'POST') {
        ...
        if (!libraryPat) {
          sendJSON(res, 400, { error: 'No server PAT configured (GANTRY_LIBRARY_PAT) — cannot promote to a library repo.' })
          return
        }
        const files = readWorkspaceVersionFolderFiles(workspaceDefinitionsDir, rawId, vNum)
        const results = await fanOutPromoteDefinitionVersion({ definitionId: rawId, version: vNum, files, repos, pat: libraryPat })
      ...
      if (promotionsCheckMatch && req.method === 'POST') {
        ...
        if (!libraryPat) {
          sendJSON(res, 400, { error: 'No server PAT configured (GANTRY_LIBRARY_PAT) — cannot check promotion status.' })
          return
        }
        const vNum = Number(rawVersion)
        const workspaceDefinitionsDir = serverWorkspaceDefinitionsDir(instancesDir, row.home.id)
        ...
        for (const promotion of existing) {
          ...
          if (!repo) continue
          try {
            const updated = await checkPromotionStatus({ promotion, repo, pat: libraryPat })
      ...
      // WI #383 (ADR-0036) — an Azure DevOps workspace's own `definitions/` folder, committed straight
      // to that workspace repo's `main` (single commits, not stage branches or PRs — `lib/definitionAzureDevOps.js`'s
      // own doc comment has the full reasoning). A deliberately smaller surface than the library/server-
      // workspace routes above: list, create-blank, new-draft-version, load, save and publish, gated
      // behind the caller's own PAT exactly like every other Azure-DevOps-backed route in this file
      // (`withAzureDevOpsCredential`) — no templates/reference-docx/clone yet (see this ticket's final
      // report for what's left). `:workspaceId` is the Azure DevOps workspace's own registry id
      // (`lib/workspaceRegistry.js`), not a server workspace folder name.
      const azureDevOpsDefinitionsListMatch = url.pathname.match(/^\/api\/workspaces\/([^/]+)\/definitions$/)
      if (azureDevOpsDefinitionsListMatch && req.method === 'GET') {
        const workspaceId = decodeURIComponent(azureDevOpsDefinitionsListMatch[1])
      ...
      // into an Azure DevOps workspace isn't implemented yet; bring content in via WI #382's copy flow
      // once the definition exists). Same "unique across the library and every workspace" 409 guard as
      // the library/server-workspace create route, extended with this workspace's own already-known
      // ids (fetched with the caller's own PAT, since this module can't scan an Azure DevOps workspace
      // without one — see lib/definitionHome.js's own doc comment on why).
      if (azureDevOpsDefinitionsListMatch && req.method === 'POST') {
        const workspaceId = decodeURIComponent(azureDevOpsDefinitionsListMatch[1])
        const workspace = resolveWorkspace(workspaceId, { instancesDir })
        if (!workspace) {
          sendJSON(res, 404, { error: `Unknown workspace "${workspaceId}"` })
        ...
        if (body?.sourceId) {
          sendJSON(res, 400, { error: 'Cloning into an Azure DevOps workspace is not supported yet — create blank, then copy elements in.' })
          return
        }
        await withAzureDevOpsCredential(req, res, workspace.location, async (azureDevOps) => {
          const existingIds = await listAzureDevOpsDefinitionIds({ azureDevOps })
          // No-shadowing (ADR-0036) also covers every *other* registered Azure DevOps workspace, not
          // just this one — see `collectOtherAzureDevOpsDefinitionIds`'s own doc comment above.
          ...
          try {
            assertDefinitionIdAvailable(newId, { definitionsDir, instancesDir, extraKnownIds: [...existingIds, ...otherAzureDevOpsIds] })
          } catch (err) {
      ...
      const azureDevOpsNewDraftMatch = url.pathname.match(/^\/api\/workspaces\/([^/]+)\/definitions\/([^/]+)\/versions$/)
      if (azureDevOpsNewDraftMatch && req.method === 'POST') {
        const workspaceId = decodeURIComponent(azureDevOpsNewDraftMatch[1])
        const rawId = decodeURIComponent(azureDevOpsNewDraftMatch[2])
        const workspace = resolveWorkspace(workspaceId, { instancesDir })
        if (!workspace) {
          sendJSON(res, 404, { error: `Unknown workspace "${workspaceId}"` })
          return
      ...
      const azureDevOpsVersionMatch = url.pathname.match(/^\/api\/workspaces\/([^/]+)\/definitions\/([^/]+)\/versions\/([^/]+)$/)
      if (azureDevOpsVersionMatch && (req.method === 'GET' || req.method === 'PUT')) {
        const workspaceId = decodeURIComponent(azureDevOpsVersionMatch[1])
        const rawId = decodeURIComponent(azureDevOpsVersionMatch[2])
        const rawVersion = decodeURIComponent(azureDevOpsVersionMatch[3])
        const workspace = resolveWorkspace(workspaceId, { instancesDir })
        if (!workspace) {
          sendJSON(res, 404, { error: `Unknown workspace "${workspaceId}"` })
          return
        ...
        if (req.method === 'GET') {
          await withAzureDevOpsCredential(req, res, workspace.location, async (azureDevOps) => {
            try {
              const def = await loadAzureDevOpsDefinition(rawId, vNum, { azureDevOps })
              sendJSON(res, 200, definitionVersionProjection(def))
            } catch (err) {
        ...
        // when" bar: this is the path exercised against the fake Azure DevOps server).
        const rawBody = await readBody(req)
        if (rawBody.length > 512 * 1024) {
          sendJSON(res, 413, { error: 'Request body too large' })
          return
      ...
      const azureDevOpsPublishMatch = url.pathname.match(/^\/api\/workspaces\/([^/]+)\/definitions\/([^/]+)\/versions\/([^/]+)\/publish$/)
      if (azureDevOpsPublishMatch && req.method === 'POST') {
        const workspaceId = decodeURIComponent(azureDevOpsPublishMatch[1])
        const rawId = decodeURIComponent(azureDevOpsPublishMatch[2])
        const rawVersion = decodeURIComponent(azureDevOpsPublishMatch[3])
        const workspace = resolveWorkspace(workspaceId, { instancesDir })
        if (!workspace) {
          sendJSON(res, 404, { error: `Unknown workspace "${workspaceId}"` })
          return
      ...
      const azureDevOpsArchiveMatch = url.pathname.match(/^\/api\/workspaces\/([^/]+)\/definitions\/([^/]+)\/(archive|restore)$/)
      if (azureDevOpsArchiveMatch && req.method === 'POST') {
        const workspaceId = decodeURIComponent(azureDevOpsArchiveMatch[1])
        const rawId = decodeURIComponent(azureDevOpsArchiveMatch[2])
        const action = azureDevOpsArchiveMatch[3]
        const workspace = resolveWorkspace(workspaceId, { instancesDir })
        if (!workspace) {
          sendJSON(res, 404, { error: `Unknown workspace "${workspaceId}"` })
          return
      ...
      if (url.pathname === '/api/workspaces' && req.method === 'POST') {
        ...
        const ticketingSystem = body?.ticketingSystem
        ...
        // `ticketingSystem` only means anything for azure-devops (github has no ticketing-system
        // choice of its own — the provider itself is the suite, ADR-0037) and only for the pre-#3
        // flat wire shape — under the provider model a nested caller's `provider` already says this —
        // but it's still validated for that flat caller so "jira" keeps being rejected exactly as before.
        if (provider === 'azure-devops' && !isNested) {
          try {
        ...
        // See createServer's own doc comment on allowAzureDevOpsBaseUrlOverride/allowGitHubBaseUrlOverride.
        const baseUrlAllowed = provider === 'github' ? allowGitHubBaseUrlOverride : allowAzureDevOpsBaseUrlOverride
        if (rawLocation?.baseUrl !== undefined && !baseUrlAllowed) {
          sendJSON(res, 400, { error: `${provider === 'github' ? 'github' : 'azureDevOps'}.baseUrl overrides are not permitted on this server` })
          return
        }
        ...
        try {
          ...
          if (provider === 'github') {
            await checkGitHubRepo({ ...location, pat })
          } else {
            await checkAzureDevOpsRepo({ ...location, pat, definitionsDir })
          }
        } catch (err) {
          if (err instanceof AuthenticationError) {
            sendAuthenticationRequired(res, { provider: err.provider })
      ...
      if (url.pathname === '/api/instances' && req.method === 'POST') {
        ...
        // instead. No stage-branch targeting yet (#12) — createInstance's GitHub path always writes
        // to `main`.
        if (github !== undefined) {
      ...
      if (url.pathname === '/api/instance' && req.method === 'GET') {
        ...
        if (githubLocation) {
          await withGitHubCredential(req, res, githubLocation, async (githubBase) => {
            // #11: no stage-branch lifecycle yet (#12) — every read is against `main` directly, unlike
            // the Azure-DevOps-backed branch above which resolves a per-stage branch first.
            ...
            const modules = await Promise.all(
              stage.modules.map(async (moduleId) => {
                let data = { status: 'draft', owner: '', fields: {} }
                try {
                  data = await readModule(definition, slug, moduleId, { github: githubBase })
                } catch (err) {
                  if (err instanceof AuthenticationError) throw err
                  if (!/has no saved data/.test(err.message)) throw err
                }
      ...
      if (url.pathname === '/api/instance/modules' && req.method === 'PUT') {
        ...
        if (githubLocation) {
          await withGitHubCredential(req, res, githubLocation, async (githubBase) => {
            ...
            // itself (a single Git Data API commit for every changed module); no stage-branch
            // targeting yet (#12), so this always lands on `main`.
            const saved = await writeModules(definition, slug, modules, { github: githubBase, message: `Save ${stage.title}: ${titles.join(', ')}` })
            const status = await getStatus(slug, { github: githubBase, definitionsDir, stageId })
            sendJSON(res, 200, { ...status, saved: saved.modules, commit: saved.commit })
          })
          return
        }
      ...
      if (moduleMatch && req.method === 'PUT') {
        ...
        if (githubLocation) {
          await withGitHubCredential(req, res, githubLocation, async (githubBase) => {
            ...
            const body = JSON.parse(await readBody(req))
            await writeModule(definition, slug, moduleId, body, { github: githubBase })
            // No render-on-save (WI #376, ADR-0034): documents are rendered only when the author clicks Render.
            sendJSON(res, 200, await getStatus(slug, { github: githubBase, definitionsDir, stageId }))
          })
          return
        }
      ...
      if (wasmPrepareMatch && req.method === 'POST') {
        ...
        if (!azureDevOpsLocation && !githubWasmPrepareLocation) {
      ...
      if (wasmFinishMatch && req.method === 'POST') {
        ...
        if (!azureDevOpsLocation && !githubWasmFinishLocation) {
      ...
      // Identity search endpoint (#145 Part 2; extended #10 for GitHub, docs/adr/0040) — proxies a
      // provider's own identity/collaborator directory behind the standard PAT credential seam.
      // Returns an array of `{ uniqueName, displayName, emailAddress, id }` objects for the front-end's
      // identity picker component — plus, for GitHub, `canAssign` and (when `false`) `blockedReason`,
      // since GitHub rejects an issue assignee without repo access. Scoped to whichever location the
      // caller names: a `slug` (an already-known instance — Azure DevOps only today, since a
      // GitHub-backed instance doesn't have this lookup wired in yet), explicit `organization`+`project`
      // (Azure DevOps) or `provider=github`+`owner`+`repository` query params (mirrors
      // GET /api/azure-devops/work-item-types — the wizard's own Register step, before any workspace
      // exists at all), or — same as before #10 — the first registered workspace, of whichever
      ...
      if (url.pathname === '/api/local/definitions/promote' && req.method === 'POST') {
        ...
        const results = await fanOutPromoteDefinitionVersion({ definitionId, version, files, repos, pat: libraryPat })
