...
export function createServer(options = {}) {
  ...
  return createHttpServer(async (req, res) => {
    try {
      ...
      // SPA fallback: a path that isn't a real static file under webDir and
      // doesn't look like it's asking for one (no file extension — /app.js,
      // /style.css etc. all have one and fall through to serveStaticFile's
      // own 404) is a client-side route for preact-iso's <Router> to match
      // (e.g. /setup, the instance-setup wizard, #78) — served index.html
      // exactly as / is, rather than 404ing on a direct navigation/reload.
      // serveStaticFile itself still guards the real-file path below against
      // traversal, so this existsSync check is only ever a routing decision,
      // never what actually serves a file.
      if (!extname(url.pathname) && !existsSync(join(webDir, url.pathname.slice(1)))) {
        const html = readFileSync(join(webDir, 'index.html'), 'utf8').replace(
          '"__IMPORT_MAP__"',
          JSON.stringify(importMap)
        )
