...
export function createServer(options = {}) {
  ...
  return createHttpServer(async (req, res) => {
    try {
      ...
      // Which workspace (if any) a slug belongs to (#104) — a small, read-only, registry-only lookup with no Azure DevOps call and no PAT required (mirrors `GET /api/workspaces`'s own "no credential needed" note). This is what the client-side request layer (web/lib/apiFetch.js's `apiFetchForInstance`) asks *before* it decides which PAT to attach to a request that actually does touch Azure DevOps — resolving that from the response body alone would be too late, since the very first Azure-DevOps-touching request for a workspace-overridden instance needs the right credential attached from the start, not just on a 401 retry. `workspaceId` is `null` for a local instance, or one this registry has never seen.
