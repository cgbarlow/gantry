...
export function createServer(options = {}) {
  ...
  return createHttpServer(async (req, res) => {
    try {
      ...
      // Registers a new workspace directly — the primitive the future Settings screen (#101/#104) will build on. Structural validation errors (missing location fields, or an unsupported `ticketingSystem` such as `jira`) are checked first and reported as 400 with no network cost, exactly like every other validating route in this file. Unlike a plain metadata store, though, this route *establishes* a remote location the same way `POST /api/instances`/`POST /api/instances/adopt` already do — so it requires the caller's own PAT (the same credential-provider seam, #86) and actually proves that PAT against this exact location via `checkAzureDevOpsRepo` (the same check `/api/instances/adopt` already performs) before persisting anything. Without this, any caller with mere network access to this server — no credential needed at all — could register a workspace (or, since a location isn't unique here, have it silently reused by a later legitimate registration for the exact same repo via `getOrCreateWorkspace`'s tuple matching) carrying a spoofed `owner`/`baseUrl` for a repo it has no real access to. `baseUrl` is gated behind the same `allowAzureDevOpsBaseUrlOverride` flag every other caller-supplied `baseUrl` in this file already requires, for the same SSRF-prevention reason documented on that option above.
      //
      // Accepts either wire shape (ticket #5, ADR-0037): the #3 nested `{ provider, location: {...}, owner? }` the "+ New Workspace" wizard now sends, or the pre-#3 flat `{ organization, project, repository, baseUrl?, owner?, ticketingSystem? }` still exercised by this suite's own back-compat coverage — detected the same way `lib/workspaceRegistry.js`'s own `isNestedInput` is, on the presence of a `location` key. Either way, only `provider: 'azure-devops'` actually registers anything today: the repo-proving step below (`checkAzureDevOpsRepo`) has no GitHub equivalent yet, so a `provider: 'github'` request gets a clear "not supported yet" 400 rather than silently mis-proving access against the wrong API.
      if (url.pathname === '/api/workspaces' && req.method === 'POST') {
        ...
        if (provider !== 'azure-devops') {
          sendJSON(res, 400, { error: `Provider "${provider}" workspaces are not supported yet — only azure-devops is available today` })
          return
        }
        ...
        // `ticketingSystem` only means anything for the pre-#3 flat wire shape — under the provider
        // model (ADR-0037) a nested caller's `provider` already says this — but it's still validated
        // for that flat caller so "jira" keeps being rejected exactly as before.
        if (!isNested) {
          try {
        ...
        if (rawLocation?.baseUrl !== undefined && !allowAzureDevOpsBaseUrlOverride) {
