  ...
  return createHttpServer(async (req, res) => {
    try {
      ...
      if (url.pathname === '/api/instance/work-items/sync' && req.method === 'POST') {
        ...
        const pat = getCredential(req)
        if (!pat) {
          sendAuthenticationRequired(res, { provider: githubLocation ? 'github' : 'azure-devops' })
          return
        }
        ...
        } else if (githubLocation) {
          // No per-stage GitHub branch yet (#12) — reads/writes go straight to `main`.
          instanceOptions = { github: { ...githubLocation, pat } }
        }
