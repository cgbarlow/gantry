...
function buildReviewSummaryForArtefact(definition, artefact, instance, azureDevOpsLocation, commitInfo = null, provider = 'azure-devops') {
  ...
  const commitUrlBuilders = { github: githubCommitUrl, 'azure-devops': commitUrl }
...
// #24: every `/api/workspaces/:workspaceId/definitions*` route below dispatches on
// `workspace.provider` — a GitHub branch, then an *unconditional* Azure DevOps one, the same
// "not GitHub ⇒ must be Azure DevOps" assumption ADR-0039 already rejected. Since #24 also adds
// `gitlab` as a validated, `getOrCreateWorkspace`-accepted provider (ADR-0041) with no
// workspace-definitions support of its own yet, an unguarded fallback here would run Azure DevOps
// logic against a GitLab-shaped workspace location. Call this once the GitHub branch has already
// returned, immediately before the Azure DevOps logic; a `gitlab` (or any other non-azure-devops)
// workspace gets a clear, correct "not supported yet" instead — returns whether it already responded,
// so a caller writes `if (rejectUnlessAzureDevOpsWorkspace(res, workspace)) return`.
  ...
  return createHttpServer(async (req, res) => {
    try {
      ...
      if (url.pathname === '/api/definitions' && req.method === 'POST') {
        ...
        // no-shadowing rule — a collision in any *other* home, including an Azure DevOps or GitHub
        // workspace's (`collectOtherAzureDevOpsDefinitionIds`/`collectOtherGitHubDefinitionIds`,
        // above) — is the new 409, checked only when the target itself is free, so the pre-existing
        // 400 contract for a same-store collision never changes underfoot.
        if (!definitionIdExistsIn(targetDefinitionsDir, newId)) {
          try {
            ...
            assertDefinitionIdAvailable(newId, { definitionsDir, instancesDir, extraKnownIds: [...azureDevOpsIds, ...githubIds] })
          } catch (err) {
      ...
      // WI #383/#17 (ADR-0036, ADR-0037) — a remote workspace's own `definitions/` folder, committed
      // straight to that workspace repo's `main` (single commits, not stage branches or PRs —
      // `lib/definitionAzureDevOps.js`'s own doc comment has the full reasoning, identical for
      // GitHub). A deliberately smaller surface than the library/server-workspace routes above: list,
      // create-blank, new-draft-version, load, save and publish, gated behind the caller's own PAT
      // exactly like every other provider-backed route in this file (`withAzureDevOpsCredential`/
      // `withGitHubCredential`) — no templates/reference-docx/clone yet (see this ticket's final
      // report for what's left). `:workspaceId` is the workspace's own registry id
      // (`lib/workspaceRegistry.js`), not a server workspace folder name — dispatched on
      // `workspace.provider` (#17: Azure DevOps and GitHub share this one route surface).
      ...
      // plus every *other* Azure DevOps and GitHub workspace's (fetched with the caller's own PAT,
      // since this module can't scan a remote workspace without one — see lib/definitionHome.js's own
      // doc comment on why) — a duplicate id is refused regardless of which two homes it collides
      // across (ADR-0036/#17's own "no shadowing" acceptance criterion).
      if (workspaceDefinitionsListMatch && req.method === 'POST') {
        ...
        if (workspace.provider === 'github') {
          await withGitHubCredential(req, res, workspace.location, async (github) => {
            ...
            try {
              assertDefinitionIdAvailable(newId, { definitionsDir, instancesDir, extraKnownIds: [...existingIds, ...otherGitHubIds, ...azureDevOpsIds] })
            } catch (err) {
        ...
        await withAzureDevOpsCredential(req, res, workspace.location, async (azureDevOps) => {
          ...
          // No-shadowing (ADR-0036) also covers every *other* registered Azure DevOps or GitHub
          // workspace, not just this one — see `collectOtherAzureDevOpsDefinitionIds`/
          // `collectOtherGitHubDefinitionIds`'s own doc comments above.
          ...
          try {
            assertDefinitionIdAvailable(newId, { definitionsDir, instancesDir, extraKnownIds: [...existingIds, ...otherAzureDevOpsIds, ...githubIds] })
          } catch (err) {
            sendJSON(res, err instanceof DefinitionIdConflictError ? 409 : 400, { error: err.message })
            return
          }
      ...
      if (workspaceVersionMatch && (req.method === 'GET' || req.method === 'PUT')) {
        ...
        if (!isGitHub && rejectUnlessAzureDevOpsWorkspace(res, workspace)) return

        if (req.method === 'GET') {
          const loadFn = isGitHub ? (id, v, opts) => loadGitHubDefinition(id, v, opts) : (id, v, opts) => loadAzureDevOpsDefinition(id, v, opts)
          const withCredential = isGitHub ? withGitHubCredential : withAzureDevOpsCredential
          await withCredential(req, res, workspace.location, async (providerConfig) => {
            try {
              const def = await loadFn(rawId, vNum, isGitHub ? { github: providerConfig } : { azureDevOps: providerConfig })
              sendJSON(res, 200, definitionVersionProjection(def))
            } catch (err) {
        ...
        // when" bar: this is the path exercised against the fake Azure DevOps/GitHub servers).
        const rawBody = await readBody(req)
        if (rawBody.length > 512 * 1024) {
          sendJSON(res, 413, { error: 'Request body too large' })
          return
      ...
      if (url.pathname === '/api/workspaces' && req.method === 'POST') {
        ...
        // `checkAzureDevOpsRepo` below) for any third *declared* provider, e.g. a `gitlab` workspace
        // (ADR-0041 — `gitlab` validates as a supported provider per lib/provider.js, but has no
        // registered repo-check or base-url-override flag of its own yet). Provider not in either
        // table below → base URL overrides default to disallowed (fails safe) and the repo check is
        // reported as unsupported rather than run against the wrong provider's client.
        const BASE_URL_OVERRIDE_ALLOWED = { github: allowGitHubBaseUrlOverride, 'azure-devops': allowAzureDevOpsBaseUrlOverride }
        const REPO_CHECKERS = { github: checkGitHubRepo, 'azure-devops': checkAzureDevOpsRepo }
        ...
        try {
          ...
          await checkRepo({ ...location, pat, definitionsDir })
        } catch (err) {
          if (err instanceof AuthenticationError) {
            sendAuthenticationRequired(res, { provider: err.provider })
            return
      ...
      if (url.pathname === '/api/instance/work-items/link' && req.method === 'POST') {
        ...
        // #24: a declared provider other than 'github' used to fall straight through to the Azure
        // DevOps branch below unconditionally (`body?.provider === 'github' ? ... : <assume azure-
        // devops>`) — for a third, genuinely-declared provider (e.g. 'gitlab', ADR-0041, which has no
        // work-items linker of its own yet — see lib/workItemLink.js's own guard) that meant running
        // Azure DevOps's own field validation against a body that was never shaped for it, surfacing a
        // confusing "missing organization/project/parentId" 400 instead of a clear "not supported yet"
        // one. An absent `provider` still means Azure DevOps, unchanged (every caller before #14).
      ...
      if (url.pathname === '/api/identities' && req.method === 'GET') {
        ...
        if (!location) {
          ...
          // #24: a declared `provider` other than 'github' used to fall straight through to the Azure
          // DevOps branch below unconditionally — for a third, genuinely-declared provider (e.g.
          // 'gitlab', ADR-0041, which has no identity lookup of its own yet) with no
          // `organization`/`project` params either, that silently fell all the way through to this
          // route's own "first registered workspace" default instead of reporting the request's own
          // `provider` as unsupported.
          if (requestedProvider !== null && requestedProvider !== 'github' && requestedProvider !== 'azure-devops') {
