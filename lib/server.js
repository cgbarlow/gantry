...
import { GitHubAuthenticationError, GitHubRepoNotFoundError } from './githubClient.js'
...
async function collectOtherAzureDevOpsDefinitionIds({ instancesDir, pat, excludeWorkspaceId } = {}) {
  ...
  for (const workspace of workspaces) {
    try {
      const workspaceIds = await listAzureDevOpsDefinitionIds({ azureDevOps: { ...workspace, pat } })
...
export function createServer(options = {}) {
  ...
  const libraryPat = options.libraryPat !== undefined ? options.libraryPat : (process.env.GANTRY_LIBRARY_PAT ?? null)
  ...
  if (!options.skipLibraryRepoStartupRefresh) {
    ...
    if (configuredRepos.length > 0) {
      refreshAllLibraryRepos(configuredRepos, { instancesDir, pat: libraryPat }).catch((err) => {
  ...
  return createHttpServer(async (req, res) => {
    try {
      ...
      // Refresh. Uses the server's own PAT (`libraryPat`, resolved once at server startup — never a
      ...
      if (url.pathname === '/api/library-repos' && req.method === 'POST') {
        ...
        try {
          repo = addLibraryRepo(
            { organization: body?.organization, project: body?.project, repository: body?.repository, baseUrl: body?.baseUrl, codeOwner: body?.codeOwner },
            { instancesDir }
          )
        } catch (err) {
          sendJSON(res, 400, { error: err.message })
        ...
        const [refreshResult] = await refreshAllLibraryRepos([repo], { instancesDir, pat: libraryPat })
      ...
      if (url.pathname === '/api/library-repos/refresh' && req.method === 'POST') {
        const results = await refreshAllLibraryRepos(listLibraryRepos({ instancesDir }), { instancesDir, pat: libraryPat })
      ...
      if (azureDevOpsDefinitionsListMatch && req.method === 'GET') {
        ...
        const includeArchived = url.searchParams.get('archived') === '1'
        await withAzureDevOpsCredential(req, res, workspace, async (azureDevOps) => {
          const rows = await listAzureDevOpsDefinitions({ azureDevOps }, { includeArchived })
          sendJSON(res, 200, rows.map((row) => ({ ...row, home: { kind: 'azure-devops-workspace', id: workspaceId, name: workspace.repository } })))
        })
        return
      }
      ...
      // Registers a new workspace directly — the primitive the future Settings screen (#101/#104) will build on. Structural validation errors (missing location fields, an unavailable `provider` such as `atlassian`, or — for azure-devops — an unsupported `ticketingSystem` such as `jira`) are checked first and reported as 400 with no network cost, exactly like every other validating route in this file. Unlike a plain metadata store, though, this route *establishes* a remote location the same way `POST /api/instances`/`POST /api/instances/adopt` already do — so it requires the caller's own PAT (the same credential-provider seam, #86) and actually proves that PAT against this exact location via `checkAzureDevOpsRepo`/`checkGitHubRepo` (#8, docs/adr/0037/0039 — the same check `/api/instances/adopt` already performs for Azure DevOps) before persisting anything. Without this, any caller with mere network access to this server — no credential needed at all — could register a workspace (or, since a location's own fields aren't unique here, have it silently reused by a later legitimate registration for the exact same repo via `getOrCreateWorkspace`'s tuple matching) carrying a spoofed `owner`/`baseUrl` for a repo it has no real access to. `baseUrl` is gated behind the same per-provider allow flag (`allowAzureDevOpsBaseUrlOverride` / `allowGitHubBaseUrlOverride`) every other caller-supplied `baseUrl` in this file already requires, for the same SSRF-prevention reason documented on those options above.
      //
      // `provider` (#8, docs/adr/0037) is chosen first in the wizard because it decides which
      // location fields this body carries: azure-devops takes `organization`/`project`/`repository`;
      // github takes `repoOwner`/`repository` — named `repoOwner` here, not ADR-0037's literal
      // `owner`, to avoid colliding on the wire with this same body's *workspace* `owner` (the Owner
      // *person* every provider's workspace carries, unrelated to a GitHub repo's own owner/org).
      if (url.pathname === '/api/workspaces' && req.method === 'POST') {
        const body = JSON.parse(await readBody(req))
        const { organization, project, repository, repoOwner, baseUrl, owner, ticketingSystem } = body ?? {}
        ...
        const missingFields = (isGitHub ? ['repoOwner', 'repository'] : ['organization', 'project', 'repository']).filter(
          (key) => !{ organization, project, repository, repoOwner }[key]
        )
        if (missingFields.length) {
          sendJSON(res, 400, {
            error: `A workspace is missing: ${missingFields.map((key) => (key === 'repoOwner' ? 'owner' : key)).join(', ')}`,
          })
          return
        }
        ...
        const baseUrlAllowed = isGitHub ? allowGitHubBaseUrlOverride : allowAzureDevOpsBaseUrlOverride
        if (baseUrl !== undefined && !baseUrlAllowed) {
          sendJSON(res, 400, { error: `${isGitHub ? 'github' : 'azureDevOps'}.baseUrl overrides are not permitted on this server` })
          return
        }
        ...
        try {
          ...
          if (isGitHub) {
            await checkGitHubRepo({ owner: repoOwner, repository, baseUrl, pat })
          } else {
            await checkAzureDevOpsRepo({ organization, project, repository, baseUrl, pat, definitionsDir })
          }
        } catch (err) {
          if (err instanceof AzureDevOpsAuthenticationError || err instanceof GitHubAuthenticationError) {
            sendAuthenticationRequired(res)
            return
          }
          if (err instanceof AzureDevOpsRepoNotFoundError || err instanceof GitHubRepoNotFoundError) {
            sendJSON(res, 400, { error: err.message })
            return
          }
          throw err
        ...
        try {
          const location = isGitHub
            ? { provider, repoOwner, repository, baseUrl }
            : { provider, organization, project, repository, baseUrl }
          const existing = findWorkspaceByLocation(location, { instancesDir })
          const workspace = getOrCreateWorkspace({ ...location, owner, ticketingSystem }, { instancesDir })
