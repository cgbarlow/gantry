...
export function createServer(options = {}) {
  ...
  // The GitLab twin of `allowGitHubBaseUrlOverride` above (#30) — the same SSRF-prevention posture,
  // for a caller-supplied `baseUrl` on `POST /api/instance/work-items/link`'s own `gitlab` branch (a
  // self-hosted GitLab CE/EE host, ADR-0041). Left `false` by default; only tests (against the fake
  // GitLab server) opt in.
