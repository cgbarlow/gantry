...
import { loadDefinition, listDefinitions, loadDefinitionChangelog } from './definition.js'
import { readInstance, readModule, writeModule, createInstance, updateInstanceAssignee, updateInstanceRequiredReviewer, recordInstanceWorkItemLink, instanceDefinitionVersion } from './instance.js'
import { getStatus } from './status.js'
import { checkGate, resolveCheckStage } from './check.js'
import { renderArtefact, renderStageArtefacts } from './render.js'
import { findStageBranch, resolveStageBranch, stageBranchName } from './stageBranch.js'
...
import { AzureDevOpsAuthenticationError, AzureDevOpsNotFoundError, AzureDevOpsRepoNotFoundError, createAzureDevOpsClient } from './azureDevOpsClient.js'
    ...
    try {
      ...
      if (definitionVersionMatch && req.method === 'GET') {
        ...
        try {
          ...
          // Ordering rule: walk stages[].modules in order, then each artefacts[].requires moduleId, falling back to any remaining modules by id.
          const orderedIds = []
          const seen = new Set()
          ...
          // Fallback sort for any ids not yet ordered (remaining modules by id) is already by insertion above; ensure leftover sorted
          // Actually remaining modules not seen are already appended in Map iteration order; sort them by id for determinism
          ...
          const modules = finalOrder.map((mid) => {
            ...
            return { id: m.id, title: m.title, purpose: m.purpose, fields: m.fields.map((f) => ({ id: f.id, title: f.title, type: f.type, required: f.required, requiredAt: f.requiredAt, guidance: f.guidance })) }
          })
          sendJSON(res, 200, {
            id: def.id,
            title: def.title,
            ...
            version: def.version,
            status: def.status,
            stages: def.stages.map((s) => ({ id: s.id, title: s.title, purpose: s.purpose, gate: s.gate, modules: s.modules })),
            artefacts: def.artefacts.map((a) => ({ id: a.id, title: a.title, purpose: a.purpose, template: a.template, gate: a.gate, requires: a.requires })),
            modules,
          })
        } catch (err) {
          const msg = err.message ?? ''
