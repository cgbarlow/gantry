...
// found wherever its own registry entry actually says it lives. A slug with no registry entry at
// all (an unmigrated legacy bare instance, or the "Populate example text" cross-instance read
// against one) falls back to `instancesDir` itself unchanged — exactly how a bare instance has
// always been read. Never throws on an ambiguous or unknown slug; callers see whatever readInstance
// itself reports for a directory that doesn't exist, not a resolution error invented here.
  ...
  return createHttpServer(async (req, res) => {
    try {
      ...
      if (url.pathname === '/api/instance/check' && req.method === 'GET') {
        ...
        sendJSON(res, 200, checkGate(slug, { instancesDir: resolveLocalDataDir(slug, instancesDir), definitionsDir, gate }))
        return
      }
      ...
      if (url.pathname === '/api/instance/work-items/link' && req.method === 'POST') {
        ...
        const azureDevOpsLocation = resolveAzureDevOpsLocation(slug, instancesDir)
        let instanceOptions = { instancesDir: resolveLocalDataDir(slug, instancesDir) }
        let azureDevOpsBase
        if (azureDevOpsLocation) {
      ...
      if (url.pathname === '/api/instance/advance-stage' && req.method === 'POST') {
        ...
        try {
          const result = advanceStage(slug, { instancesDir: resolveLocalDataDir(slug, instancesDir), definitionsDir })
          sendJSON(res, 200, result)
        } catch (err) {
          sendJSON(res, 400, { error: err.message })
        }
      ...
      if (url.pathname === '/api/instance/synced-fields' && req.method === 'GET') {
        ...
        const resolvedSyncedFieldsDir = resolveLocalDataDir(slug, instancesDir)
      ...
      if (url.pathname === '/api/instance/synced-fields' && req.method === 'PUT') {
        ...
        try {
          sendJSON(
            res,
            200,
            await saveStageSyncedFieldOverrides(slug, { ...updates, stageId }, { instancesDir: resolveLocalDataDir(slug, instancesDir), definitionsDir })
          )
        } catch (err) {
          sendJSON(res, 400, { error: err.message })
        }
      ...
      if (url.pathname === '/api/instance' && req.method === 'GET') {
        ...
        const resolvedInstanceDir = resolveLocalDataDir(slug, instancesDir)
      ...
      if (url.pathname === '/api/instance/assignee' && req.method === 'PUT') {
        ...
        const instance = updateInstanceAssignee(slug, assignee, { instancesDir: resolveLocalDataDir(slug, instancesDir) })
        sendJSON(res, 200, { slug, assignee: instance.assignee })
        return
      }
      ...
      if (moduleMatch && req.method === 'PUT') {
        ...
        const resolvedModuleDir = resolveLocalDataDir(slug, instancesDir)
      ...
      if (renderMatch && req.method === 'POST') {
        ...
        const resolvedRenderDir = resolveLocalDataDir(slug, instancesDir)
        ...
        const result = renderArtefact(slug, artefactId, { instancesDir: resolvedRenderDir, definitionsDir, reviewSummary: localReviewSummary, format: localFormat })
        // Absolute, not relative to whatever directory `gantry serve` happened to be launched from — the browser has no way to know that directory. Exactly one of docxPath/mdPath is non-null, matching `format`.
        sendJSON(res, 200, {
          artefact: artefactId,
          format: localFormat,
          docxPath: result.docxPath ? resolve(result.docxPath) : null,
          mdPath: result.mdPath ? resolve(result.mdPath) : null,
        })
        return
      }
      ...
      if (wasmPrepareMatch && req.method === 'POST') {
        ...
        if (!azureDevOpsLocation) {
          ...
          const resolvedWasmPrepareDir = resolveLocalDataDir(slug, instancesDir)
      ...
      if (wasmFinishMatch && req.method === 'POST') {
        ...
        if (!azureDevOpsLocation) {
          ...
          const result = renderArtefact(slug, artefactId, { instancesDir: resolveLocalDataDir(slug, instancesDir), definitionsDir, dryRun: true })
      ...
      if (url.pathname === '/api/instance/assets' && req.method === 'GET') {
        ...
        let instance
        const resolvedAssetsDir = resolveLocalDataDir(slug, instancesDir)
        try {
      ...
      if (url.pathname === '/api/instance/assets' && req.method === 'POST') {
        ...
        try {
          const asset = createAsset(
            ...
            { instancesDir: resolveLocalDataDir(slug, instancesDir) }
          )
      ...
      if (assetFileMatch && req.method === 'GET') {
        ...
        try {
          const { path } = getAsset(slug, assetFileMatch[1], { instancesDir: resolveLocalDataDir(slug, instancesDir) })
          if (!existsSync(path)) {
