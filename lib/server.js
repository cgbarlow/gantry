...
import { checkAzureDevOpsRepo, checkGitHubRepo, checkGitLabRepo } from './repoCheck.js'
import {
  ...
  return createHttpServer(async (req, res) => {
    try {
      ...
      if (url.pathname === '/api/workspaces' && req.method === 'POST') {
        ...
        // (ADR-0041 — `gitlab` validates as a supported provider per lib/provider.js, but registering a
        // *new* one this way is #25's own job, not yet landed — #35 only gives GitLab an *adopt* path
        // via `POST /api/instances/adopt`/`GET /api/gitlab/repo-check` below, which re-check a location
        // the caller already knows holds data rather than proving access before first registration).
