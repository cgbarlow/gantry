...
export function createServer(options = {}) {
  ...
  if (options.migrateWorkspacesOnStart) {
    ...
    // actually registered under. `GANTRY_BOOTSTRAP_PATS` and the MCP server's `GANTRY_WORKSPACE_PATS`
    ...
    // involved: this runs before `GANTRY_BOOTSTRAP_PATS` is even read, and prints registry facts only.
    ...
    // #113 (parent #109, docs/adr/0046): `GANTRY_BOOTSTRAP_PATS` — opt-in, off by default, a
    // deliberate narrow departure from ADR-0038's "no server-side credential" recorded in docs/adr/0046
    // — a `{ workspaceId: pat }` map (same shape/validation style as `GANTRY_WORKSPACE_PATS`,
    // mcp-server/src/credentials.js's `parseWorkspacePats`) letting a bootstrap-registered workspace's
    // instances be discovered immediately at boot (`lib/workspaceBootstrap.js`'s
    // `discoverBootstrapPatInstances`, wrapping #112's `backfillProviderInstances`), so `GET
    // /api/instances` is already populated before any browser request arrives — closing the gap #112
    // alone leaves (instances reappear only on the first authenticated request), which for an
    // unattended deployment (the hosted demo) looks indistinguishable from "still broken". Parsed here
    // in the same gated block as `GANTRY_BOOTSTRAP_WORKSPACES` itself, with the same
    // options-first-env-var-fallback convention; a malformed value throws here, before this server ever
    // calls `.listen()`, exactly like a malformed `GANTRY_BOOTSTRAP_WORKSPACES` already does. Unset —
    // the default — parses to `{}`, `discoverBootstrapPatInstances` has nothing to iterate, and nothing
    // below this point runs: zero behavior change, discovery still only ever runs on #112's
    // request-credential path.
    const rawBootstrapPats = options.bootstrapPats !== undefined ? options.bootstrapPats : process.env.GANTRY_BOOTSTRAP_PATS
    const bootstrapPats = parseBootstrapPats(rawBootstrapPats)
    if (Object.keys(bootstrapPats).length > 0) {
      ...
      // listen), and a rejected/expired boot PAT degrades to "this workspace just isn't discovered at
      ...
      discoverBootstrapPatInstances(bootstrappedWorkspaces, bootstrapPats, { instancesDir }).catch((err) => {
        console.error(`lib/server.js: startup boot-PAT instance discovery failed — ${err.message}`)
      })
    }
  }
