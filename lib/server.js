...
// reason `buildModuleEntry` is. `isWorkspaceBacked` is the same "does the
// instance registry resolve this slug's storage as Azure DevOps" check
// every other single-instance route already makes (`resolveAzureDevOpsLocation`)
// — surfaced here (as `workspaceBacked`) so the web form's Stage
// advancement panel (#115) knows to hide its self-serve "Advance to next
// stage" action entirely for a Workspace-backed instance, which only ever
// advances via its own Pull Request flow instead (ADR-0014, #122-#125).
function buildInstanceResponse(slug, definitionsDir, definition, stage, instance, modules, isWorkspaceBacked) {
  return {
    ...
    modules,
    // The instance-level Azure DevOps work-item link (#103) — `null` for
    // an (the default) unlinked instance, or `{ organization, project,
    // workItemType, parentId, stages: { [stageId]: childWorkItemId } }`
    // once linked. Never includes a PAT or anything credential-shaped;
    // this is the same purely-descriptive shape `instance.yaml`'s own
    // `azureDevOps` field already follows.
 ...
 * #86/#92, plus `PUT /api/instance/assignee`, #97) resolves its own slug
 * against the shared instance registry
...
export function createServer(options = {}) {
  ...
  return createHttpServer(async (req, res) => {
    try {
      ...
      // The workspace registry (#96): every workspace gantry knows about —
      // each one an Azure DevOps organization/project/repository tuple,
      // with its own owner and ticketing-system setting. Read-only listing;
      // no PAT is required or consulted (no PAT is ever stored here — see
      // lib/workspaceRegistry.js's own doc comment).
      if (url.pathname === '/api/workspaces' && req.method === 'GET') {
        sendJSON(res, 200, listWorkspaces({ instancesDir }))
        return
      }

      // Registers a new workspace directly — the primitive the future
      // Settings screen (#101/#104) will build on. Structural validation
      // errors (missing organization/project/repository, or an unsupported
      // `ticketingSystem` such as `jira`) are checked first and reported as
      // 400 with no network cost, exactly like every other validating route
      // in this file. Unlike a plain metadata store, though, this route
      // *establishes* an Azure DevOps location the same way `POST
      // /api/instances`/`POST /api/instances/adopt` already do — so it
      // requires the caller's own PAT (the same credential-provider seam,
      // #86) and actually proves that PAT against this exact organization/
      // project/repository via `checkAzureDevOpsRepo` (the same check
      // `/api/instances/adopt` already performs) before persisting
      // anything. Without this, any caller with mere network access to
      // this server — no credential needed at all — could register a
      // workspace (or, since organization/project/repository aren't unique
      // here, have it silently reused by a later legitimate registration
      // for the exact same repo via `getOrCreateWorkspace`'s tuple
      // matching) carrying a spoofed `owner`/`baseUrl` for a repo it has no
      // real access to. `baseUrl` is gated behind the same
      // `allowAzureDevOpsBaseUrlOverride` flag every other caller-supplied
      // `baseUrl` in this file already requires, for the same SSRF-
      // prevention reason documented on that option above.
      if (url.pathname === '/api/workspaces' && req.method === 'POST') {
        ...
        try {
          // A location check, not an instance check — whether this repo
          // already holds instance data ('found') or not ('empty') is
          // irrelevant here; only a rejected PAT changes this route's
          // response.
      ...
      // Updates a workspace's `owner` and/or `ticketingSystem` — the
      // Settings screen's Workspace tab (#104): owner can be viewed/edited
      // there, and a workspace's ticketing system can be overridden away
      // from whatever the global default was at the moment the workspace
      // was created. Deliberately narrower than `registerWorkspace`'s own
      // field set — `organization`/`project`/`repository`/`baseUrl` are not
      // accepted here, since changing *those* would re-point an already-
      // established workspace at a different Azure DevOps location, which
      // (unlike a plain metadata edit) would need the same real-access
      // proof `POST /api/workspaces` requires; that's out of this ticket's
      // scope, so it's simply not an accepted field on this route rather
      // than a security hole. No PAT is required or consulted, for the
      // same reason `GET /api/workspaces` doesn't: neither `owner` nor
      // `ticketingSystem` is used to establish or prove Azure DevOps
      // access, only to record metadata this server already trusts itself
      // to serve.
      ...
      if (workspaceMatch && req.method === 'PATCH') {
        ...
        if (body && typeof body === 'object') {
          // `owner` must be a string (or omitted) — `workspaceRegistry.js`'s
          // own "optional, blank until set" convention is `''`, never
          // `null`/a number/an object; guarding the type here (rather than
          // letting a non-string value straight through to
          // `updateWorkspace`) avoids silently persisting a value that
          // diverges from that convention, e.g. an explicit `owner: null`
          // being stored as `null` instead of normalizing to `''` the way
          // an *omitted* `owner` already does.
      ...
      if (url.pathname === '/api/instances' && req.method === 'POST') {
        const body = JSON.parse(await readBody(req))
        // `owner` seeds each first-stage module file's own frontmatter
        // `owner` (the separate Design Authority sign-off convention,
        // untouched by #97); `assignee` sets the instance record's own
        // stored assignee (#97) — a caller may supply either, both, or
        // neither.
      ...
      if (url.pathname === '/api/instances/adopt' && req.method === 'POST') {
        ...
        // `'multiple'` (#100: one Azure DevOps repo can now hold more than
        // one instance under gantry-workspace/<slug>/) is reported
        // distinctly from `'empty'` — data *was* found here, it's just
        // ambiguous which instance to adopt, and this route has no slug
        // input to disambiguate with (left for the multi-instance/settings
        // UI work, #101/#104) — reporting it as "nothing to adopt" would be
        // actively misleading.
      ...
      if (url.pathname === '/api/instance/check' && req.method === 'GET') {
        ...
        // Dual local/Azure-DevOps-backed dispatch (#103) — this route
        // previously only ever checked the local `instancesDir`, silently
        // ignoring the registry entirely: an Azure-DevOps-backed instance's
        // "Check" action always evaluated a local directory that instance's
        // data never actually lived in. Fixed here as part of #103's own
        // gate-pass-then-sync flow, which needs a working check for
        // Azure-DevOps-backed instances too, following the exact same
        // resolve-then-gate pattern every other single-instance route uses.
      ...
      // Links an instance to an Azure DevOps parent work item, auto-creating
      // one child work item per stage in its definition underneath it
      // (#95/#103). `organization`/`project` name where the *work item*
      // lives — independent of, and not required to match, wherever this
      // instance's own module data is stored (local or a different Azure
      // DevOps repo entirely). `workItemType` is optional, defaulting to
      // `DEFAULT_WORK_ITEM_TYPE` ("Task") — the configurable-with-a-sensible-
      // default acceptance criterion. Requires the caller's own PAT (Work
      // Items scope), via the same credential-provider seam as every other
      // Azure-DevOps-backed route; if this instance's own data also happens
      // to be Azure-DevOps-backed, the same PAT is reused for both calls
      ...
      if (url.pathname === '/api/instance/work-items/link' && req.method === 'POST') {
        ...
        // Same SSRF guard as POST /api/instances'/adopt's own azureDevOps.baseUrl
        // (see createServer's own doc comment on allowAzureDevOpsBaseUrlOverride):
        // an unvalidated, caller-chosen baseUrl must never be honored on a real
        // deployment — only tests (against the fake Azure DevOps server) opt in.
      ...
      // The confirmed half of #95/#103's read-write sync: re-checks the
      // gate server-side (never trusting an earlier client-side check —
      // the same instance the confirmation dialog checked a moment
      // earlier may since have changed) and, only if it genuinely passes,
      // pushes a new state to that stage's linked work item. The
      // confirmation itself already happened client-side before this
      // request was ever sent — declining it means this route is simply
      // never called, leaving the work item's state genuinely untouched.
      ...
      // The self-serve "Advance to next stage" action for a *local*
      // instance (ADR-0012's local-instance mode, unaffected by ADR-0014's
      // ticketing-mode pivot; #115): moves the instance's own persisted
      // stage pointer forward by one, gated on its current stage's gate
      // having genuinely passed (re-checked server-side by `advanceStage`,
      // lib/stageAdvancement.js — never trusted from the confirm dialog's
      // own earlier client-side check, mirroring every other gated write
      // in this file). A Workspace-backed instance never reaches
      // `advanceStage` at all: it always advances via its own Pull Request
      // flow instead (ADR-0014, #122-#125), so this route rejects it
      // outright here — before ever calling `advanceStage` — rather than
      // leaving that distinction enforced only by the web form hiding the
      // button (`buildInstanceResponse`'s `workspaceBacked` flag above).
      // No PAT is required or consulted: this only ever writes to the
      // local filesystem.
      ...
      // Updates the instance record's own stored `assignee` (#97) — the
      // instance detail UI's edit affordance for it. Distinct from `PUT
      // /api/instance/modules/:id` below, which writes a *module's* own
      // frontmatter `owner`/`status` (the separate Design Authority
      // sign-off convention, untouched by #97). Resolves local vs.
      // Azure-DevOps-backed the same way every other single-instance route
      // does — per-request, via the shared instance registry.
