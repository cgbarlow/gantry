  ...
  return createHttpServer(async (req, res) => {
    try {
      ...
      if (wasmPrepareMatch && req.method === 'POST') {
        ...
        const azureDevOpsLocation = resolveAzureDevOpsLocation(slug, instancesDir)
        if (!azureDevOpsLocation) {
          sendJSON(res, 400, { error: `"${slug}" is not an Azure-DevOps-hosted instance — use /api/local/compile for a local workspace` })
          return
        }
        await withAzureDevOpsCredential(req, res, azureDevOpsLocation, async (azureDevOpsBase) => {
          const bootstrapInstance = await readInstance(slug, { azureDevOps: azureDevOpsBase })
      ...
      if (wasmFinishMatch && req.method === 'POST') {
        ...
        const azureDevOpsLocation = resolveAzureDevOpsLocation(slug, instancesDir)
        if (!azureDevOpsLocation) {
          sendJSON(res, 400, { error: `"${slug}" is not an Azure-DevOps-hosted instance` })
          return
        }
        let body
        try {
