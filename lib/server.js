...
export function createServer(options = {}) {
  ...
  return createHttpServer(async (req, res) => {
    try {
      ...
      // editor per instance) have no corresponding file under web/ — a
      // fresh navigation or reload at one of those URLs must still get the
      // app shell so the router can take over, exactly as / does. Every
      // real static asset this app serves (app.js, style.css,
      // prototypes/*.html, etc.) has a file extension; a client route
      // path never does, so that's what distinguishes the two here.
