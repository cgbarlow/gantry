  ...
  return createHttpServer(async (req, res) => {
    try {
      ...
      if (url.pathname === '/api/definitions' && req.method === 'POST') {
        ...
        // no-shadowing rule — a collision in any *other* home — is the new 409, checked only when the
        ...
        if (!definitionIdExistsIn(targetDefinitionsDir, newId)) {
          try {
            assertDefinitionIdAvailable(newId, { definitionsDir, instancesDir })
          } catch (err) {
      ...
      if (azureDevOpsDefinitionsListMatch && req.method === 'POST') {
        ...
        await withAzureDevOpsCredential(req, res, workspace, async (azureDevOps) => {
          ...
          try {
            assertDefinitionIdAvailable(newId, { definitionsDir, instancesDir, extraKnownIds: existingIds })
          } catch (err) {
