...
import { readInstance, readModule, writeModule, createInstance, importInstanceToAzureDevOps, updateInstanceAssignee, updateInstanceRequiredReviewer, recordInstanceWorkItemLink, instanceDefinitionVersion, AZURE_DEVOPS_WORKSPACE_ROOT } from './instance.js'
import { getStatus } from './status.js'
import { checkGate, resolveCheckStage } from './check.js'
import { renderArtefact, renderStageArtefacts, prepareAzureDevOpsWasmRender, finishAzureDevOpsWasmRender } from './render.js'
...
// WI #356: the concrete directory a *cross-instance* local read (the "Populate example text"
// button's source, `stage.example` — always a different slug from the instance actually being
// viewed) should use. Resolved dynamically via the registry, unlike `localInstancesDir` (which is
// always the one reserved `default` server workspace, since that's every instance this server
// itself creates today): a fresh scratch fixture whose example instance was created/migrated the
// normal way resolves to its own real server workspace; the real, not-yet-migrated (pre-#358)
// `instances/examples` at this repo's own bare root falls back to `instancesDir` itself unchanged,
// exactly matching how it's always been read, since the registry has no entry for a bare instance at
// all. Never throws on an ambiguous bare slug — an example instance failing to resolve just means no
// example text is offered, not a broken page.
function resolveLocalDataDir(slug, instancesDir) {
  let location
  try {
    location = resolveInstanceLocation(slug, { instancesDir })
  } catch {
    return instancesDir
  }
  return location?.kind === 'directory' ? join(instancesDir, location.workspace) : instancesDir
}
...
export function createServer(options = {}) {
  ...
  // the same scope a pre-#356 bare local instance always implicitly used (`LOCAL_SCOPE`). Real
  // multi-workspace *selection* over HTTP (creating/addressing an instance in some other named
  // server workspace) is not wired into this route layer yet — `lib/instanceRegistry.js` and
  // `lib/numberRegistry.js` already fully support it underneath (see their own tests), so adding it
  // here is a follow-on, additive change, not a data-model one.
  ...
  if (options.migrateWorkspacesOnStart) {
    ...
    if (!existsSync(join(instancesDir, MIGRATED_DEFAULT_WORKSPACE_FOLDER, 'workspace.json'))) {
      writeWorkspaceJson(instancesDir, MIGRATED_DEFAULT_WORKSPACE_FOLDER, {
        name: 'default',
        kind: 'local',
  ...
  return createHttpServer(async (req, res) => {
    try {
      ...
      if (url.pathname === '/api/instance/check' && req.method === 'GET') {
        ...
        sendJSON(res, 200, checkGate(slug, { instancesDir: localInstancesDir, definitionsDir, gate }))
        return
      }
      ...
      if (url.pathname === '/api/instance/work-items/link' && req.method === 'POST') {
        ...
        const azureDevOpsLocation = resolveAzureDevOpsLocation(slug, instancesDir)
        let instanceOptions = { instancesDir: localInstancesDir }
        let azureDevOpsBase
        if (azureDevOpsLocation) {
      ...
      if (url.pathname === '/api/instance/advance-stage' && req.method === 'POST') {
        ...
        try {
          const result = advanceStage(slug, { instancesDir: localInstancesDir, definitionsDir })
          sendJSON(res, 200, result)
        } catch (err) {
          sendJSON(res, 400, { error: err.message })
        }
      ...
      if (url.pathname === '/api/instance/synced-fields' && req.method === 'GET') {
        ...
        if (readInstance(slug, { instancesDir: localInstancesDir }).workItem) {
          const pat = getCredential(req)
          if (!pat) {
            sendAuthenticationRequired(res)
            return
        ...
        try {
          sendJSON(res, 200, await getStageSyncedFields(slug, { instancesDir: localInstancesDir, definitionsDir, stageId, pat: getCredential(req) }))
        } catch (err) {
          if (err instanceof AzureDevOpsAuthenticationError) {
            sendAuthenticationRequired(res)
            return
      ...
      if (url.pathname === '/api/instance/synced-fields' && req.method === 'PUT') {
        ...
        try {
          sendJSON(
            res,
            200,
            await saveStageSyncedFieldOverrides(slug, { ...updates, stageId }, { instancesDir: localInstancesDir, definitionsDir })
          )
        } catch (err) {
          sendJSON(res, 400, { error: err.message })
        }
      ...
      if (url.pathname === '/api/instance' && req.method === 'GET') {
        ...
        const instance = readInstance(slug, { instancesDir: localInstancesDir })
        const definition = loadDefinition(instance.definition, { definitionsDir, version: instanceDefinitionVersion(instance) })
        ...
        const modules = stage.modules.map((moduleId) => {
          let data = { status: 'draft', owner: '', fields: {} }
          if (existsSync(join(localInstancesDir, slug, 'modules', `${moduleId}.md`))) {
            data = readModule(definition, slug, moduleId, { instancesDir: localInstancesDir })
          }
          // The "Populate example text" button's source — a curated example instance declared per-stage
          // in the definition, not fabricated placeholder text. Deliberately always read from the raw
          // (pre-#356, not-yet-migrated) `instancesDir` root, not `localInstancesDir` — the bundled
          // `examples` fixture still lives there directly until Feature #358 moves it under a real
          // server workspace, and this lookup is a plain filesystem read that never touches the registry.
          let exampleData = null
          if (stage.example) {
            const exampleInstancesDir = resolveLocalDataDir(stage.example, instancesDir)
            if (existsSync(join(exampleInstancesDir, stage.example, 'modules', `${moduleId}.md`))) {
      ...
      if (url.pathname === '/api/instance/assignee' && req.method === 'PUT') {
        ...
        const instance = updateInstanceAssignee(slug, assignee, { instancesDir: localInstancesDir })
        sendJSON(res, 200, { slug, assignee: instance.assignee })
        return
      }
      ...
      if (moduleMatch && req.method === 'PUT') {
        ...
        const definition = loadDefinition(instance.definition, { definitionsDir, version: instanceDefinitionVersion(instance) })
        const body = JSON.parse(await readBody(req))
        writeModule(definition, slug, moduleId, body, { instancesDir: localInstancesDir })
        ...
        sendJSON(res, 200, getStatus(slug, { instancesDir: localInstancesDir, definitionsDir, stageId }))
        return
      }
      ...
      if (renderMatch && req.method === 'POST') {
        ...
        const localInstance = readInstance(slug, { instancesDir: localInstancesDir })
        ...
        const localReviewSummary = localArtefactSpec
          ...
          : undefined
        const result = renderArtefact(slug, artefactId, { instancesDir: localInstancesDir, definitionsDir, reviewSummary: localReviewSummary })
      ...
      if (wasmPrepareMatch && req.method === 'POST') {
        ...
        if (!azureDevOpsLocation) {
          ...
          const localReviewSummary = localArtefactSpec
            ? buildReviewSummaryForArtefact(localDefinition, localArtefactSpec, localInstance, null)
            : undefined
          const result = renderArtefact(slug, artefactId, { instancesDir: localInstancesDir, definitionsDir, reviewSummary: localReviewSummary, dryRun: true })
          mkdirSync(dirname(result.mdPath), { recursive: true })
          writeFileSync(result.mdPath, result.markdown)
          sendJSON(res, 200, {
            artefact: artefactId,
      ...
      if (wasmFinishMatch && req.method === 'POST') {
        ...
        if (!azureDevOpsLocation) {
          // WI #349 — local instance: land the browser-converted bytes at the .docx path the
          // native route would have written. The path is recomputed here from the artefact spec,
          // never taken from the request, so the client cannot steer the write.
          if (!body || typeof body.docxBase64 !== 'string') {
            sendJSON(res, 400, { error: 'Body must include docxBase64' })
            return
          }
          const result = renderArtefact(slug, artefactId, { instancesDir: localInstancesDir, definitionsDir, dryRun: true })
          mkdirSync(dirname(result.docxPath), { recursive: true })
          writeFileSync(result.docxPath, Buffer.from(body.docxBase64, 'base64'))
          sendJSON(res, 200, { artefact: artefactId, docxPath: resolve(result.docxPath) })
          return
        }
      ...
      if (url.pathname === '/api/instance/assets' && req.method === 'GET') {
        ...
        let instance
        try {
          instance = readInstance(slug, { instancesDir: localInstancesDir })
        } catch {
          sendJSON(res, 400, {
        ...
        const definition = loadDefinition(instance.definition, { definitionsDir, version: instanceDefinitionVersion(instance) })
        sendJSON(res, 200, listAssets(definition, slug, { instancesDir: localInstancesDir }))
        return
      }
      ...
      if (assetFileMatch && req.method === 'GET') {
        ...
        try {
          const { path } = getAsset(slug, assetFileMatch[1], { instancesDir: localInstancesDir })
          if (!existsSync(path)) {
