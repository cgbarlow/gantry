...
function buildReviewSummaryForArtefact(definition, artefact, instance, azureDevOpsLocation, commitInfo = null, provider = 'azure-devops') {
  ...
  for (const review of reviewRequests) {
    ...
    // #15: a GitHub-linked instance's review.workItemId is an issue number, reached through the web
    // (not API) root — never Azure DevOps' own `_workitems/edit` convention.
    const referenceUrl =
  ...
  return createHttpServer(async (req, res) => {
    try {
      ...
      if (url.pathname === '/api/instance/request-review' && req.method === 'POST') {
        ...
        const azureDevOpsLocation = resolveAzureDevOpsLocation(slug, instancesDir, scopeId)
        const githubLocation = resolveGitHubLocation(slug, instancesDir, scopeId)
        if (!azureDevOpsLocation && !githubLocation) {
          sendJSON(res, 400, {
        ...
        try {
          const result = await requestStageReview(slug, {
            ...
            ? { github: { ...githubLocation, pat }, definitionsDir }
            : { azureDevOps: { ...azureDevOpsLocation, pat }, definitionsDir })
          sendJSON(res, 200, result)
        } catch (err) {
          if (err instanceof AuthenticationError) {
      ...
      if (url.pathname === '/api/instance/review-status' && req.method === 'POST') {
        ...
        try {
          const result = await checkStageReviewStatus(slug, { reviewId, stageId }, githubLocation
            ? { github: { ...githubLocation, pat }, definitionsDir }
            : { azureDevOps: { ...azureDevOpsLocation, pat }, definitionsDir })
          sendJSON(res, 200, result)
        } catch (err) {
      ...
      if (url.pathname === '/api/instance/request-approval' && req.method === 'POST') {
        ...
        try {
          const result = await requestStageApproval(slug, {
            ...(azureDevOpsLocation ? { azureDevOps: { ...azureDevOpsLocation, pat } } : { github: { ...githubLocation, pat } }),
            definitionsDir,
            gate,
          })
          sendJSON(res, 200, result)
