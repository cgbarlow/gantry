...
export function createServer(options = {}) {
  ...
  return createHttpServer(async (req, res) => {
    try {
      ...
      // moduleFiles, assets? }` (+ `gate` for check, `artefact` for render) —
      ...
      // from the `/api/instance*` routes above.
      const localComputeMatch = url.pathname.match(/^\/api\/local\/(status|check|validate|render)$/)
