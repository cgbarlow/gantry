...
async function withAzureDevOpsCredential(req, res, azureDevOpsLocation, fn, { sharedPat } = {}) {
  const pat = getCredential(req) || sharedPat
  if (!pat) {
...
// error names the provider it came from"). Same #125 `options.sharedPat` fallback as
// withAzureDevOpsCredential above — see its doc comment.
async function withGitHubCredential(req, res, githubLocation, fn, { sharedPat } = {}) {
  const pat = getCredential(req) || sharedPat
  if (!pat) {
...
// workspace-definitions routes. Same #125 `options.sharedPat` fallback as withAzureDevOpsCredential
// above — see its doc comment.
async function withGitLabCredential(req, res, gitlabLocation, fn, { sharedPat } = {}) {
  const pat = getCredential(req) || sharedPat
  if (!pat) {
      ...
      if (url.pathname === '/api/instance/check' && req.method === 'GET') {
        ...
        if (azureDevOpsLocation) {
          await withAzureDevOpsCredential(
            ...
            { sharedPat: sharedReadCredential(scopeId, sharedWorkspacePats) }
          )
          return
        }
