...
import { checkWriteAccess } from './writeAccess.js'
import {
  instanceNumbersFor,
  getOrAssignWorkspaceNumber,
  getOrAssignInstanceNumber,
  scopeHasInstanceNumbers,
...
import { createSharedWorkspaceCache } from './sharedWorkspaceCache.js'
...
// #188: the reference number a new instance records in its own instance.yaml. The number registry
// lives on the server's disk, which an ephemeral host wipes on redeploy — so before handing out a
// number in a workspace this process hasn't listed yet, it lists that workspace once (which claims
// every number its existing instances already recorded), so the new one can't collide with them. A
// workspace not yet registered at all has no instances to collide with. A failed listing only costs
// that guarantee, never the create.
async function numberForNewInstance(slug, scopeId, { instancesDir, definitionsDir, pat }) {
  if (!scopeId) return 1
  if (scopeId !== LOCAL_SCOPE && !scopeHasInstanceNumbers(scopeId, { instancesDir })) {
    try {
      await listRegistry({ instancesDir, definitionsDir, pat, workspaceId: scopeId })
    } catch (err) {
      console.error(`gantry serve: couldn't list workspace ${scopeId} before numbering "${slug}" — ${err.message}`)
    }
  }
  return getOrAssignInstanceNumber(scopeId, slug, { instancesDir })
}
...
// #127 (parent #109, docs/adr/0047): computes the `sharedCache` object `withAzureDevOpsCredential`/
// `withGitHubCredential`/`withGitLabCredential` attach to `fn`'s own base location. `cacheReads` is true
// only when the request itself carried no credential of its own (`requestPat` falsy) *and* a
// `sharedPat` was actually offered to fall back to — i.e. only when the shared credential is genuinely
// what answered this particular call, never merely because the workspace happens to be shared. A write
// call site may pass `sharedCache` (for write-through — see `lib/sharedWorkspaceCache.js`'s own doc
// comment on why that's a different, looser condition than read-eligibility) while deliberately never
// passing `sharedPat`; `cacheReads` then correctly comes out `false`, exactly as if `sharedCache` had
// never been passed at all. This is the one place that decides "did *this* read actually resolve to the
// workspace's shared credential" — every module downstream (`lib/instance.js`'s `xClientFor` helpers,
// `lib/providerRegistry.js`'s `resolveContentStore`, `lib/sharedWorkspaceCache.js` itself) just forwards
// whatever this function decided, never re-derives it.
function sharedCacheContext(sharedCache, sharedPat, requestPat) {
  if (!sharedCache?.cache) return undefined
  const cacheReads = !requestPat && Boolean(sharedPat)
  return { cache: sharedCache.cache, storeKey: sharedCache.storeKey, cacheReads }
}
...
//
// #125 (parent #109, docs/adr/0047): `options.sharedPat` — omitted by every WRITE call site in this
// file, so a mutation's credential resolution is exactly `getCredential(req)` with no fallback, unchanged
// — is a READ call site's opt-in fallback to a shared workspace's own credential
// (`sharedReadCredential`, below) when the request itself carries none. With no `options.sharedPat`
// (every call site before this ticket, and every write call site after it) this function's behavior is
// byte-for-byte what it always was.
//
// #127: `options.sharedCache` (`{ cache, storeKey }` — see `sharedReadOptions`/`sharedCacheFor`
// below) rides along on the base object `fn` receives as `sharedCache: { cache, storeKey, cacheReads }`
// — `cacheReads` computed fresh by `sharedCacheContext` above from *this call's own* credential
// resolution, never carried over from the caller. A write call site may pass `sharedCache` (for
// write-through — see `lib/sharedWorkspaceCache.js`'s own doc comment on why that's a different, looser
// condition than read-eligibility) without ever passing `sharedPat`; `cacheReads` still correctly comes
// out `false` there, since `pat` can only be `requestPat` when `sharedPat` was never offered.
async function withAzureDevOpsCredential(req, res, azureDevOpsLocation, fn, { sharedPat, sharedCache } = {}) {
  const requestPat = getCredential(req)
  const pat = requestPat || sharedPat
  if (!pat) {
    sendAuthenticationRequired(res)
    return
  }
  try {
    await fn({ ...azureDevOpsLocation, pat, sharedCache: sharedCacheContext(sharedCache, sharedPat, requestPat) })
  } catch (err) {
...
// error names the provider it came from"). Same #125 `options.sharedPat` fallback, and same #127
// `options.sharedCache` handling, as withAzureDevOpsCredential above — see its doc comments.
async function withGitHubCredential(req, res, githubLocation, fn, { sharedPat, sharedCache } = {}) {
  const requestPat = getCredential(req)
  const pat = requestPat || sharedPat
  if (!pat) {
  ...
  try {
    await fn({ ...githubLocation, pat, sharedCache: sharedCacheContext(sharedCache, sharedPat, requestPat) })
  } catch (err) {
    if (err instanceof AuthenticationError) {
      sendAuthenticationRequired(res, { provider: err.provider })
      return
...
// workspace-definitions routes. Same #125 `options.sharedPat` fallback, and same #127
// `options.sharedCache` handling, as withAzureDevOpsCredential above — see its doc comments.
async function withGitLabCredential(req, res, gitlabLocation, fn, { sharedPat, sharedCache } = {}) {
  const requestPat = getCredential(req)
  const pat = requestPat || sharedPat
  if (!pat) {
  ...
  try {
    await fn({ ...gitlabLocation, pat, sharedCache: sharedCacheContext(sharedCache, sharedPat, requestPat) })
  } catch (err) {
    if (err instanceof AuthenticationError) {
      sendAuthenticationRequired(res, { provider: err.provider })
      return
...
// #125 (parent #109, docs/adr/0047): the read-path counterpart of lib/registry.js's own
// resolveRowCredential — the credential a content READ falls back to when the request itself carries
// none, for `scopeId` (an instance's own workspace id — the same id `lib/instanceRegistry.js`'s
// `listRegisteredInstances` row carries as `entry.scopeId`, and the same id `lib/registry.js`'s
// `resolveRowCredential` resolves `sharedPats` against for the listing #121 already covers). Returns
// `undefined` when this workspace has no entry in `sharedWorkspacePats` (the default — nothing set
// `GANTRY_SHARED_WORKSPACE_PATS`, or this particular workspace isn't in it) — `withAzureDevOpsCredential`/
// `withGitHubCredential`/`withGitLabCredential`'s own `pat = getCredential(req) || sharedPat` already
// treats `undefined` as "nothing to fall back to", so an unshared workspace's reads behave exactly as
// they did before this ticket. Called only from READ call sites in this file — never from a write's own
// credential resolution, which stays plain `getCredential(req)` with no fallback, the one seam that
// keeps a shared credential from ever reaching a mutation (docs/adr/0047's load-bearing rule).
function sharedReadCredential(scopeId, sharedWorkspacePats) {
  return sharedWorkspacePats[scopeId] || undefined
}

// #127 (parent #109, docs/adr/0047): `scopeId`'s own cache handle — `{ cache, storeKey }` when this
// workspace is shared (has an entry in `sharedWorkspacePats`), `undefined` otherwise. `storeKey` is the
// workspace's own registry id: unique by construction, so two different workspaces can never collide in
// `sharedWorkspaceCache` — cache keys additionally distinguish by ref/branch, entirely inside
// `lib/sharedWorkspaceCache.js` itself, via each call's own `opts.branch`. Passing this to a call site
// never grants read-caching by itself — `sharedCacheContext` (above) only turns on `cacheReads` when the
// same call also offers a `sharedPat` *and* the request had none of its own — so a write call site can
// pass this (for write-through) without ever passing a `sharedPat`, and reads through that same call
// stay live, exactly as if this had never been passed.
function sharedCacheFor(scopeId, sharedWorkspacePats, sharedWorkspaceCache) {
  return sharedWorkspacePats[scopeId] ? { cache: sharedWorkspaceCache, storeKey: scopeId } : undefined
}

// #127: the drop-in replacement for every existing `{ sharedPat: sharedReadCredential(scopeId,
// sharedWorkspacePats) }` read call site — same `sharedPat`, plus the `sharedCache` handle that makes
// that read cache-eligible once `withXCredential` confirms the shared credential is genuinely what
// answered it. `{}` (no fallback, no cache) when this workspace isn't shared — byte-for-byte the same
// "nothing to fall back to" a bare `{ sharedPat: undefined }` already produced before this ticket.
function sharedReadOptions(scopeId, sharedWorkspacePats, sharedWorkspaceCache) {
  const sharedPat = sharedReadCredential(scopeId, sharedWorkspacePats)
  if (!sharedPat) return {}
  return { sharedPat, sharedCache: sharedCacheFor(scopeId, sharedWorkspacePats, sharedWorkspaceCache) }
}

// #124: `resolveInstanceProviderLocation`'s own provider id keyed to the credential-gating helper
// above that goes with it — the genuine N-way twin of hand-chaining
// `resolveAzureDevOpsLocation`/`resolveGitHubLocation`/`resolveGitLabLocation` and picking a
// `with*Credential` call to match by hand at each site. A route that already resolves via
// `resolveInstanceProviderLocation` (or `INSTANCE_PROVIDER_LOCATION_RESOLVERS` directly) can gate its
// credential with `WITH_PROVIDER_CREDENTIAL[resolved.provider]` instead of re-deriving which helper
// goes with which provider inline.
const WITH_PROVIDER_CREDENTIAL = {
  'azure-devops': withAzureDevOpsCredential,
  github: withGitHubCredential,
  gitlab: withGitLabCredential,
}
...
//
// A Module the Stage lists in `read-only-modules` (#152, docs/adr/0050) carries `readOnly: { homeStage }`
// — the Stage it is edited at, for the editor's note — and no required Fields: the editor only asks
// for what this Stage owns. Every other Module's entry is exactly as before.
function buildModuleEntry(definition, stage, moduleId, data, exampleData) {
  const moduleSpec = definition.modules.get(moduleId)
  const readOnly = readOnlyModuleAt(definition.stages, stage.id, moduleId)
...
// #152 (docs/adr/0050): both module-write routes refuse a Module that is read-only at the Stage being
// written — the browsed `?stage=`, else the instance's current Stage, the same Stage each route
// reports status against. Checked before any branch is created or file written, so a refused save
// leaves no trace. MCP's `update_instance_modules` is a client of `PUT /api/instance/modules`, so this
// one check covers it too.
function readOnlyModulesRefusal(definition, stage, moduleIds) {
  return readOnlyWriteRefusal(definition.stages, stage, moduleIds, (id) => definition.modules.get(id)?.title ?? id)
}
...
function buildInstanceResponse(
  ...
  const { workspaceNumber, instanceNumber } = instanceNumbersFor(slug, { instancesDir, scopeId, persistedNumber: instance.number })
  ...
  return {
    ...
    // when unset — AppHeader/InstanceSwitcher/InstanceCard fall back to the slug themselves rather
    // than this route baking that fallback in.
    ...
    artefacts: definition.artefacts
      ...
      // #151: `satisfiesGate: false` only when set, so the editor can hint that this one won't pass the gate.
      .map((a) => ({ id: a.id, title: a.title, requires: a.requires, ...(a.satisfiesGate === false ? { satisfiesGate: false } : {}) })),
    modules,
...
export function createServer(options = {}) {
  // #130: the very first thing this server does — before any boot step that could throw (a malformed
  // `GANTRY_SHARED_WORKSPACE_PATS` does, below) and before it listens — is name, on stderr, every
  // `GANTRY_`-prefixed variable set in its environment that it does not read. Variable *names* only,
  // never values (several are credentials), and warning-only: an unrecognised variable never blocks
  // startup, the same never-block-startup posture as the library-repo refresh further down. A
  // correctly configured deployment prints nothing here at all. `options.env`, when supplied,
  // overrides `process.env` — the same options-first-env-var-fallback convention the settings below
  // use, so a test can exercise this without mutating the real process environment.
  warnAboutUnreadEnvVars(options.env ?? process.env)
  ...
  // map (same shape/validation style as `GANTRY_MCP_WORKSPACE_PATS`, mcp-server/src/credentials.js's
  ...
  // #127 (parent #109, docs/adr/0047 "Cache design"): one process-lifetime shared-workspace content
  // cache per running server — `options.sharedWorkspaceCache`, when supplied, overrides it, the same
  // options-first convention `sharedWorkspacePats` above uses, so a test can inject its own isolated
  // instance (to assert cache hits/misses directly) instead of reaching into this server's own private
  // one. Threaded through `sharedReadOptions`/`sharedCacheFor` below to every read/write call site that
  // opts in — never constructed more than once per server, since a fresh instance on every request would
  // cache nothing across requests, defeating the entire point.
  const sharedWorkspaceCache = options.sharedWorkspaceCache ?? createSharedWorkspaceCache()
  ...
  if (options.migrateWorkspacesOnStart) {
    ...
    // actually registered under. `GANTRY_SHARED_WORKSPACE_PATS` and the MCP server's `GANTRY_MCP_WORKSPACE_PATS`
    ...
    // #188: number every workspace now, in a fixed order, before any request can number them in
    // whatever order requests happen to arrive — see `backfillWorkspaceNumbers`'s own comment.
  ...
  return createHttpServer(async (req, res) => {
    try {
      ...
      if (url.pathname === '/api/workspaces' && req.method === 'GET') {
        ...
        // `hasRegisteredInstances` (#122, parent #109, docs/adr/0047) — a cheap, uncredentialed,
        // no-network read of the instance registry's own entry for this workspace (see
        // `workspaceHasRegisteredInstances`'s own doc comment on exactly what it can and can't prove).
        // This is what lets the dashboard's join between this route and `GET /api/instances` tell "has
        // real content, just unreadable by this request" apart from "nothing registered here yet" —
        // the "can't see in here" vs "nothing in here yet" distinction #122's own acceptance criteria
        // ask for — without this route ever needing a credential of its own.
        sendJSON(
          res,
          200,
          listWorkspaces({ instancesDir, includeArchived }).map((workspace) => ({
            ...
            hasRegisteredInstances: workspaceHasRegisteredInstances(workspace.id, { instancesDir }),
            // #187: set when the workspace has archived instances — with hasRegisteredInstances false,
            // the dashboard shows it as an ordinary empty workspace, not an unreadable one.
            hasArchivedInstances: workspaceHasArchivedInstances(workspace.id, { instancesDir }),
          }))
        )
        return
      }
      ...
      // #126 (parent #109, docs/adr/0047): "whether someone can write is established by an explicit
      // check when they enter a credential" — this is that check. Answers "can THIS request's own
      // credential push to this workspace", once, for the web client to call exactly when a credential
      // is entered and remember the answer (web/lib/credential.js), never per render and never per
      // other request. Deliberately resolved through `WITH_PROVIDER_CREDENTIAL` with no `sharedPat`
      // fallback (contrast every READ route above, which passes `sharedReadCredential`): a shared
      // workspace's own read credential answers nothing about what *this* viewer's credential can do,
      // and omitting the fallback here is what keeps that true without a second gate to remember —
      // the same "no `sharedPat` ⇒ plain `getCredential(req)`" contract `withGitHubCredential` and its
      // two siblings already have. A rejected/expired credential surfaces as the same structured
      // `authentication_required` response every other provider-backed route already gives it
      // (`AuthenticationError` caught inside `WITH_PROVIDER_CREDENTIAL[...]` itself), so the caller can
      // tell "rejected" apart from "confirmed read-only" by status/body shape alone, exactly as #126
      // requires.
      const workspaceWriteAccessMatch = url.pathname.match(/^\/api\/workspaces\/([^/]+)\/write-access$/)
      if (workspaceWriteAccessMatch && req.method === 'GET') {
        const workspaceId = decodeURIComponent(workspaceWriteAccessMatch[1])
        const workspace = resolveWorkspace(workspaceId, { instancesDir })
        if (!workspace) {
          sendJSON(res, 404, { error: `Unknown workspace "${workspaceId}"` })
          return
        }
        const withCredential = WITH_PROVIDER_CREDENTIAL[workspace.provider]
        if (!withCredential) {
          sendJSON(res, 400, { error: `Write-access check is not supported yet for provider "${workspace.provider}".` })
          return
        }
        await withCredential(req, res, workspace.location, async (location) => {
          const canWrite = await checkWriteAccess(workspace.provider, location)
          sendJSON(res, 200, { canWrite })
        })
        return
      }

      // #131 (parent #109, docs/adr/0047): one workspace's own instance listing, built with the
      // credential THIS request carries for it — the missing counterpart of `GET /api/instances`.
      //
      // Why a second listing route rather than a flag on the first: `GET /api/instances` spans every
      // workspace at once, so the browser has no single workspace whose credential it could attach
      // (docs/adr/0038 removed the global default deliberately — a credential's blast radius is one
      // workspace) and attaches none. The consequence, live at 0.8.0-beta, is that a credential
      // entered in the browser can never populate or show a Provider-backed workspace at all:
      // uncredentialed, the server skips instance discovery (it needs a credential to attempt) *and*
      // omits every Provider-backed row (it needs one to read). Only a deployment-held shared
      // credential (#121) could do either. Naming the workspace in the URL is what gives the browser
      // something to attach its own stored credential to, and what keeps that credential scoped to the
      // one workspace it was entered for — `listRegistry`'s own `options.workspaceId` filters both the
      // discovery attempt (`backfillProviderInstances`'s existing per-workspace scoping) and the rows
      // built, so workspace A's credential is never attempted against workspace B's repo.
      //
      // Discovery here is the same idempotent #112 pass `GET /api/instances` already runs for a
      // request that does carry a credential — it registers instance entries in this server's own
      // registry and writes nothing whatsoever to the Provider's repo, and it no-ops entirely once
      // that workspace has at least one registered instance. A GET, not a POST, for exactly that
      // reason: from the caller's side this is a read of a workspace's designs; the registry entries
      // are a cache of what the read found.
      //
      // No `sharedPat` fallback (contrast `GET /api/instances`, which has one): this route answers for
      // the viewer's own credential, and a deployment that does configure a shared credential already
      // gets that workspace's rows from the unscoped listing — so this path is purely additive and
      // changes nothing for it. `surfaceAuthenticationErrors` makes a rejected credential come back as
      // the same structured `authentication_required` response every other provider-backed route gives
      // it, rather than being swallowed into an empty listing indistinguishable from "this workspace
      // is genuinely empty".
      const workspaceInstancesMatch = url.pathname.match(/^\/api\/workspaces\/([^/]+)\/instances$/)
      if (workspaceInstancesMatch && req.method === 'GET') {
        const workspaceId = decodeURIComponent(workspaceInstancesMatch[1])
        const workspace = resolveWorkspace(workspaceId, { instancesDir })
        if (!workspace) {
          sendJSON(res, 404, { error: `Unknown workspace "${workspaceId}"` })
          return
        ...
        const withCredential = WITH_PROVIDER_CREDENTIAL[workspace.provider]
        if (!withCredential) {
          // No `WITH_PROVIDER_CREDENTIAL` entry means no instance-registry entry kind either
          // (`lib/instanceRegistry.js`'s `PROVIDER_ENTRY_KIND` — atlassian, today), so there is
          // nothing here to discover or list; said plainly rather than answered with an empty listing.
          sendJSON(res, 400, { error: `Listing a workspace's own instances is not supported yet for provider "${workspace.provider}".` })
          return
        }
        await withCredential(req, res, workspace.location, async ({ pat }) => {
          try {
            sendJSON(
              res,
              200,
              await listRegistry({ instancesDir, definitionsDir, pat, workspaceId, surfaceAuthenticationErrors: true })
            )
          } catch (err) {
            if (err instanceof AuthenticationError) {
              // `credentialRejected` (unlike `withCredential`'s own generic catch) because reaching
              // here proves a credential was supplied and the Provider itself turned it down — the
              // distinction web/lib/credential.js's `markCredentialRejected` is driven off.
              sendAuthenticationRequired(res, {
                credentialRejected: true,
                operation: 'listing this workspace’s designs',
      ...
      if (url.pathname === '/api/instances' && req.method === 'POST') {
        ...
        if (azureDevOps !== undefined) {
          ...
          const number = await numberForNewInstance(slug, findWorkspaceByLocation({ provider: 'azure-devops', location }, { instancesDir })?.id, { instancesDir, definitionsDir, pat })
          try {
            await createInstance(definitionId, slug, { definitionsDir, owner, assignee, definitionVersion: definitionVersionToUse, number, azureDevOps: { ...location, pat } })
          } catch (err) {
            if (err instanceof AuthenticationError) {
              sendAuthenticationRequired(res, { provider: err.provider })
              return
        ...
        if (github !== undefined) {
          ...
          const number = await numberForNewInstance(slug, findWorkspaceByLocation({ provider: 'github', location }, { instancesDir })?.id, { instancesDir, definitionsDir, pat })
          try {
            await createInstance(definitionId, slug, { definitionsDir, owner, assignee, definitionVersion: definitionVersionToUse, number, github: { ...location, pat } })
          } catch (err) {
            if (err instanceof AuthenticationError) {
              sendAuthenticationRequired(res, { provider: err.provider })
              return
        ...
        const number = await numberForNewInstance(slug, LOCAL_SCOPE, { instancesDir, definitionsDir })
        try {
          createInstance(definitionId, slug, { instancesDir: localInstancesDir, definitionsDir: instanceCreationDefinitionsDir, owner, assignee, definitionVersion: definitionVersionToUse, number })
        } catch (err) {
          const status = /already exists/.test(err.message) ? 409 : 400
          sendJSON(res, status, { error: err.message })
          return
      ...
      if (url.pathname === '/api/instance/check' && req.method === 'GET') {
        ...
        // #125 (parent #109, docs/adr/0047): a stage's gate evaluation is a read — no shared workspace
        // is ever mutated by a "Check" click — so every branch below passes `sharedReadCredential` as
        // this route's own opt-in fallback to a shared workspace's credential when the request carries
        // none of its own.
        ...
        if (azureDevOpsLocation) {
          await withAzureDevOpsCredential(
            ...
            sharedReadOptions(scopeId, sharedWorkspacePats, sharedWorkspaceCache)
          )
          return
        }

        // The GitHub twin of the Azure-DevOps-backed branch above (#14). #188: this used to read
        // straight off `main`, written before GitHub stage branches existed (#12) — so a stage's saved
        // edits, which live on its own branch until its Pull Request merges, always checked as
        // outstanding. Same read-only branch resolution as the Azure DevOps branch above.
        ...
        if (githubLocation) {
          await withGitHubCredential(
            ...
            async (githubBase) => {
              ...
              sendJSON(res, 200, await checkGate(slug, { github, definitionsDir, gate }))
            },
            sharedReadOptions(scopeId, sharedWorkspacePats, sharedWorkspaceCache)
          )
          return
        ...
        // #124: the GitLab twin of the GitHub branch above — this route used to test only for Azure
        // DevOps and fall straight through to a *local* check for a GitLab-backed instance (`checkGate`
        // itself has supported `options.gitlab` since #33; only this route's own dispatch never grew a
        // branch for it).
        const gitlabLocation = resolveGitLabLocation(slug, instancesDir, scopeId)
        if (gitlabLocation) {
          await withGitLabCredential(
            req,
            res,
            gitlabLocation,
            async (gitlabBase) => {
              // #188: the same stage-branch resolution as the GitHub branch above.
              ...
              sendJSON(res, 200, await checkGate(slug, { gitlab, definitionsDir, gate }))
            },
            sharedReadOptions(scopeId, sharedWorkspacePats, sharedWorkspaceCache)
          )
          return
      ...
      if (url.pathname === '/api/instance/work-items/link' && req.method === 'POST') {
        ...
        if (body?.provider === 'gitlab') {
          ...
          // #124: this instance's own *data* storage — independent of `namespace`/`repository` above
          // (the GitLab *issue*'s own location) — used to always resolve locally: `resolveGitLabLocation`
          // didn't exist when this branch was first written (its own comment said so verbatim), but has
          // existed for a while now, so a GitLab-backed instance linking to a GitLab issue was silently
          // writing the link onto a local directory its data never actually lived in, exactly like the
          // `github` branch above already correctly avoids via `resolveGitHubLocation`.
          const gitlabInstanceLocationForLink = resolveGitLabLocation(slug, instancesDir, scopeId)
          const instanceOptions = gitlabInstanceLocationForLink
            ? { gitlab: { ...gitlabInstanceLocationForLink, pat } }
            : { instancesDir: resolveLocalDataDir(slug, instancesDir, scopeId) }
      ...
      if (url.pathname === '/api/instance/work-items/tag' && req.method === 'POST') {
        ...
        // #124: genuine N-way dispatch for this instance's own *data* storage — previously
        // Azure-DevOps-or-local only, so a GitHub- or GitLab-backed instance's tag repair always read
        // `instance.workItem` off a local directory its data never actually lived in (the tag operation
        // itself stays Azure-DevOps-only downstream — `tagLinkedWorkItems` throws its own clear error for
        // a work item that isn't Azure-DevOps-linked — this only fixes where the *instance record* is
        // read from).
        const resolved = resolveInstanceProviderLocation(slug, instancesDir, scopeId)
        let instanceOptions = { instancesDir: resolveLocalDataDir(slug, instancesDir, scopeId) }
        if (resolved) {
          const base = { ...resolved.location, pat }
          const findBranch =
            resolved.provider === 'azure-devops' ? findStageBranch : resolved.provider === 'github' ? findGitHubStageBranch : findGitLabStageBranch
          try {
            const bootstrapInstance = await readInstance(slug, { [PROVIDER_OPTION_KEY[resolved.provider]]: base })
            const branch = await findBranch(base, slug, bootstrapInstance.stage)
            instanceOptions = { [PROVIDER_OPTION_KEY[resolved.provider]]: branch ? { ...base, branch } : base }
          } catch (err) {
            if (err instanceof AuthenticationError) {
              sendAuthenticationRequired(res, { provider: err.provider })
              return
      ...
      if (url.pathname === '/api/instance/work-items/sync' && req.method === 'POST') {
        ...
        if (resolved?.provider === 'azure-devops') {
          ...
          try {
            ...
            const definition = loadDefinition(bootstrapInstance.definition, { definitionsDir: definitionsDirForScope(definitionsDir, instancesDir, slug, scopeId, bootstrapInstance.definition), version: instanceDefinitionVersion(bootstrapInstance) })
            const stage = resolveCheckStage(definition, bootstrapInstance, slug, { gate })
            // Read-only, like GET /api/instance/check above (#142 review): this route only ever reads
            // that stage's content to re-check its gate and to look up its linked work item — it pushes
            // to the *work item*, never to git — so it must never itself start a stage's branch
            // lifecycle. resolveStageBranch would happily recreate an arbitrary completed stage's
            // already-merged-and-deleted branch from a stale `main` (reachable here via `gate`, e.g. the
            // MCP `sync_work_item` tool's own `gate` parameter), which is exactly the "Re-open then
            // 'branch already exists'" hazard #142 closed for module saves. findStageBranch only reports
            // a branch that already exists, falling back to 'main' otherwise.
            const branch = await findStageBranch(azureDevOpsBase, slug, stage.id)
      ...
      if (url.pathname === '/api/instance/advance-stage' && req.method === 'POST') {
        ...
        // #124: `advanceStage` itself is local-filesystem-only (no provider dispatch of its own) — this
        // guard is what stops it from ever running against a Provider-backed slug's `instancesDir`
        // resolution at all. Previously tested only for Azure DevOps, so a GitHub- or GitLab-backed
        // instance (which *also* only ever advances via its own Pull Request flow, ADR-0040/0041's
        // "full capability parity") fell straight through to this self-serve local write instead of
      ...
      if (url.pathname === '/api/instance/commits' && req.method === 'GET') {
        ...
        await withAzureDevOpsCredential(
          ...
          // #125 (parent #109, docs/adr/0047): a stage's commit history is a read — this route never
          // mutates the repo — so it falls back to a shared workspace's own credential the same way
          // GET /api/instance/check above does.
          sharedReadOptions(scopeId, sharedWorkspacePats, sharedWorkspaceCache)
        )
        return
      }
      ...
      if (url.pathname === '/api/instance/synced-fields' && req.method === 'GET') {
        ...
        // #124: genuine N-way dispatch — the same `resolveInstanceProviderLocation` every other
        // corrected route uses — instead of testing only for Azure DevOps and falling through to a
        // *local* read for anything else (silently reading a same-slug local design, or erroring on a
        // perfectly healthy one).
        const resolved = resolveInstanceProviderLocation(slug, instancesDir, scopeId)
        if (resolved) {
          // #125 (parent #109, docs/adr/0047): read-only (this is the GET route; its PUT sibling below
          // deliberately does NOT pass `sharedPat` — a save always resolves the viewer's own credential
          // or does not proceed) — falls back to this instance's own workspace's shared credential when
          // the request carries none of its own.
          await WITH_PROVIDER_CREDENTIAL[resolved.provider](
            req,
            res,
            resolved.location,
            async (base) => {
              sendJSON(res, 200, await getStageSyncedFields(slug, { [PROVIDER_OPTION_KEY[resolved.provider]]: base, definitionsDir, stageId }))
            },
            sharedReadOptions(scopeId, sharedWorkspacePats, sharedWorkspaceCache)
          )
          return
        ...
        // Neither a recognized provider nor a local (directory) entry — a genuinely orphaned/malformed
        // registry entry (#124's acceptance criterion: fail clearly here rather than silently resolving
        // to a local read). An entirely unregistered slug (`rawLocation` undefined) is *not* this case —
        // that's the ordinary "never seen this slug" path, left to resolve locally exactly as before,
        // surfacing readInstance's own "no instance" error.
        const rawLocation = resolveInstanceLocation(slug, { instancesDir, scopeId })
        if (rawLocation && rawLocation.kind !== 'directory') {
          sendJSON(res, 400, {
            error: `Instance "${slug}" has a registry entry this server doesn't know how to resolve (kind "${rawLocation.kind}") — neither a known Workspace provider nor a local instance.`,
          })
          return
        }
      ...
      if (url.pathname === '/api/instance/synced-fields' && req.method === 'PUT') {
        ...
        // #124: same genuine N-way dispatch as the GET route above, instead of Azure-DevOps-or-local.
        const resolved = resolveInstanceProviderLocation(slug, instancesDir, scopeId)
        if (resolved) {
          await WITH_PROVIDER_CREDENTIAL[resolved.provider](req, res, resolved.location, async (base) => {
            sendJSON(
              res,
              200,
              await saveStageSyncedFieldOverrides(slug, { ...updates, stageId }, { [PROVIDER_OPTION_KEY[resolved.provider]]: base, definitionsDir })
            )
          })
          return
        }

        // Same "genuinely orphaned/malformed registry entry" guard as the GET route above.
        const rawLocation = resolveInstanceLocation(slug, { instancesDir, scopeId })
        if (rawLocation && rawLocation.kind !== 'directory') {
          sendJSON(res, 400, {
            error: `Instance "${slug}" has a registry entry this server doesn't know how to resolve (kind "${rawLocation.kind}") — neither a known Workspace provider nor a local instance.`,
      ...
      if (url.pathname === '/api/instance' && req.method === 'GET') {
        ...
        // #125 (parent #109, docs/adr/0047): opening a design is the core read this ticket exists for
        // — every branch below falls back to a shared workspace's own credential (`sharedReadCredential`)
        // when the request carries none of its own.
        const azureDevOpsLocation = resolveAzureDevOpsLocation(slug, instancesDir, scopeId)
        if (azureDevOpsLocation) {
          await withAzureDevOpsCredential(req, res, azureDevOpsLocation, async (azureDevOpsBase) => {
            ...
            const completed = isCompletedStage(definition.stages, stage.id, bootstrapInstance.stage)
            ...
            // #142: captured alongside the loop below so a completed Stage's own crossStageEdit check
            // (further down) can compare against exactly the main content this response already read,
            // rather than reading each shared Module's main content a second time.
            const mainDataByModuleId = new Map()
            const modules = await Promise.all(
              stage.modules.map(async (moduleId) => {
                ...
                mainDataByModuleId.set(moduleId, data)
            ...
            // #142: when this Stage is completed, whether the instance's real current Stage has its
            // own unmerged changes to a Module the two Stages both mount — named so the completed-
            // Stage banner can point an author at where those changes actually live, closing the
            // HLD/SOAP mix-up the issue describes (a shared Module edited on a later Stage's branch
            // looks, from a completed earlier Stage's own main-only screen, exactly like it was never
            // touched at all).
            let crossStageEdit = null
            if (completed) {
              const currentBranchName = await findStageBranch(azureDevOpsBase, slug, bootstrapInstance.stage)
              if (currentBranchName) {
                const currentStage = definition.stages.find((s) => s.id === bootstrapInstance.stage)
                const sharedModuleIds = stage.modules.filter((id) => currentStage?.modules.includes(id))
                for (const moduleId of sharedModuleIds) {
                  let currentStageData
                  try {
                    currentStageData = await readModule(definition, slug, moduleId, { azureDevOps: { ...azureDevOpsBase, branch: currentBranchName } })
                  } catch (err) {
                    if (err instanceof AuthenticationError) throw err
                    if (/has no saved data/.test(err.message)) continue
                    throw err
                  }
                  const mainData = mainDataByModuleId.get(moduleId)
                  if (JSON.stringify(currentStageData.fields) !== JSON.stringify(mainData?.fields ?? {})) {
                    crossStageEdit = { id: currentStage.id, title: currentStage.title }
                    break
                  }
                }
              }
            ...
            sendJSON(res, 200, {
              ...
              stageSync,
              // #142: the completed-stage label's own two facts (Workspace-backed only — a local
              // instance has no main/branch split to complete against, see web/lib/stageCompletion.js).
              stageCompleted: completed,
              crossStageEdit,
              // #126 (parent #109, docs/adr/0047): "shared" — this instance's own workspace has a
              // `GANTRY_SHARED_WORKSPACE_PATS` entry, i.e. it can be (and, since this response reached
              // here, may well have been) read with no credential at all. The web client uses this,
              // together with whether it holds a credential for this workspace, to decide whether the
              // new read-only-until-write-access-confirmed gating applies at all — an unshared
              // workspace's existing PAT-prompt-on-401 behaviour is unaffected by this ticket, and this
              // flag is what keeps that true without the client having to guess.
              shared: Boolean(sharedWorkspacePats[scopeId]),
            })
          }, sharedReadOptions(scopeId, sharedWorkspacePats, sharedWorkspaceCache))
          return
        }
        ...
        if (githubLocation) {
          await withGitHubCredential(req, res, githubLocation, async (githubBase) => {
            ...
            // #142: same "keep the already-fetched main content" capture as the Azure-DevOps-backed
            // branch above.
            const mainDataByModuleId = new Map()
            const modules = await Promise.all(
              stage.modules.map(async (moduleId) => {
            ...
            // #142: same cross-Stage-edit detection as the Azure-DevOps-backed branch above.
            let crossStageEdit = null
            if (completed) {
              const currentBranchName = await findGitHubStageBranch(githubBase, slug, bootstrapInstance.stage)
              if (currentBranchName) {
                const currentStage = definition.stages.find((s) => s.id === bootstrapInstance.stage)
                const sharedModuleIds = stage.modules.filter((id) => currentStage?.modules.includes(id))
                for (const moduleId of sharedModuleIds) {
                  let currentStageData
                  try {
                    currentStageData = await readModule(definition, slug, moduleId, { github: { ...githubBase, branch: currentBranchName } })
                  } catch (err) {
                    if (err instanceof AuthenticationError) throw err
                    if (/has no saved data/.test(err.message)) continue
                    throw err
            ...
            sendJSON(res, 200, {
              ...
              stageSync: { behind: false, behindFiles: [], ahead: false },
              // #142 — see the Azure-DevOps-backed branch above for what these are and why.
              stageCompleted: completed,
              crossStageEdit,
              // #126 — see the Azure-DevOps-backed branch above for what this flag is and why.
              shared: Boolean(sharedWorkspacePats[scopeId]),
            })
          }, sharedReadOptions(scopeId, sharedWorkspacePats, sharedWorkspaceCache))
          return
        ...
        if (gitlabInstanceLocation) {
          await withGitLabCredential(req, res, gitlabInstanceLocation, async (gitlabBase) => {
            ...
            // #142: same cross-Stage-edit detection as the Azure-DevOps-backed branch above. GitLab
            // has no module-write route yet (#124), but that only means this can never fire from a
            // GitLab-backed instance's own saves — reading it, at least, is still worth agreeing with
            // every other Workspace-backed provider on.
            let crossStageEdit = null
            if (completed) {
              const currentBranchName = await findGitLabStageBranch(gitlabBase, slug, bootstrapInstance.stage)
              if (currentBranchName) {
                const currentStage = definition.stages.find((s) => s.id === bootstrapInstance.stage)
                const sharedModuleIds = stage.modules.filter((id) => currentStage?.modules.includes(id))
                for (const moduleId of sharedModuleIds) {
                  let currentStageData
                  try {
                    currentStageData = await readModule(definition, slug, moduleId, { gitlab: { ...gitlabBase, branch: currentBranchName } })
                  } catch (err) {
                    if (err instanceof AuthenticationError) throw err
                    if (/has no saved data/.test(err.message)) continue
                    throw err
        ...
        sendJSON(res, 200, {
          ...
          archived: isInstanceArchived(slug, { instancesDir, scopeId }),
          stageSync: { behind: false, behindFiles: [], ahead: false },
          // #126 — a local (directory-backed) instance has no Provider credential of its own and no
          // write-access-confirmation concept — always editable (subject only to `archived` above),
          // exactly as before this ticket. `false`, never derived from `sharedWorkspacePats` (which is
          // keyed by Provider-backed workspace id and never has an entry for a local instance anyway).
          shared: false,
        })
        return
      }
      ...
      if (url.pathname === '/api/instance/stage-branch/sync' && req.method === 'POST') {
        ...
        await withAzureDevOpsCredential(req, res, azureDevOpsLocation, async (azureDevOpsBase) => {
          ...
          if (isCompletedStage(definition.stages, stage.id, bootstrapInstance.stage)) {
      ...
      if (url.pathname === '/api/instance/assignee' && req.method === 'PUT') {
        ...
        // #124: genuine N-way dispatch — previously Azure-DevOps-or-local only, so updating a GitHub-
        // or GitLab-backed instance's assignee silently wrote the change onto a local directory that
        // instance's data never actually lived in.
        const resolved = resolveInstanceProviderLocation(slug, instancesDir, scopeId)
        if (resolved) {
          await WITH_PROVIDER_CREDENTIAL[resolved.provider](req, res, resolved.location, async (base) => {
            ...
            const resolveBranch =
              resolved.provider === 'azure-devops' ? resolveStageBranch : resolved.provider === 'github' ? resolveGitHubStageBranch : resolveGitLabStageBranch
            const branch = await resolveBranch(base, definition, slug, stage.id)
            const instance = await updateInstanceAssignee(slug, assignee, { [PROVIDER_OPTION_KEY[resolved.provider]]: { ...base, branch } })
      ...
      if (url.pathname === '/api/instance/modules' && req.method === 'PUT') {
        ...
        if (azureDevOpsLocation) {
          await withAzureDevOpsCredential(req, res, azureDevOpsLocation, async (azureDevOpsBase) => {
            ...
            const readOnlyRefusal = readOnlyModulesRefusal(definition, stage, moduleIds)
            if (readOnlyRefusal) {
              sendJSON(res, 400, { error: readOnlyRefusal })
              return
            }
            // #142: refused before resolveStageBranch runs, so a save aimed at a completed Stage can
            // never recreate (or stack onto) its already-merged-and-cleaned-up branch — Re-open
            // (docs/adr/0026) is the only path back to editing it.
            const completedRefusal = completedStageRefusal(definition.stages, stage, bootstrapInstance.stage)
            if (completedRefusal) {
              sendJSON(res, 400, { error: completedRefusal })
              return
            }
            const branch = await resolveStageBranch(azureDevOpsBase, definition, slug, stage.id)
        ...
        if (githubLocation) {
          await withGitHubCredential(req, res, githubLocation, async (githubBase) => {
            ...
            // #142: same completed-Stage refusal as the Azure-DevOps-backed branch above, checked
            // before resolveGitHubStageBranch runs for the same reason.
            const completedRefusal = completedStageRefusal(definition.stages, stage, bootstrapInstance.stage)
            if (completedRefusal) {
              sendJSON(res, 400, { error: completedRefusal })
              return
        ...
        // #124: `writeModules`/`writeModule` (lib/instance.js) have no GitLab-write branch at all yet
        // (module content GitLab *reads* work — `readModuleFromGitLab` — but saving one doesn't); this
        // route used to silently fall through such a save onto a local directory the GitLab-backed
        // instance's data never actually lived in. Failing clearly here, rather than adding a
        // half-built write path, until that capability lands.
        if (resolveGitLabLocation(slug, instancesDir, scopeId)) {
          sendJSON(res, 400, { error: `Instance "${slug}" is GitLab-backed — saving modules is not supported for GitLab-backed instances yet.` })
          return
        }
        ...
        const stage = definition.stages.find((s) => s.id === stageId)
        // An unknown `?stage=` is refused before writing, not after — otherwise it would skip the
        // read-only check below and write the Module before status reporting failed.
        if (!stage) {
          sendJSON(res, 400, { error: `Definition "${definition.id}" has no stage "${stageId}"` })
          return
        }
      ...
      if (moduleMatch && req.method === 'PUT') {
        ...
        if (azureDevOpsLocation) {
          await withAzureDevOpsCredential(req, res, azureDevOpsLocation, async (azureDevOpsBase) => {
            ...
            const readOnlyRefusal = readOnlyModulesRefusal(definition, stage, [moduleId])
            if (readOnlyRefusal) {
              sendJSON(res, 400, { error: readOnlyRefusal })
              return
            }
            // #142: refused before resolveStageBranch runs — see the bulk PUT /api/instance/modules
            // route above for why.
            const completedRefusal = completedStageRefusal(definition.stages, stage, bootstrapInstance.stage)
            if (completedRefusal) {
              sendJSON(res, 400, { error: completedRefusal })
            ...
            // #127 (parent #109, docs/adr/0047): write-through — this instance's *own* copy of the
            // module the writeModule call above already updated via azureDevOps's own sharedCache (see
            // that write flowing through lib/instance.js's azureDevOpsClientFor -> resolveContentStore
            // -> lib/sharedWorkspaceCache.js's recordWrite) is now current, so the very next read of this
            // module sees it immediately, no freshness-check delay. Only takes effect when this workspace
            // is actually shared (`sharedCache` below is `undefined` otherwise) — never on an unshared
            // workspace's save, which behaves byte-for-byte as before this ticket.
          }, { sharedCache: sharedCacheFor(scopeId, sharedWorkspacePats, sharedWorkspaceCache) })
          return
        }
        ...
        if (githubLocation) {
          await withGitHubCredential(req, res, githubLocation, async (githubBase) => {
            ...
            // #142: same completed-Stage refusal as the Azure-DevOps-backed branch above.
            const completedRefusal = completedStageRefusal(definition.stages, stage, bootstrapInstance.stage)
            if (completedRefusal) {
              sendJSON(res, 400, { error: completedRefusal })
              return
            ...
            // #127: same write-through as the Azure-DevOps-backed branch above.
          }, { sharedCache: sharedCacheFor(scopeId, sharedWorkspacePats, sharedWorkspaceCache) })
          return
        }

        // #124: same "no GitLab-write path yet" guard as PUT /api/instance/modules above — fail clearly
        // rather than silently falling through to a local write.
        if (resolveGitLabLocation(slug, instancesDir, scopeId)) {
          sendJSON(res, 400, { error: `Instance "${slug}" is GitLab-backed — saving modules is not supported for GitLab-backed instances yet.` })
          return
        }
      ...
      if (url.pathname === '/api/instance/assets' && req.method === 'GET') {
        ...
        // #125 (parent #109, docs/adr/0047): listing an instance's assets is a read — every branch
        // below falls back to a shared workspace's own credential when the request carries none of its
        // own.
        ...
        if (azureLoc) {
          await withAzureDevOpsCredential(req, res, azureLoc, async (azureDevOpsBase) => {
            ...
            const azureDevOps = branch ? { ...azureDevOpsBase, branch } : azureDevOpsBase
            // #127: resolveContentStore (not createAzureDevOpsClient directly) so this read — like its
            // GitHub/GitLab siblings just below, which already went through resolveContentStore — picks
            // up azureDevOps.sharedCache when this is a shared-credential read of a shared workspace.
            const client = resolveContentStore('azure-devops', azureDevOps)
      ...
      if (assetFileMatch && req.method === 'GET') {
        ...
        // #125 (parent #109, docs/adr/0047): serving an asset (an image, most often) is a read — every
        // branch below falls back to a shared workspace's own credential when the request carries none
        // of its own.
        const azureLoc = resolveAzureDevOpsLocation(slug, instancesDir, scopeId)
        if (azureLoc) {
          ...
          await withAzureDevOpsCredential(req, res, azureLoc, async (azureDevOpsBase) => {
            ...
            const azureDevOps = branch ? { ...azureDevOpsBase, branch } : azureDevOpsBase
            // #127: resolveContentStore (not createAzureDevOpsClient directly), same reason as the
            // asset-listing route above — picks up azureDevOps.sharedCache for a shared-credential read.
            const client = resolveContentStore('azure-devops', azureDevOps)
            ...
            try {
              ...
              if (err instanceof NotFoundError) {
                ...
                if (azureDevOps.branch && azureDevOps.branch !== 'main') {
                  try {
                    const mainClient = resolveContentStore('azure-devops', { ...azureDevOpsBase })
      ...
      // Anything under /api/ that reached this point matched none of the routes above — an
      // unknown path, or a known one called with a method it doesn't handle. Without this guard
      // it would fall into the extension-less SPA fallback just below and come back as a 200
      // `index.html`, which is worse than a plain 404: a caller (a script, an MCP client) reads
      // that as success (#141). Every real API route replies for itself before this point, so
      // this is reached only by requests none of them claimed.
      if (url.pathname === '/api' || url.pathname.startsWith('/api/')) {
        sendJSON(res, 404, { error: `No API route matches ${req.method} ${url.pathname}` })
        return
      }
