 ...
 *
 * Cross-provider (#20, docs/adr/0037/0039): a fan-out's repos need not share a provider — each
 * resolves its own content-store/pull-requests/identity capability from `repo.provider`, and each is
 * credentialed independently (see `pat`/`pats` below). "Source" and "target" are independent
 ...
 * already true by construction once GitHub registers these same three capabilities.
 *
...
// A provider-appropriate web URL for a promotion Pull Request (#20) — Azure DevOps's own
// `_git/.../pullrequest/<id>` shape, or GitHub's `/pull/<id>`. GitHub's REST `baseUrl` for an
// Enterprise Server host is `<host>/api/v3` (its own documented convention); the equivalent web UI is
// the bare host, so that suffix is stripped here rather than linking into the API host itself.
...
// scale vs. GitHub's APPROVED/CHANGES_REQUESTED/COMMENTED/DISMISSED, docs/adr/0040) — each provider's
// own pull-requests client returns whichever raw shape its API produces (`reviewers` vs. `reviews`),
// so this is the one place that picks the right interpreter for it.
function interpretReviewState(provider, pr) {
  return provider === 'github' ? interpretGitHubReviews(pr.reviews) : interpretReviewerVotes(pr.reviewers)
}
...
export async function promoteDefinitionVersionToRepo({ definitionId, version, files, repo, pat }) {
  ...
  if (repo.codeOwner && repo.codeOwner.trim()) {
    const identityClient = resolveIdentity(repo.provider, { ...repo.location, pat })
