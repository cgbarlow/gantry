 ...
 * `lib/provider.js`, and `lib/workspaceRegistry.js`'s identical treatment (Azure DevOps keeps
 * `{ organization, project, repository, baseUrl? }`, GitHub takes `{ owner, repository, baseUrl? }`).
 * A pre-#3 flat record (`{ organization, project, repository, baseUrl?, codeOwner?, addedAt }`, no
 * `provider`/`location` keys of its own) is read forward as `provider: 'azure-devops'` with those
 * fields lifted into `location`, the same auto-backfill-on-read convention every registry here
 * already follows (`lib/instanceRegistry.js`'s `migrateLegacyFlatShape`). No one-shot boot
 * migration — an interrupted upgrade leaves a tolerant reader, not a half-written file.
 *
 * **Flat location accessors removed (ticket #6):** every function now takes and returns the nested
 * `{ provider, location, codeOwner? }` shape only — no flat `organization`/`project`/`repository`/
 * `baseUrl` acceptance or denormalization. The flat shape survives only on disk, for a pre-#3 record
 * `loadWithBackfill` hasn't rewritten yet.
 *
...
// Normalizes input into the canonical `{ provider, location, codeOwner }` this module stores.
// Nested-only (ticket #6) — a caller passes `{ provider?, location, codeOwner? }`; there is no flat
// `{ organization, project, repository, baseUrl?, codeOwner? }` fallback left to detect.
...
// The public record: the nested `provider`/`location` shape only (ticket #6 — no
// `organization`/`project`/`repository`/`baseUrl` denormalized onto the top level for any provider).
 ...
 *
 * Takes the nested `{ provider?, location, codeOwner? }` shape (ticket #6 — no flat
 * `{ organization, project, repository, baseUrl?, codeOwner? }` fallback; `provider` defaults to
 ...
 * is registered yet. Takes the nested `{ provider?, location }` shape (ticket #6 — no flat fallback).
