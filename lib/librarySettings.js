 ...
 *
 * Every function still accepts (and every returned Azure DevOps record still carries) the pre-#3
 * flat `organization`/`project`/`repository`/`baseUrl` fields — no existing consumer
 * (`lib/definitionPromote.js`, the pre-#3 callers in `lib/server.js`) is migrated by this ticket. A
 * caller that wants a GitHub library repo (#19), or wants to address a record by its nested shape
