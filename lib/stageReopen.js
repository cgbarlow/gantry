...
import { readInstance, recordInstanceReopened, instanceDefinitionVersion, azureDevOpsInstancePath } from './instance.js'
 ...
 *
 * Workspace-backed instances only — `options.azureDevOps` is required.
 * Throws on:
 * - unknown stage
...
export async function reopenStage(slug, stageId, options = {}) {
  ...
  if (!azureDevOpsBase) {
    throw new Error('reopenStage is for Workspace-backed instances only — pass options.azureDevOps')
  }
