...
// completely silent — the process started normally, nothing was logged, and the eventual symptom (a
// registered workspace showing no designs, because the PAT map was set on the web server instead of
// the MCP server) pointed nowhere near the cause. #129's `GANTRY_MCP_` rename makes that particular
// mistake less likely to be *made*; this module makes the whole class of it *detectable*, at the one
// moment an operator is still looking at the deploy log.
//
...
//      legitimately set variables for their own tooling. Same fire-and-forget posture as
//      `lib/server.js`'s startup library-repo refresh — warn and carry on.
...
//
// This is deliberately duplicated in `mcp-server/src/envVarCheck.js` rather than shared: the two live
// in different packages that this codebase does not import across (see `lib/workspaceBootstrap.js`'s
// own doc comment for the same decision made for the same reason). `tests/envVarCheck.test.js` holds
// the two copies' tables to each other so the duplication cannot drift.

/**
 * The `GANTRY_`-prefixed environment variables the Gantry web service (`gantry serve`, i.e. everything
 * reachable from `bin/gantry.js` and `lib/`) actually reads. `tests/envVarCheck.test.js` greps the
 * source for `process.env.GANTRY_*` reads and asserts this list matches them exactly, in both
 * directions — a new variable cannot silently fall out of the check, and a removed one cannot silently
 * linger in it.
 *
 * `PORT` is read too (`bin/gantry.js`), but only `GANTRY_`-prefixed variables are ever considered here:
 * an operator's environment is full of unrelated variables, and warning about any of them would bury
 * the one line that matters.
 */
export const READ_ENV_VARS = [
...
export const OTHER_SERVICE_LABEL = 'the Gantry MCP server'

/**
 * What the other service reads (`mcp-server/src/index.js`, post-#129). Set on *this* service, one of
 ...
 */
export const OTHER_SERVICE_ENV_VARS = ['GANTRY_MCP_ACCESS_TOKEN', 'GANTRY_MCP_BASE_URL', 'GANTRY_MCP_WORKSPACE_PATS']
...
/** Log-line prefix, matching the existing `gantry serve: ...` startup lines in `lib/server.js`. */
const LINE_PREFIX = 'gantry serve:'
 ...
 * Both this service's own variables and the other service's are candidates: a near-miss on an MCP
 * variable name is still most usefully answered with the name that was probably meant.
 */
