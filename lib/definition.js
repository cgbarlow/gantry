...
export function findDefinitionProblemsInStructure(data) {
  ...
  const artefacts = (data.artefacts ?? []).map((artefact) => ({ id: artefact.id, requires: artefact.requires ?? [], filename: artefact.filename }))
...
export function definitionVersionProjection(def) {
  ...
  const artefacts = def.artefacts.map((a) => {
    ...
    // use them projects exactly as it did before they existed.
    if (a.documentControl === false) base.documentControl = false
