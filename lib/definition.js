 ...
 *
 * Now also carries `latestPublished` and `versions` per definition for the version picker (#231).
 *
 * @param {{ definitionsDir?: string }} [options]
 * @returns {{ id: string, title: string, stages: { id: string, title: string }[], latestPublished: number|null, versions: { version:number, status:string }[] }[]}
 */
