...
const AUTHORING_ONLY_PROBLEM_TYPES = new Set(['unknown-artefact-gate', 'unknown-required-at-gate', 'invalid-required-at'])
...
//
// `stages`/`artefacts` carry `id` and `gate`; `modules` carry `id` and `fields: [{ id, requiredAt }]`
...
export function findDefinitionProblemsInStructure(data) {
  const stages = (data.stages ?? []).map((stage) => ({ id: stage.id, gate: stage.gate, modules: stage.modules ?? [] }))
