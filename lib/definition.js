...
/**
 * Structural problems with a definition: module references that don't
 * resolve, invalid field types, and required/required-at mutual-exclusivity
 * violations. Returns every problem found in one pass (never throws), so
 * `gantry validate` can report all of them instead of just the first.
 *
 ...
 * validating that all stage/artefact module references resolve. Fails fast
 * on the first structural problem found (see `findDefinitionProblems` for
 * the non-throwing, report-everything variant used by `gantry validate`).
 *
 * @param {string} definitionId
 * @param {{ definitionsDir?: string }} [options]
 */
export function loadDefinition(definitionId, options = {}) {
  ...
  const stages = (raw.stages ?? []).map((stage) => ({
    ...
    gate: stage.gate,
    modules: stage.modules ?? [],
    // The instance the web form's "Populate example text" button reads
    // from for this stage — optional, a stage with none just has no
    // example available.
...
/**
 * `gantry definitions` / `GET /api/definitions`: every definition available
 * in `definitionsDir`, sorted by id, with just what a picker screen needs to
 * render a choice — id, title, and each stage's id/title — not the full
 * `loadDefinition` shape (module specs, artefacts), which is more than a
 * picker (the instance-setup wizard, #78) needs to show.
 *
 * @param {{ definitionsDir?: string }} [options]
