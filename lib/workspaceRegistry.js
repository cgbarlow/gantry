 ...
 * rewritten yet — that read-forward path is unaffected by this ticket. `ticketingSystem` is a
 * separate, unrelated field (predating the Provider model) and keeps working exactly as before.
 */
...
// The public record: the nested `provider`/`location`/`owner` shape. `ticketingSystem` is still
// denormalized onto the top level for an Azure DevOps workspace — it's a separate, pre-#3 field
// (predating the Provider model, see `lib/server.js`'s Settings-tab route) rather than a location
// accessor, and ticket #6 only removes the latter (`organization`/`project`/`repository`/`baseUrl`
// no longer appear anywhere but under `location`).
function toPublicWorkspace(id, record) {
  return {
    ...
    owner: record.owner ?? '',
    ...(record.provider === 'azure-devops' ? { ticketingSystem: DEFAULT_TICKETING_SYSTEM } : {}),
 ...
 *
 * Takes the nested `{ owner?, provider?, location?, ticketingSystem? }` shape (ticket #6 — no flat
 * `organization`/`project`/`repository`/`baseUrl` correction fields). `location` is merged onto the
 * existing one so a partial correction doesn't have to repeat every field. `ticketingSystem` is a
 * separate, unrelated field (predating the Provider model) still validated here exactly as before —
 * see this module's own `TICKETING_SYSTEMS` doc comment — independent of whether `location` is also
 * being updated.
 */
