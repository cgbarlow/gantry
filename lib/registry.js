...
// The GitHub twin of buildAzureDevOpsRow above (#11) — same "any read failure just omits this one row
// rather than failing the whole listing" contract and the same diagnostic breadcrumb. Simpler than its
// Azure DevOps counterpart: there is no stage-branch lifecycle for a GitHub-backed instance yet (#12),
// so every read here is against `main` directly — no findStageBranch, no pull-request summary (#13).
async function buildGitHubRow(slug, location, definitionsDir, pat, instancesDir) {
  ...
  try {
    ...
    const { complete } = await evaluateStage(definition, stage, slug, { github: githubBase })
...
export function listRegistry(options = {}) {
  ...
  const registered = listRegisteredInstances({ instancesDir: workspacesDir, registryPath: options.registryPath, includeArchived })
  ...
  return (async () => {
    ...
    for (const entry of registered) {
      ...
      if (entry.location.kind === 'directory') {
        row = buildLocalRow(entry.slug, directoryRowInstancesDir(workspacesDir, entry), definitionsDir, workspacesDir)
      } else if (entry.location.kind === 'github') {
        row = await buildGitHubRow(entry.slug, entry.location, definitionsDir, options.pat, workspacesDir)
      } else {
        row = await buildAzureDevOpsRow(entry.slug, entry.location, definitionsDir, options.pat, workspacesDir)
      }
