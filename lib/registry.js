...
function buildLocalRow(slug, instancesDir, definitionsDir) {
  ...
  return {
    ...
    // non-empty frontmatter `owner`. That module-level convention (Design
    // Authority sign-off, docs/adr/0001) is untouched; it just no longer
    // feeds this row.
    assignee: instance.assignee ?? '',
  }
}

// The row's `workspace` field (#102): every Azure-DevOps-backed row is
// grouped, on the dashboard, by the workspace (#96) backing its location —
// `{ organization, project, repository, baseUrl? }` is already resolved to
// a real, registered workspace by the time an instance is registered
// against it (see lib/instanceRegistry.js's normalizeAzureDevOpsLocation),
// so this is always found, never auto-created here. A local instance has
// no workspace at all (Workspace is an Azure-DevOps-repo concept only,
// per spec #95) — buildLocalRow simply never sets this field, rather than
// setting it to `null`, so a purely-local registry's `GET /api/instances`
// response is byte-for-byte unchanged from before this ticket.
...
async function buildAzureDevOpsRow(slug, location, definitionsDir, pat, instancesDir) {
  ...
  try {
    ...
    return {
      ...
      stage: stageId,
      status: complete ? 'complete' : 'incomplete',
      // See buildLocalRow's own comment — the instance record's own stored
      // `assignee` (#97), not a module-frontmatter-derived rollup.
      assignee: instance.assignee ?? '',
      // See rowWorkspace's own comment (#102) — the workspace this
      // instance's location belongs to, so the dashboard can group rows
      // by it.
 ...
 * `status` ('complete'/'incomplete') for that stage, and its stored
 * `assignee`.
 *
 ...
 * current-stage status) — this is the shape `GET /api/instances` wants,
 * not a replacement for that lower-level listing.
 *
 * `status` reflects the instance's *current* stage only — the same stage
 * `gantry status` would report against — not every stage the instance has
 * touched. `assignee` (#97) is a plain, stage-independent field on the
 * instance record itself, so it stays the same across stage transitions
 * rather than being recomputed per stage the way `status` is.
 *
 * An Azure-DevOps-backed row also carries a `workspace` field (#102) — the
 * `{ id, organization, project, repository, owner }` of the workspace
 * (lib/workspaceRegistry.js, #96) its location belongs to, so the
 * Workspaces dashboard can group rows by it. A local row has no
 * `workspace` field at all (Workspace is an Azure-DevOps-repo concept
 * only) — never `null`, so a purely-local registry's output shape is
