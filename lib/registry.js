...
/**
 * The registry: every instance gantry knows about, enriched with what a listing screen needs to render a row without fetching each instance individually — name (`slug`), `definition`, current `stage`, an overall `status` ('complete'/'incomplete') for that stage, its stored `assignee`, stage position/title, current-stage PR presence, and last-updated timestamp.
 *
...
export function listRegistry(options = {}) {
  ...
  if (!registered.some((entry) => entry.location.kind === 'azureDevOps')) {
    return registered
      ...
      .filter((row) => row !== null)
      .map((row) => withNumericRef(row, instancesDir))
  }
  ...
  return (async () => {
    ...
    for (const entry of registered) {
      ...
      if (row !== null) rows.push(withNumericRef(row, instancesDir))
    }
    return rows
  })()
}
