...
export function listRegistry(options = {}) {
  ...
  if (!registered.some((entry) => entry.location.kind === 'azureDevOps')) {
    return registered
      .map((entry) => {
        ...
        return row === null ? null : withArchivedFlag(withNumericRef(row, workspacesDir, entry.workspace), entry, includeArchived)
      })
      .filter((row) => row !== null)
  }
  ...
  return (async () => {
    ...
    for (const entry of registered) {
      ...
      if (row !== null) rows.push(withArchivedFlag(withNumericRef(row, workspacesDir, entry.workspace), entry, includeArchived))
    }
    return rows
  })()
}
