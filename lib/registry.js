...
function stageRowFields(definition, stage, instance) {
  return {
    ...
    // #188: the number recorded in instance.yaml at creation — consumed (and removed) by
    // withNumericRef below, never sent to a client as-is.
    persistedNumber: instance.number,
  }
}
...
function withNumericRef(row, workspacesDir, workspace) {
  const { persistedNumber, ...rest } = row
  const { workspaceNumber, instanceNumber } = instanceNumbersFor(row.slug, { instancesDir: workspacesDir, workspace, persistedNumber })
  return { ...rest, workspaceNumber, instanceNumber, ref: formatInstanceRef({ workspaceNumber, instanceNumber }) }
}

// #188: numbers `pending` rows (`{ row, entry }`, in listing order) with every row that carries a
// persisted number claiming it first, so an instance still on a lazily-assigned number can never take
// a number an instance recorded for itself at creation just because it happened to be listed earlier.
function numberRows(pending, workspacesDir, includeArchived) {
  const numbered = new Map()
  const claimsFirst = [...pending].sort((a, b) => Number(b.row.persistedNumber !== undefined) - Number(a.row.persistedNumber !== undefined))
  for (const item of claimsFirst) numbered.set(item, withNumericRef(item.row, workspacesDir, item.entry.workspace))
  return pending.map((item) => withArchivedFlag(numbered.get(item), item.entry, includeArchived))
}
...
function buildLocalRow(slug, instancesDir, definitionsDir, workspacesRootDir) {
  ...
  const row = {
    ...
    // when unset — the dashboard/switcher fall back to `slug` themselves rather than this row
    // baking that fallback in.
...
async function buildAzureDevOpsRow(slug, location, definitionsDir, pat, instancesDir) {
  ...
  try {
    ...
    const row = {
      ...
      assignee: instance.assignee ?? '',
      // See buildLocalRow's own comment — the instance's optional display name (#145), or `null`.
      name: instance.name ?? null,
 ...
 *
 * `options.workspaceId` (#131, parent #109, docs/adr/0047) scopes the whole listing — discovery
 * included — to that one workspace's own registered instances. This is what `GET
 * /api/workspaces/:workspaceId/instances` is built on: the browser holds one credential per workspace
 * (docs/adr/0038), so the only listing it can attach one to is a listing that names the workspace that
 * credential belongs to. Unset, the listing spans every registered instance exactly as before.
 * `options.surfaceAuthenticationErrors` goes with it — see `lib/instanceRegistry.js`'s
 * `backfillProviderInstances` for why a scoped caller wants a rejected credential back rather than
 * swallowed, and why an unscoped one never does.
 */
...
async function buildRegistryRows(registered, workspacesDir, definitionsDir, pat, includeArchived, sharedPats) {
  ...
  for (const entry of registered) {
    ...
    pending.push({ row, entry })
  }
  return numberRows(pending, workspacesDir, includeArchived)
}
...
export function listRegistry(options = {}) {
  ...
  // #131 (parent #109, docs/adr/0047): `options.workspaceId` scopes this whole listing — discovery
  // included — to one workspace's own entries. This is what makes a *credentialed* listing possible at
  // all without violating docs/adr/0038's "a credential's blast radius is one workspace": the
  // dashboard holds one credential per workspace, never a deployment-wide one, so the only listing it
  // can safely attach a credential to is one that names the workspace that credential belongs to.
  // Unset (every pre-#131 caller, `GET /api/instances` included) this is a no-op and the listing spans
  // every registered instance exactly as before.
  const scopeToWorkspace = (entries) =>
    options.workspaceId ? entries.filter((entry) => entry.scopeId === options.workspaceId) : entries
  ...
  // it did before #112.
  //
  // #133 (parent #109): `sharedPats` rides along on both calls too, so a workspace that was never
  // declared in `GANTRY_BOOTSTRAP_WORKSPACES` — registered instead through the New Workspace wizard,
  // or already present in the registry some other way — still gets discovered from its own
  // `GANTRY_SHARED_WORKSPACE_PATS` entry the first time any request reaches it, not only at boot.
  // `lib/instanceRegistry.js`'s `credentialFor` is what resolves each workspace's own entry (never a
  // different workspace's), and its `isDiscoveredEmpty`/`markDiscoveredEmpty` are what stop a
  // genuinely empty shared workspace from being re-listed on every subsequent request — the boot-time
  // path (`lib/workspaceBootstrap.js`'s `discoverBootstrapPatInstances`) is unaffected and keeps
  // running exactly as before.
  //
  // #131: `options.workspaceId` rides along on both calls, so a scoped listing only ever attempts its
  // own workspace's repo with the credential it was given — `backfillProviderInstances`'s own
  // `options.workspaceId` scoping is what enforces that, in the one place every caller shares.
  // `options.surfaceAuthenticationErrors` likewise: an unscoped listing keeps the pre-#131 swallow (a
  // rejected credential shortens the listing, never fails it); only a caller that asked for it gets
  // the rejection back.
  if (hasUndiscoveredProviderWorkspaces({ ...registryOptions, pat: options.pat, sharedPats, workspaceId: options.workspaceId })) {
    return (async () => {
      await backfillProviderInstances({
        ...registryOptions,
        pat: options.pat,
        sharedPats,
        workspaceId: options.workspaceId,
        surfaceAuthenticationErrors: options.surfaceAuthenticationErrors,
      })
      const registered = scopeToWorkspace(listRegisteredInstances(registryOptions))
