...
async function buildRegistryRow(entry, definitionsDir, pat, workspacesDir) {
  ...
  return buildRow(entry, definitionsDir, pat, workspacesDir)
}
...
// has in hand.
async function buildRegistryRows(registered, workspacesDir, definitionsDir, pat, includeArchived) {
  const rows = []
  for (const entry of registered) {
    const row =
      entry.location.kind === 'directory'
        ...
        : await buildRegistryRow(entry, definitionsDir, pat, workspacesDir)
...
export function listRegistry(options = {}) {
  ...
  if (hasUndiscoveredProviderWorkspaces({ ...registryOptions, pat: options.pat })) {
    return (async () => {
      ...
      return buildRegistryRows(registered, workspacesDir, definitionsDir, options.pat, includeArchived)
    })()
  }
