...
function localRowDefinitionsDir(instancesDir, definitionsDir, definitionId) {
  const workspaceDefinitionsDir = join(instancesDir, 'definitions')
  return storage.exists(join(workspaceDefinitionsDir, definitionId)) ? workspaceDefinitionsDir : definitionsDir
}
...
function buildLocalRow(slug, instancesDir, definitionsDir) {
  ...
  const definition = loadDefinition(definitionId, {
    definitionsDir: localRowDefinitionsDir(instancesDir, definitionsDir, definitionId),
...
export function listRegistry(options = {}) {
  ...
  if (!registered.some((entry) => entry.location.kind === 'azureDevOps')) {
    return registered
      .map((entry) => {
        const row = buildLocalRow(entry.slug, directoryRowInstancesDir(workspacesDir, entry), definitionsDir)
    ...
    for (const entry of registered) {
      const row =
        entry.location.kind === 'directory'
          ? buildLocalRow(entry.slug, directoryRowInstancesDir(workspacesDir, entry), definitionsDir)
          : await buildAzureDevOpsRow(entry.slug, entry.location, definitionsDir, options.pat, workspacesDir)
