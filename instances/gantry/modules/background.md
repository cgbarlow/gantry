...
- **Consistent stage gates and sign-off.** Business Case Approved, HLD approved at TAC and Build Readiness Confirmed are checked by the tool against the fields each artefact actually requires, so a gate means the same thing for every initiative.
